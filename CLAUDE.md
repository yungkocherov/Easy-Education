...
**Вкладка "Применение"** объединяет 5 блоков: `Где применяется / Сильные стороны / Ограничения / Когда применять — когда не стоит (таблица) / Альтернативы`. Старой вкладки `proscons` больше нет — она была слита в `applications` (см. коммит `c26d70e`).
