...
App.registerTopic({
  ...
  tabs: {
    theory: `
      ...
      <p>Это <span class="term" data-tip="ε-insensitive loss. Функция потерь, равная нулю если |y − f(x)| ≤ ε, и |y − f(x)| − ε иначе. Похожа на «мёртвую зону» вокруг предсказания.">ε-нечувствительные потери</span>: если отклонение ≤ ε — штраф равен нулю. Если больше — штраф линейный, как в MAE, но только сверх порога ε.</p>
      ...
      <ul>
        <li><b>Большой C (≥ 10):</b> модель старается вместить в трубу как можно больше точек. Труба «гнётся» и становится сложной. Риск переобучения.</li>
        <li><b>Маленький C (≤ 0.1):</b> модель предпочитает простоту, допускает выбросы снаружи трубы. Более гладкая кривая. Риск недообучения.</li>
      </ul>
      ...
      <ul>
        ...
        <li><b>Gradient Boosting регрессия</b> — сильнее на табличных данных, но медленнее обучается.</li>
      </ul>
    `,
    ...
    examples: [
      {
        ...
        content: `
          ...
          <div class="step" data-step="2">
            ...
            <div class="why">Итого штраф = C·(ξ_C + ξ_E) = C·1.4. Для C=1 штраф = 1.4. Сравни с MSE: (0.25+0.64+4.84+0.25+1.44)/5 = 1.48 — SVR игнорирует маленькие ошибки!</div>
          </div>
    ...
    proscons: `
      <div class="proscons">
        <div class="pros">
          <h4>✓ Плюсы</h4>
          <ul>
            <li>Устойчив к выбросам (ε-труба поглощает малые ошибки)</li>
            <li>Эффективен в высоких размерностях (текст, геномика)</li>
            <li>Ядерный трюк даёт нелинейные модели без явного расширения признаков</li>
            <li>Хороший выбор для малых/средних датасетов (до ~10 000 строк)</li>
            <li>Экономно по памяти: хранит только опорные векторы</li>
            <li>Явная «зона безразличия» — полезна когда шум известен</li>
          </ul>
        </div>
        <div class="cons">
          <h4>✗ Минусы</h4>
          <ul>
            <li>Медленно на больших данных: O(n²)–O(n³)</li>
            <li>Обязательная стандартизация признаков</li>
            <li>Три параметра для подбора (C, ε, γ) — дорогой GridSearch</li>
            <li>Не выдаёт вероятности (только точечный прогноз)</li>
            <li>Плохо интерпретируем при нелинейном ядре</li>
            <li>Чувствителен к масштабу целевой переменной (нужно масштабировать y)</li>
          </ul>
        </div>
      </div>

      <h3>SVR vs другие регрессоры</h3>
      <div class="example-data-table">
        <table>
          <tr><th>Критерий</th><th>SVR</th><th>Linear Reg</th><th>Random Forest</th><th>Gradient Boosting</th></tr>
          <tr><td>Нелинейность</td><td>Ядро</td><td>Нет</td><td>Да</td><td>Да</td></tr>
          <tr><td>Выбросы</td><td>Устойчив (ε)</td><td>Чувствителен</td><td>Устойчив</td><td>Умеренно</td></tr>
          <tr><td>Масштабирование</td><td>Обязательно</td><td>Желательно</td><td>Не нужно</td><td>Не нужно</td></tr>
          <tr><td>Интерпрет.</td><td>Нет (RBF)</td><td>Высокая</td><td>Средняя</td><td>Средняя</td></tr>
          <tr><td>Скорость обуч.</td><td>O(n²–n³)</td><td>O(np)</td><td>O(n·p·log n·T)</td><td>O(n·p·T)</td></tr>
          <tr><td>Большие данные</td><td>Плохо</td><td>Хорошо</td><td>Хорошо</td><td>Хорошо</td></tr>
        </table>
      </div>
    `,
  },
