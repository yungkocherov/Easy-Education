...
for name, pipe in pipelines.items():
    ...
    applications: `
      ...
      <ul>
        <li><b>Классификация коротких текстов и спам-фильтры.</b> TF-IDF + LinearSVC/LogReg до сих пор рабочая лошадка для модерации комментариев, детекции спама, категоризации статей и тикетов. Обучается за секунды, интерпретируется по словам, не требует GPU — и для 80% подобных задач этого достаточно.</li>
        <li><b>Информационный поиск и сортировка документов.</b> BM25 (наследник TF-IDF) лежит в основе Elasticsearch, Lucene, Solr и большинства корпоративных поисковых систем. Даже современные гибридные RAG-системы начинают с TF-IDF/BM25 и только потом ранжируют через эмбеддинги.</li>
        <li><b>Content-based рекомендации.</b> Подбор похожих статей, фильмов, товаров по TF-IDF-векторам описаний и cosine similarity. Простой, объяснимый и работающий подход для старта контентной рекомендательной системы.</li>
        <li><b>Анализ тональности и модерация.</b> Детекция токсичности, оценка отзывов, классификация обращений по эмоциональной окраске. Для общих случаев TF-IDF-модель справляется; для тонких случаев (сарказм, контекст) переходят на BERT.</li>
        <li><b>Feature engineering для табличных моделей.</b> Если в твоих данных есть текстовые колонки (описание товара, комментарий клиента), TF-IDF превращает их в численные фичи для XGBoost/LightGBM. Самый быстрый способ добавить текст в классическую ML-модель.</li>
        <li><b>Быстрый baseline перед запуском тяжёлой модели.</b> Золотое правило NLP: сначала TF-IDF + LinearSVC за 5 минут, потом смотри, нужен ли BERT. Часто baseline оказывается в пределах 2–3% от SOTA, а стоимость — на порядки ниже.</li>
        <li><b>Препроцессинг для эмбеддингов.</b> Токенизация, стемминг, нормализация, удаление стоп-слов — основа любого NLP-пайплайна, включая современные Transformer-модели. BPE-токенизация GPT — это эволюция тех же классических идей.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Мгновенное обучение и inference.</b> TF-IDF + LogReg обучается на 100k документов за секунды на ноутбуке. Нет GPU, нет подбора гиперпараметров, нет часов обучения. Для итеративной разработки, прототипирования и запуска MVP это критично.</p>
      <p><b>Отличная интерпретируемость.</b> Можно посмотреть, какие слова дали наибольший вклад в классификацию: «бесплатно», «срочно», «заработок» → спам. Объяснять решения продакту, регулятору и клиенту не просто можно, а легко. У BERT этого преимущества нет.</p>
      <p><b>Работает на малых датасетах.</b> TF-IDF + LinearSVC стабильно даёт приличные результаты на 500–10 000 примеров. BERT на той же выборке либо переобучится, либо потребует полноценного fine-tuning с осторожной регуляризацией.</p>
      <p><b>Не требует предобученных моделей.</b> Работает на любом языке, любом домене, любом жаргоне — достаточно обучающего корпуса. Для редких языков и специфических доменов (медицинский жаргон, юридический сленг) это часто единственный рабочий путь.</p>
      <p><b>Лучше на длинных документах.</b> BERT имеет жёсткое ограничение в 512 токенов и начинает «терять» контекст на длинных текстах. TF-IDF нативно агрегирует всю частотность документа — и на технических отчётах, статьях, книгах часто обгоняет эмбеддинги.</p>
      <p><b>Фундаментальный базис для современных методов.</b> BPE-токенизация, субсловные представления, attention — всё это выросло из классического NLP. Понимать базовые методы = понимать, почему Transformer работает так, как работает.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Bag-of-Words и TF-IDF игнорируют порядок слов.</b> «Не очень хорошо» и «очень не хорошо» дают один и тот же вектор. Для тонкой семантики, отрицаний, сарказма этого катастрофически не хватает — нужны n-gram или переход на Transformer.</p>
      <p><b>Нет семантической близости.</b> «Кот» и «котёнок» — два разных измерения в TF-IDF с нулевой корреляцией, хотя по смыслу почти одинаковы. Word2Vec и BERT решают это через плотные эмбеддинги, но цена — потеря интерпретируемости.</p>
      <p><b>OOV (out-of-vocabulary) слова.</b> Если слово не встречалось на обучении, TF-IDF просто его игнорирует. Для пользовательского контента с опечатками, сленгом, неологизмами это проблема. FastText (char n-grams) и BPE в Transformer решают это нативно.</p>
      <p><b>Высокая размерность и разреженность.</b> TF-IDF для 100k документов даёт матрицу на десятки тысяч признаков. Для KNN, k-means, PCA на плотных векторах это плохо — <a class="glossary-link" onclick="App.selectTopic('glossary-curse-dimensionality')">проклятие размерности</a> бьёт в полную силу.</p>
      <p><b>Проигрывает Transformer-моделям на сложных задачах.</b> Sentiment с контекстом, NER, вопросно-ответные системы, машинный перевод — везде BERT и LLM заметно опережают TF-IDF. Для SOTA-результатов на NLU-задачах классика не конкурент.</p>
      <p><b>Чувствительна к препроцессингу.</b> Результаты сильно зависят от токенизатора, стеммера, стоп-листа, удаления пунктуации. Ошибка в препроцессинге может ухудшить модель сильнее, чем выбор самого алгоритма — и это бывает трудно найти.</p>

      <h3>🧭 Когда брать классические NLP-методы — и когда точно не стоит</h3>
      <table>
        <tr><th>✅ Бери TF-IDF / BoW / Word2Vec когда</th><th>❌ НЕ бери когда</th></tr>
        <tr>
          <td>Нужен быстрый baseline за 5 минут без GPU и подбора гиперпараметров</td>
          <td>Нужен SOTA-результат на NER, QA, sentiment с тонким контекстом</td>
        </tr>
        <tr>
          <td>Датасет маленький (500–10 000 документов) — BERT переобучится</td>
          <td>Задача требует понимания порядка слов, отрицаний, сарказма</td>
        </tr>
        <tr>
          <td>Документы длинные (статьи, отчёты, книги) — BERT упрётся в 512 токенов</td>
          <td>Нужна семантическая близость «кот»/«котёнок» — плотные эмбеддинги лучше</td>
        </tr>
        <tr>
          <td>Требуется интерпретируемость — объяснить регулятору, клиенту или продакту</td>
          <td>Пользовательский контент с опечатками, сленгом, неологизмами (OOV)</td>
        </tr>
        <tr>
          <td>Редкий язык или узкий домен без доступных предобученных моделей</td>
          <td>Есть GPU и тысячи примеров — Transformer почти наверняка обгонит</td>
        </tr>
        <tr>
          <td>Text-фичи для табличной модели (XGBoost, LightGBM)</td>
          <td>Задача мультиязычная и требуется cross-lingual transfer</td>
        </tr>
        <tr>
          <td>Поисковая система и ранжирование — BM25 работает отлично</td>
          <td>Нужна генерация текста — классика на это не способна в принципе</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы — что взять вместо и почему</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('transformer')">Transformer</a> (BERT, RoBERTa, DeBERTa)</b> — если нужна SOTA на NLU-задачах, есть GPU и тысячи примеров. Понимает контекст, порядок слов, семантику.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('glossary-embedding')">Word2Vec / FastText / GloVe</a></b> — если нужны плотные векторные представления слов без тяжёлого Transformer. FastText решает проблему OOV через char n-grams.</li>
        <li><b>Sentence-BERT / sentence-transformers</b> — для семантического поиска и похожести предложений. Быстрее BERT на inference, лучше TF-IDF по семантике.</li>
        <li><b>BM25 / Elasticsearch</b> — если нужен именно информационный поиск, а не классификация. Эволюция TF-IDF с учётом длины документа.</li>
        <li><b>Гибридный подход (BM25 + dense retrieval)</b> — стандарт современных RAG-систем: TF-IDF/BM25 для recall, эмбеддинги для precision. Лучшее из обоих миров.</li>
      </ul>
    `,
    links: `
      <h3>📺 Видео</h3>
