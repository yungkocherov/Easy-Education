...
App.registerTopic({
  ...
  tabs: {
    theory: `
      ...
      <h3>Типичные ошибки продуктовой аналитики</h3>
      <ul>
...
plt.show()</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>SaaS/B2B: MRR, churn, activation, feature adoption.</b> Понять, какие фичи двигают retention, где обрыв в onboarding, какие сегменты клиентов держатся дольше. Стандартный вопрос: «почему у нас churn 8%/месяц» — и ответ лежит ровно в воронке активации и когортном retention.</li>
        <li><b>E-commerce: конверсия, AOV, funnel drop-off.</b> На какой странице пользователи отваливаются, сколько добавлений в корзину заканчиваются оплатой, когда запускать триггерные письма о брошенной корзине. <a class="glossary-link" onclick="App.selectTopic('glossary-funnel')">Воронка</a> покупки — центральный объект анализа.</li>
        <li><b>Mobile apps: D1/D7/D30 retention, session length, ARPU.</b> Первая неделя — критическая. Если D1 retention &lt; 20%, продукт «течёт» быстрее, чем наполняется — нет смысла лить трафик. Когортный анализ показывает, улучшается ли это со временем или фича добавлена зря.</li>
        <li><b>Social/media: DAU/MAU, stickiness, time spent.</b> Основная метрика здоровья — соотношение DAU/MAU (sticky ratio). Ниже 20% — приложение «открывают раз в неделю», выше 50% — ежедневный habit. Определяет, стоит ли инвестировать в монетизацию или ещё работать над PMF.</li>
        <li><b>Marketplaces: GMV, take rate, ликвидность, network effects.</b> Двустороннее уравнение: продавцы уйдут без покупателей, покупатели — без товаров. Продуктовая аналитика отслеживает баланс: время поиска, fill rate, повторные покупки, cross-side growth.</li>
        <li><b>Gaming: K-factor, ARPPU, monetization funnel.</b> Свободный игрок vs платящий (whales). Основная метрика — LTV против CAC в разрезе канала и когорты. Retention по уровням показывает, на каком этапе люди бросают игру.</li>
        <li><b>FinTech и banking: активация счёта, первая транзакция, cross-sell.</b> Регистрация ничего не стоит — важна «активация» (первое внесение, первая карта, первый перевод). Метрики времени до активации и доля активированных за 7 дней — главные KPI growth-команды.</li>
      </ul>

      <h3>Сильные стороны — и почему они важны</h3>
      <p><b>Превращает интуицию в измеримые решения.</b> «Мне кажется, новая фича популярна» → «adoption 12% за 2 недели при норме 25% — не взлетела, надо копать». Без аналитики команды спорят мнениями; с аналитикой — цифрами. Это не «для отчётов», это инструмент принятия решений.</p>
      <p><b><a class="glossary-link" onclick="App.selectTopic('glossary-cohort-analysis')">Когортный анализ</a> честно показывает динамику продукта.</b> Общий retention может выглядеть стабильным, хотя продукт деградирует: старые когорты держат среднее, новые — хуже. Разрез по неделям регистрации моментально это вскрывает. Единственный способ увидеть, работает ли то, что вы делаете.</p>
      <p><b>Funnel-анализ локализует проблему.</b> Из 100% зашедших 40% регистрируются, 20% активируются, 5% покупают — видно, что главная утечка на шаге активации, а не регистрации. Без воронки команда тратит месяцы на оптимизацию шага с потенциалом +1%, вместо шага с потенциалом +15%.</p>
      <p><b>Retention → Activation → Revenue → Acquisition → Referral.</b> Правильный приоритет (pirate metrics в обратном порядке). Лить трафик в «дырявое ведро» — сжигать деньги. Сначала убедись, что пользователи остаются, потом что активируются, потом зарабатывай — и только потом масштабируй acquisition. Эта последовательность экономит стартапам миллионы.</p>
      <p><b>Даёт общий язык для product, marketing, finance, exec.</b> LTV, CAC, payback period — не ML-термины, их понимают CFO и CEO. Аналитик, умеющий считать unit-экономику, становится мостом между командами и автоматически выигрывает в влиянии.</p>
      <p><b>Быстро и дёшево по сравнению с ML и causal inference.</b> SQL + dashboard в Amplitude/Mixpanel/Metabase → ответ за день. ML-модель того же качества — месяц работы. Для 80% product-вопросов аналитики на SQL-уровне достаточно, и это правильный default.</p>

      <h3>Ограничения — и когда они реально бьют</h3>
      <p><b>Vanity metrics вводят в заблуждение.</b> DAU растёт → «продукт растёт». А это просто маркетинг привёл больше регистраций, и все они отвалятся через неделю. Настоящий health check — retention по когортам и return ratio, а не абсолютные числа. Хороший аналитик отличает одно от другого; плохой — рисует растущий график в дашборде CEO.</p>
      <p><b>Корреляция — не причинность: аналитика отвечает «что», а не «почему».</b> «Пользователи, использовавшие фичу X, имеют retention 80% vs 40% у остальных» — не значит, что фича X повышает retention. Возможно, её находят только самые активные. Причинный ответ требует <a class="glossary-link" onclick="App.selectTopic('ab-testing-intro')">A/B-теста</a> или <a class="glossary-link" onclick="App.selectTopic('causal-inference')">causal inference</a>. Путать эти вещи — главная ошибка junior-аналитиков.</p>
      <p><b>Плохое определение «активного пользователя» искажает всё.</b> Activity = «открыл приложение» → завышает DAU (случайные свайпы). Activity = «сделал ключевое действие» → даёт реальную картину, но требует договорённости с продактом. Любой дашборд начинается с этого определения, и его нужно защищать от размывания.</p>
      <p><b>LTV-формулы часто основаны на нереалистичных допущениях.</b> Стандартная $LTV = ARPU / churn$ предполагает постоянный churn на бесконечном горизонте. В реальности churn сильно варьируется по когортам и снижается со стажем. Для молодого продукта LTV — оценка в вилке, а не число.</p>
      <p><b>Сегментация без достаточных данных — иллюзия инсайта.</b> «Пользователи из Москвы старше 35 с iOS имеют retention 67%» — если в этом сегменте 12 человек, это шум. Нужны доверительные интервалы и sanity check по размеру подвыборки. Иначе продакт будет принимать решения по артефактам.</p>
      <p><b>Аналитика не заменяет качественные исследования.</b> Цифры показывают, что 40% бросают на шаге онбординга. Почему? Дашборд молчит. Интервью и usability-тесты за час объясняют то, что дата-команда ищет месяц. Методы дополняют друг друга, а не конкурируют.</p>

      <h3>Когда применять — и когда не стоит</h3>
      <table>
        <tr><th>Хватит продуктовой аналитики когда</th><th>НЕ хватает — нужны другие методы когда</th></tr>
        <tr>
          <td>Вопрос «что происходит»: где утечка, какой сегмент растёт, какова конверсия</td>
          <td>Вопрос «повлияла ли фича»: нужен <a class="glossary-link" onclick="App.selectTopic('ab-testing-intro')">A/B-тест</a>, аналитика корреляций не доказывает causation</td>
        </tr>
        <tr>
          <td>Нужен быстрый ответ для принятия решения сегодня/завтра</td>
          <td>Нужна количественная оценка эффекта с CI и защитой от конфаундеров → <a class="glossary-link" onclick="App.selectTopic('causal-inference')">causal inference</a></td>
        </tr>
        <tr>
          <td>Команде нужен общий язык метрик и unit-экономики</td>
          <td>Вопрос «почему» пользователи делают X: интервью, usability tests, сессионные записи</td>
        </tr>
        <tr>
          <td>Мониторинг здоровья продукта и раннее обнаружение деградаций</td>
          <td>Персонализация и предсказания на уровне отдельного пользователя — нужен ML (churn prediction, recsys)</td>
        </tr>
        <tr>
          <td>Диагностика воронки и когорт — локализация проблемы перед A/B-тестом</td>
          <td>Политические/этические решения (приватность, ценообразование) — аналитика подсветит trade-off, но не решит</td>
        </tr>
        <tr>
          <td>Бизнес-отчёты и коммуникация с execs: LTV, CAC, payback, retention curves</td>
          <td>Очень мало пользователей (&lt;1000): метрики слишком шумные, lean интервью быстрее</td>
        </tr>
      </table>

      <h3>Альтернативы</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('ab-testing-intro')">A/B-тестирование</a></b> — когда нужно доказать, что изменение реально вызвало эффект. Аналитика строит гипотезы, A/B их проверяет. Они не заменяют друг друга — это два этапа одного процесса.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('causal-inference')">Causal inference</a></b> — когда A/B-тест невозможен (эффект слишком долгий, раскатка на всех, этика). DID, PSM, RDD дают каузальные оценки на наблюдательных данных.</li>
        <li><b>Качественные исследования (интервью, usability)</b> — когда нужен ответ «почему». Цифры скажут, что воронка ломается на шаге 3; только интервью объяснит, что форма непонятна.</li>
        <li><b>ML для churn prediction и LTV</b> — когда нужны индивидуальные прогнозы, а не агрегаты. Классификатор на gradient boosting предскажет риск оттока конкретного пользователя — аналитика так не умеет.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('hypothesis-testing')">Статистические тесты</a> поверх когорт</b> — чтобы отличить реальные отличия между сегментами от шума. Без этого сегментация легко превращается в поиск паттернов в случайности.</li>
      </ul>
    `,
