...
print(f"Лучший accuracy: {search.best_score_:.4f}")</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>Kaggle-соревнования.</b> На верхних местах разница между командами часто 0.001 ROC-AUC — и эта разница достигается подбором гиперпараметров. Optuna стал стандартом: один блок кода на сутки Hyperband — и модель улучшается на 2-5%.</li>
        <li><b>AutoML-системы.</b> Auto-sklearn, H2O AutoML, Google Vertex AutoML, FLAML под капотом — это Bayesian Optimization + Hyperband + meta-learning. Вся индустрия «ML без кода» построена на автоподборе гиперов.</li>
        <li><b>Обучение нейронных сетей.</b> Learning rate, batch size, weight decay, dropout, warmup-шаги, размер эмбеддинга, число слоёв — десятки ручек. Ray Tune, Optuna, Weights &amp; Biases Sweeps — стандартные инструменты для distributed tuning.</li>
        <li><b>Продуктовые ML-пайплайны.</b> В проде модели переобучаются по расписанию (еженедельно, ежедневно). Автоподбор гиперов на свежих данных не даёт метрикам уехать при concept drift — без этого за месяц качество может просесть на 10%.</li>
        <li><b>Медицина и биоинформатика.</b> Когда данных сотни, а не миллионы, неправильные гиперы ведут к переобучению. Здесь обязателен Nested CV: внешние фолды для честной оценки, внутренние — для тюнинга.</li>
        <li><b>NLP и большие трансформеры.</b> Population Based Training (PBT) от DeepMind — параллельное эволюционное обучение агентов с подгонкой гиперов на лету. Используется для RLHF, LoRA-тюнинга, длинных тренировок.</li>
        <li><b>A/B-тесты архитектур.</b> Не «какая модель лучше» в принципе, а «какая модель лучше ПРИ СВОИХ оптимальных гиперпараметрах» — честное сравнение требует тюнинга каждой.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Потенциал улучшения 5-15% «бесплатно».</b> Без изменения модели, данных, препроцессинга. Это самый дешёвый способ выжать из готовой архитектуры максимум — и часто разница между «проходит в прод» и «не проходит».</p>
      <p><b>Автоматизация рутины.</b> Раньше выбор learning rate был искусством — сейчас это 20 строк Optuna. ML-инженер освобождается от ручного подбора и концентрируется на фичах и архитектуре.</p>
      <p><b>GridSearchCV: простота и гарантии.</b> В малом дискретном пространстве (3-4 параметра по 5 значений) ты гарантированно найдёшь лучшую комбинацию. Никаких сюрпризов, легко объяснить, легко воспроизвести.</p>
      <p><b>Random Search: дёшево и почти оптимально.</b> Статья Bergstra &amp; Bengio (2012) показала: при больших размерностях случайный поиск эффективнее сетки, потому что не тратит бюджет на «бесполезные» размерности. Плюс — можно остановить в любой момент.</p>
      <p><b>Bayesian Optimization / TPE (Optuna, Hyperopt).</b> Умные методы строят модель над функцией качества и выбирают следующую точку там, где ожидаемое улучшение максимально. На дорогих обучениях (нейросети) это в разы эффективнее Random.</p>
      <p><b>Hyperband и Successive Halving.</b> Запускают много конфигураций с маленьким бюджетом, потом режут слабые и дают ресурсы выжившим. Идеально для нейросетей, где обучение можно прервать по validation loss.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Curse of dimensionality в GridSearch.</b> 6 параметров по 5 значений — 15625 комбинаций. Каждая — обучение модели с кросс-валидацией. Это дни-недели вычислений. Практически GridSearch применим только для 2-3 параметров.</p>
      <p><b>Переобучение на валидации при огромном числе триалов.</b> Если ты перебрал 10 000 конфигураций на одной validation выборке, лучший CV-скор будет смещён оптимистично: ты нашёл не лучшую модель, а лучший способ обмануть этот конкретный валидационный сет. Защита — Nested CV или финальный hold-out.</p>
      <p><b>Вычислительные затраты огромны.</b> Для LLM или большой CNN одно обучение может стоить дни на кластере. Полноценный HPO в таких условиях — недоступная роскошь для большинства команд. Приходится переносить параметры из литературы или похожих задач.</p>
      <p><b>Оптимальные параметры не переносятся.</b> learning rate, который идеален на датасете A, провалится на B. Любой HPO — это локальная оптимизация под конкретные данные, и её нельзя кэшировать «на будущее».</p>
      <p><b>Random Search может пропустить оптимум.</b> Если бюджет маленький, а оптимум узкий, RS может просто не попасть в нужную область. Bayesian методы надёжнее, но сложнее.</p>
      <p><b>Ручной feature engineering часто важнее.</b> Кремлёвский баланс между временем на HPO и временем на фичи — и обычно фичи выигрывают. Подбор гиперов не спасёт модель с плохими признаками.</p>

      <h3>🧭 Какой метод тюнинга брать — и когда вообще не стоит</h3>
      <table>
        <tr><th>✅ Используй когда</th><th>❌ НЕ используй когда</th></tr>
        <tr>
          <td><b>GridSearch</b> — 2-3 параметра с явными кандидатами (kernel, C, degree в SVM)</td>
          <td>5+ параметров с непрерывными диапазонами — взорвёт бюджет</td>
        </tr>
        <tr>
          <td><b>Random Search</b> — много параметров, ограниченный бюджет, быстрое обучение одной модели</td>
          <td>Оптимум узкий, бюджет маленький — RS может не попасть</td>
        </tr>
        <tr>
          <td><b>Optuna / TPE (Bayesian)</b> — дорогое обучение (NN, бустинг на больших данных), &gt;3 параметров</td>
          <td>Параметров 1-2 и они дешёвые — излишняя сложность, проще GridSearch</td>
        </tr>
        <tr>
          <td><b>Hyperband / ASHA</b> — нейросети, можно прервать обучение по validation loss</td>
          <td>Модель не поддерживает inкрементальное обучение (SVM, k-NN) — Hyperband бесполезен</td>
        </tr>
        <tr>
          <td><b>Nested CV</b> — публикация, медицинское исследование, Kaggle private leaderboard</td>
          <td>Прод-пайплайн, где нужно просто быстро получить «хорошую» модель — оверинжиниринг</td>
        </tr>
        <tr>
          <td><b>Никакого тюнинга (дефолты)</b> — быстрый baseline, прототип, MVP</td>
          <td>Финальная боевая модель, где 2% метрики = миллионы рублей</td>
        </tr>
        <tr>
          <td><b>PBT / Population Based Training</b> — длинные обучения (дни-недели), RLHF, большие NN</td>
          <td>Обычное табличное ML — слишком тяжёлая инфраструктура</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы — что взять вместо и почему</h3>
      <ul>
        <li><b>Разумные дефолты + небольшой тюнинг.</b> Для XGBoost/LightGBM дефолты почти всегда дают 90% от максимума. Потрать время на фичи и данные, это окупится сильнее.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('cross-validation')">Nested cross-validation</a></b> — если публикуешь результаты, это обязательно: внешний CV для честной оценки, внутренний для тюнинга. Без него оценка оптимистична.</li>
        <li><b>Transfer learning и pretrained параметры.</b> Для нейросетей чаще эффективнее взять готовую предобученную модель, чем тюнить свою с нуля.</li>
        <li><b>Meta-learning и warm-starting.</b> Hyperopt, FLAML, Auto-sklearn умеют переносить знание с похожих задач: не начинают поиск с нуля, а стартуют из «хороших» зон.</li>
        <li><b>Ручной подбор по learning curves.</b> Иногда один взгляд на график train/val ошибки даёт больше, чем 1000 триалов автоподбора: сразу видно, нужно ли усложнять модель или наоборот.</li>
      </ul>
    `,
