  ...
  tabs: {
    theory: `
      <div class="intuition">
        ...
        <p>t-тест отвечает именно на этот вопрос: «Разница средних реальна, или она могла возникнуть случайно из-за случайного разброса данных?» Он нормирует разность средних на <a class="glossary-link" onclick="App.selectTopic('glossary-standard-error')">стандартную ошибку</a> — и переводит вопрос в единицы «насколько стандартных ошибок».</p>
      </div>
      ...
      <div class="callout tip">💡 Если медиана значительно меньше среднего — данные скошены, рассмотри log-transform или <a class="glossary-link" onclick="App.selectTopic('glossary-nonparametric-tests')">непараметрический</a> Манн-Уитни.</div>

      <h3>📏 Размер эффекта: <a class="glossary-link" onclick="App.selectTopic('glossary-effect-size')">Cohen's d</a></h3>
...
plt.show()</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>A/B тесты непрерывных метрик.</b> Средний чек, ARPU, время на сайте, количество сессий на пользователя — всё, где метрика принимает много разных числовых значений, а не 0/1. Самый распространённый тест в продуктовой аналитике.</li>
        <li><b>Клинические испытания с малыми выборками.</b> Сравнение уровня холестерина, давления, биомаркеров до/после лечения. Именно из пивоваренной задачи Гиннесса (Госсет, 1908) t-тест и родился — там выборки в десятки наблюдений.</li>
        <li><b>Paired t-test для до/после.</b> Один пользователь, два измерения: до обучения и после, до редизайна и после. Парность убирает межиндивидуальную дисперсию и резко поднимает мощность.</li>
        <li><b>ML-benchmarks.</b> Сравнение accuracy двух моделей на $k$ прогонах cross-validation — paired t-test по фолдам. Единственный честный способ сказать «модель A статистически лучше B на этом датасете».</li>
        <li><b>Контроль качества (one-sample).</b> Соответствует ли средний параметр производства эталону (толщина детали, концентрация реагента)?</li>
        <li><b>Education & HR research.</b> Сравнение двух программ обучения, двух воронок найма, эффекта тренинга — классическое поле применения t-теста.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Работает при малых выборках.</b> Распределение Стьюдента с $\\nu$ степенями свободы специально учитывает, что при малом $n$ оценка дисперсии $s^2$ сама по себе шумная. У $t$ тяжелее хвосты, чем у нормального, — это даёт корректный контроль ошибки I рода даже при $n=10$, чего z-тест обеспечить не может.</p>
      <p><b>Робастность Welch к неравным дисперсиям.</b> Реальные A/B часто дают $s_A \\neq s_B$ (treatment меняет не только среднее, но и разброс). Welch-версия не предполагает равенства дисперсий и считает эффективные степени свободы по формуле Саттертуэйта — корректна в обоих случаях, поэтому в sklearn/scipy/statsmodels это default.</p>
      <p><b>Устойчивость к лёгкой ненормальности.</b> Благодаря ЦПТ среднее выборки сходится к нормальному даже из ненормального распределения. При $n \\geq 30{-}50$ t-тест остаётся корректен для умеренно скошенных данных — строгая нормальность не обязательна.</p>
      <p><b>Прямая интерпретация разности и CI.</b> Выход — разность средних с доверительным интервалом в тех же единицах, что и метрика («среднее выросло на 47 ₽, CI [18; 76]»). Бизнес моментально переводит это в деньги, в отличие от абстрактного p-value.</p>
      <p><b>Cohen's d как мера эффекта.</b> $d = (\\bar{x}_B - \\bar{x}_A)/s_p$ не зависит от $n$ — показывает «насколько далеко» группы разошлись в единицах стандартного отклонения. При миллионах пользователей p-value всегда микроскопическое, а $d$ подсказывает, велик ли эффект на практике.</p>
      <p><b>Универсальность семейства.</b> Одно- и двухвыборочный, парный, Welch — одна идея и одна интерпретация покрывают большинство сценариев сравнения средних в аналитике.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Чувствительность к выбросам.</b> Среднее нелинейно зависит от экстремумов: один чек на миллион искажает $\\bar{x}$ и раздувает $s$. В LTV/ARPU-данных с тяжёлыми хвостами это приводит либо к ложным срабатываниям, либо к потере мощности. Помогают: винсоризация, log-трансформация, bootstrap, либо переход на Манна-Уитни.</p>
      <p><b>Не работает для бинарных метрик.</b> Конверсия — это пропорция, не среднее непрерывной величины. Формально применить t-тест можно, но правильнее взять z-тест для пропорций или хи-квадрат — у них корректный учёт биномиальной дисперсии.</p>
      <p><b>Сильная правосторонняя скошенность.</b> LTV, длительность сессии, доход — скошенные распределения, где медиана в разы меньше среднего. t-тест технически работает (ЦПТ спасает на больших $n$), но теряет мощность и измеряет не то, что интересно бизнесу. Лучше log-transform, Mann-Whitney или quantile regression.</p>
      <p><b>Student при неравных дисперсиях.</b> Классический Student предполагает $\\sigma_A = \\sigma_B$. Если отношение $s_B^2/s_A^2 > 3$ или $n_A \\neq n_B$ — ошибка I рода уплывает, p-value становится врушкой. Правило: всегда используй Welch, если нет веских оснований для Student.</p>
      <p><b>Предположение независимости.</b> Если в группе есть кластеры (несколько сессий одного юзера, ученики из одного класса) — эффективное $n$ меньше номинального, стандартная ошибка занижена. Нужны cluster-robust SE или mixed-effects.</p>

      <h3>🧭 Когда применять t-тест — и когда не стоит</h3>
      <table>
        <tr><th>✅ Применяй t-тест когда</th><th>❌ НЕ применяй когда</th></tr>
        <tr>
          <td>Метрика непрерывная и распределение близко к симметричному (или $n \\geq 50$ на группу)</td>
          <td>Метрика бинарная (конверсия, клик) — бери z-тест для пропорций или хи-квадрат</td>
        </tr>
        <tr>
          <td>Выборки небольшие, но данные примерно нормальны — t-распределение корректно учитывает неопределённость $s$</td>
          <td>Распределение сильно скошено и $n$ мало — лучше Mann-Whitney или bootstrap</td>
        </tr>
        <tr>
          <td>Дисперсии в группах могут отличаться — используй Welch-версию (default в scipy)</td>
          <td>Жёсткие выбросы в данных (ARPU, доходы) — среднее нерепрезентативно, медиана честнее</td>
        </tr>
        <tr>
          <td>Есть парные наблюдения (до/после, матчинг) — paired t-test резко увеличивает мощность</td>
          <td>Нарушена независимость (повторные измерения одного юзера, кластеризация) — нужны mixed-effects</td>
        </tr>
        <tr>
          <td>Нужен понятный CI в исходных единицах — объяснить бизнесу разность средних</td>
          <td>Интересует не среднее, а квантиль (медиана, p95 времени отклика) — quantile test</td>
        </tr>
        <tr>
          <td>Сравниваешь две ML-модели на $k$ фолдах cross-validation</td>
          <td>Больше двух групп — одиночные t-тесты раздуют FWER, используй ANOVA или поправки</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('ab-z-test')">z-тест для пропорций</a></b> — если метрика бинарная (конверсия, клик). Корректно считает биномиальную дисперсию $p(1-p)/n$.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('ab-mann-whitney')">Mann-Whitney U</a></b> — непараметрический аналог: если данные скошены, ординальны или содержат выбросы. Теряет ~5% мощности при идеально нормальных данных, но мощнее на тяжёлых хвостах.</li>
        <li><b>Bootstrap разности средних/медиан</b> — когда распределение непонятное или нужна статистика сложнее среднего. Не требует аналитических допущений, только хватает вычислительных ресурсов.</li>
        <li><b>ANOVA / Kruskal-Wallis</b> — если групп больше двух. Контролирует общую ошибку I рода вместо попарных t-тестов.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('ab-bayesian')">Bayesian t-test</a></b> — когда хочется прямой вероятности $P(\\mu_B &gt; \\mu_A)$ и возможности мониторить результат непрерывно без peeking-штрафа.</li>
      </ul>
    `,
