  ...
  tabs: {
    theory: `
      ...
      <table>
        ...
        <tr><td>Разные размеры</td><td>Плохо (режет большие)</td><td>Нормально</td></tr>
        <tr><td>Разная плотность</td><td>Плохо</td><td>Плохо</td></tr>
      ...
      <ul>
        ...
        <li><b>Не работает в высоких размерностях</b> (p > 20) из-за <a class="glossary-link" onclick="App.selectTopic('glossary-curse-dimensionality')">проклятия размерности</a>.</li>
...
plt.show()</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>Geospatial-аналитика.</b> Клики такси по карте, точки заказов, POI в городе — всё это плотностные кластеры произвольной формы. DBSCAN на координатах (lat, lon) с метрикой haversine находит «горячие зоны» и оставляет одиночные вызовы как noise. K-Means здесь провалится: он разрежет районы по сферам.</li>
        <li><b>Обнаружение аномалий и fraud.</b> Всё, что DBSCAN пометил как noise (метка $-1$), — кандидаты в выбросы: подозрительные транзакции, аномальные IP-сессии, странные поведенческие паттерны. Это бесплатный побочный продукт кластеризации.</li>
        <li><b>Астрономия и космос.</b> Поиск звёздных скоплений и галактик в каталогах типа SDSS — классическая задача для DBSCAN: скопления имеют разную форму и размер, пустое пространство между ними — шум, $k$ заранее никто не знает.</li>
        <li><b>Single-cell RNA-seq биоинформатика.</b> Группировка клеток по экспрессии генов после UMAP: клеточные популяции образуют плотностные кластеры произвольной формы. HDBSCAN (эволюция DBSCAN) — де-факто стандарт в Scanpy/Seurat.</li>
        <li><b>Сегментация событий в логах и мониторинге.</b> Алерты, сыплющиеся с серверов, группируются в «инциденты» по временной близости и общим признакам. DBSCAN отделяет плотные волны ошибок от одиночных шумовых срабатываний.</li>
        <li><b>Обработка изображений.</b> Поиск объектов на карте плотности пикселей (точки на микроскопическом снимке, частицы в физике high-energy), сегментация point cloud в LiDAR-данных для автопилота.</li>
        <li><b>Поиск ботов и кликфрода в веб-аналитике.</b> Боты образуют плотные сгустки в пространстве user agent × IP × timing, люди — размазаны. DBSCAN выделяет ботоферму как отдельный плотный кластер.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Не требует знать $k$ заранее.</b> В отличие от K-Means, DBSCAN сам определяет число кластеров из структуры плотности. Это огромное преимущество в EDA, где ты буквально не знаешь, сколько в данных групп — и в production, где число групп может меняться со временем.</p>
      <p><b>Находит кластеры произвольной формы.</b> Полумесяцы, кольца, спирали, вытянутые ленты — всё, на чём K-Means и GMM ломаются. Потому что DBSCAN оперирует связностью «точка A достижима из B через цепочку плотных соседей», а не геометрией сфер/эллипсов.</p>
      <p><b>Встроенная обработка шума.</b> Точки, у которых меньше <code>min_samples</code> соседей в $\\varepsilon$-окрестности, автоматически получают метку $-1$ и не портят остальные кластеры. Это одновременно и кластеризация, и outlier detection — два в одном.</p>
      <p><b>Устойчивость к выбросам по построению.</b> В K-Means один экстремум утягивает центроид, и весь кластер разваливается. У DBSCAN выбросы просто отправляются в noise, плотные области остаются как были — алгоритм принципиально робастен.</p>
      <p><b>Один проход по данным — детерминирован.</b> В отличие от K-Means со случайной инициализацией, DBSCAN (при фиксированном порядке обработки) даёт один и тот же результат каждый раз. Нет <code>n_init=10</code>, нет «попробуй ещё раз, может повезёт».</p>
      <p><b>Хорошо ложится на KD-tree/Ball-tree.</b> При правильных индексах сложность $O(n \\log n)$ вместо наивного $O(n^2)$. На 100k-500k точек — вполне практичный алгоритм.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Один глобальный eps для всего датасета.</b> Если в данных есть плотные кластеры (центр города) и разреженные (пригород), один $\\varepsilon$ не подойдёт ни к тем, ни к другим: либо разреженные распадутся на шум, либо плотные сольются в одну кашу. Это — фундаментальная проблема, которую решает только HDBSCAN.</p>
      <p><b>Catastrophic fail в высокой размерности.</b> При $d &gt; 15$-$20$ все расстояния становятся похожими (curse of dimensionality), $\\varepsilon$-окрестность либо пуста, либо содержит всё. DBSCAN на сырых эмбеддингах 768-d не работает — обязательно прогнать через PCA/UMAP до 5-10 размерности.</p>
      <p><b>Выбор eps — нетривиальная задача.</b> Стандартный приём — k-distance plot и поиск «локтя», но на реальных данных локоть часто неявный. Ошибка в eps на 20% даёт принципиально другую кластеризацию. Это не «тонкий тюнинг», а серьёзная поломка.</p>
      <p><b>Граничные точки неоднозначны.</b> Точка на границе двух кластеров может быть приписана к любому из них в зависимости от порядка обработки. Для воспроизводимости приходится фиксировать seed и порядок, либо использовать strict-версии (HDBSCAN).</p>
      <p><b>Плохо параллелится.</b> Expansion кластера — это BFS/DFS по графу достижимости, последовательный по своей природе. На очень больших данных DBSCAN уступает K-Means и mini-batch-вариантам.</p>
      <p><b>Память $O(n^2)$ в худшем случае.</b> Если все точки близки, алгоритм держит матрицу достижимости целиком. Без индексных структур на 1M точек DBSCAN просто OOM.</p>

      <h3>🧭 Когда брать DBSCAN — и когда точно не стоит</h3>
      <table>
        <tr><th>✅ Бери DBSCAN когда</th><th>❌ НЕ бери когда</th></tr>
        <tr>
          <td>Кластеры произвольной формы: полумесяцы, кольца, вытянутые, нелинейные</td>
          <td>Кластеры сферические и известно $k$ — K-Means быстрее и проще</td>
        </tr>
        <tr>
          <td>$k$ заранее неизвестно и может меняться от запуска к запуску</td>
          <td>Плотность кластеров сильно различается — бери HDBSCAN, он переживёт разные шкалы</td>
        </tr>
        <tr>
          <td>Нужно одновременно кластеризовать и найти выбросы (шум как побочный продукт)</td>
          <td>Размерность $d &gt; 20$ без предварительной редукции — расстояния ломаются</td>
        </tr>
        <tr>
          <td>Geospatial-данные с метрикой haversine или manhattan</td>
          <td>Данных десятки миллионов — DBSCAN плохо параллелится, возьми mini-batch K-Means</td>
        </tr>
        <tr>
          <td>Данные до ~500k точек и есть KD-tree/Ball-tree</td>
          <td>Нужны мягкие вероятности принадлежности к кластеру — бери GMM</td>
        </tr>
        <tr>
          <td>Нужна устойчивость к выбросам без ручной чистки</td>
          <td>Кластеры пересекаются и граничные точки критичны — бери soft-методы</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы — что взять вместо и почему</h3>
      <ul>
        <li><b>HDBSCAN</b> — прямой наследник: автоматический подбор eps, работа с кластерами разной плотности, иерархия, stability-based selection. В 2026-м — де-факто дефолт там, где раньше был DBSCAN. Минус: тяжелее и менее интуитивный.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('kmeans')">K-Means</a></b> — если кластеры сферические, $k$ известен и нужна максимальная скорость. Проще, быстрее, но не ловит произвольные формы и чувствителен к выбросам.</li>
        <li><b>OPTICS</b> — обобщение DBSCAN, которое строит reachability-plot вместо фиксированного eps. Медленнее, но даёт более гибкую иерархию кластеров.</li>
        <li><b>Spectral clustering</b> — если граф связности важнее плотности (например, social network communities). Работает на матрице similarity, но $O(n^3)$ по построению.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('isolation-forest')">Isolation Forest</a></b> — если цель именно outlier detection, а не кластеризация. Масштабируется на миллионы точек, DBSCAN там уже не годится.</li>
      </ul>
    `,
