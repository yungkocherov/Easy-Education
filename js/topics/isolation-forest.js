...
plt.show()</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>Fraud detection в банках и платёжных системах.</b> Миллиарды транзакций в день, метки «мошенничество» редки и запаздывают — supervised-модели не успевают. Isolation Forest запускается по набору признаков (сумма, merchant, geo, velocity) и в реальном времени подсвечивает «странные» операции как кандидатов для ручного ревью или step-up-аутентификации.</li>
        <li><b>Обнаружение вторжений и аномалий в сетях.</b> Netflow, логи firewall, DNS-запросы — тысячи признаков, аномальный паттерн означает атаку (DDoS, exfiltration, lateral movement). Isolation Forest отлично работает в высокой размерности, где kNN/DBSCAN уже не справляются.</li>
        <li><b>Мониторинг SRE и production-систем.</b> CPU, память, latency, error rate, throughput — десятки метрик по сервису. Isolation Forest ловит комбинированные аномалии («всё в норме по отдельности, но такой комбинации обычно не бывает»), которые пропускают пороговые алерты.</li>
        <li><b>Quality control в производстве.</b> Параметры сенсоров на конвейере: температура, давление, вибрация, время цикла. Дефектные изделия имеют аномальные сочетания параметров — Isolation Forest быстро их изолирует, ML-команда не тратит месяцы на сбор размеченных дефектов.</li>
        <li><b>Медицина и wearable devices.</b> Показатели пульса, давления, сна от фитнес-браслетов — аномалии могут сигналить о приступе, падении, депрессии. Isolation Forest работает без размеченных данных («здоровый образ» определяется из большинства).</li>
        <li><b>Data cleaning перед тренировкой ML.</b> Перед основным обучением гоняют Isolation Forest, выкидывают топ-1% аномальных строк — повышение качества downstream-модели без ручной разметки. Особенно помогает, когда данные собраны из шумных источников.</li>
        <li><b>Отлов ботов и кликфрода в веб-аналитике.</b> Сессии ботов имеют аномальные сочетания признаков (UA + таймлайн + geo + паттерн кликов), Isolation Forest быстро выделяет их даже при отсутствии лейблов.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Линейная сложность $O(n \\log n)$.</b> В отличие от LOF и One-Class SVM, которые требуют вычисления попарных расстояний ($O(n^2)$ и хуже), Isolation Forest строит $t$ деревьев по подвыборкам размера $\\psi = 256$. Работает на миллионах и миллиардах точек без GPU.</p>
      <p><b>Не требует вычисления расстояний.</b> Это ключевое отличие от density-based методов. В высокой размерности евклидово расстояние ломается (curse of dimensionality), а случайные разрезы по осям — нет. Isolation Forest остаётся работоспособен при $d = 100$-$1000$, где LOF и kNN уже бесполезны.</p>
      <p><b>Принцип «аномалии легко изолировать» интуитивен и работает.</b> Нормальная точка окружена другими нормальными — чтобы её отделить, нужно много разрезов. Аномалия одинока — пара разрезов, и готово. Глубина изоляции напрямую становится anomaly score.</p>
      <p><b>Тривиальная параллелизация.</b> Каждое дерево строится независимо от остальных. На 32-ядерной машине обучение и инференс разлетаются по ядрам без усилий, scikit-learn и PySpark из коробки.</p>
      <p><b>Почти нет гиперпараметров.</b> <code>n_estimators</code> (100 — почти всегда ок), <code>max_samples</code> (256 по рекомендации оригинальной статьи) и <code>contamination</code> (ожидаемая доля аномалий). Никаких eps, min_samples, perplexity — минимум тюнинга.</p>
      <p><b>Работает без меток.</b> Unsupervised по построению. Не нужны размеченные аномалии — что критично, потому что «настоящие» аномалии редки, разнообразны и обычно приходят после факта.</p>
      <p><b>Быстрый инференс.</b> Одна точка прогоняется через $t$ деревьев за $O(t \\log n)$ — миллисекунды. Подходит для online-скоринга потоковых данных.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Axis-parallel разрезы.</b> Деревья режут по осям ($x_j &gt; t$), поэтому «косые» аномалии (точка отклоняется по диагональной комбинации признаков, но по каждой оси нормальна) изолируются хуже. Лечится Extended Isolation Forest (EIF), где разрезы случайные гиперплоскости.</p>
      <p><b>Стохастичность между запусками.</b> Два запуска с разными seed дают разные ранжирования аномальных точек на границе. Для production фиксируй random_state и проверяй стабильность метрик.</p>
      <p><b>Нужно знать contamination заранее.</b> Параметр задаёт, где провести порог между «нормой» и «аномалией». Ошибся на порядок — получил либо тонну false positives, либо пропустил атаку. На практике определяется экспериментально или через оценку capacity команды, которая обрабатывает алерты.</p>
      <p><b>Кластерные аномалии — слепое пятно.</b> Если атакующие массово создают 1000 похожих мошеннических аккаунтов, они образуют плотный «кластер аномалий» — и Isolation Forest воспринимает их как нормальную плотную область. Для таких сценариев нужны семантические фичи (связи, временная близость) или graph-методы.</p>
      <p><b>Низкая интерпретируемость.</b> «Почему эта точка — аномалия?» — в ответ ты получаешь score, но не объяснение. Для регуляторных отчётов и SOC-аналитика нужна SHAP-обвязка или rule-based дополнения.</p>
      <p><b>Может принять разреженные, но нормальные точки за аномалии.</b> Если реальные данные неравномерно распределены (длинный хвост), точки с редкими, но легитимными значениями постоянно получают высокий score и раздражают операторов.</p>

      <h3>🧭 Когда брать Isolation Forest — и когда точно не стоит</h3>
      <table>
        <tr><th>✅ Бери Isolation Forest когда</th><th>❌ НЕ бери когда</th></tr>
        <tr>
          <td>Нужна unsupervised anomaly detection без размеченных данных</td>
          <td>У тебя есть размеченные аномалии и их много — бери supervised (XGBoost, LightGBM) — он сильнее</td>
        </tr>
        <tr>
          <td>Размерность высокая ($d &gt; 20$), расстояния ломаются — LOF/kNN уже не работают</td>
          <td>Аномалии образуют плотные кластеры (массовый fraud, coordinated attacks) — IF примет их за норму</td>
        </tr>
        <tr>
          <td>Данных миллионы-миллиарды, нужна масштабируемость и параллелизация</td>
          <td>Нужна полная интерпретируемость решений для регулятора — строй rule-based + SHAP-обвязку</td>
        </tr>
        <tr>
          <td>Аномалии одиночные, редкие, отклоняющиеся по нескольким признакам</td>
          <td>Аномалии имеют сложную временнýю структуру (контекстные, сезонные) — бери LSTM-autoencoder или Prophet-residuals</td>
        </tr>
        <tr>
          <td>Нужен быстрый baseline anomaly detector с минимумом тюнинга</td>
          <td>Данные сильно нелинейные и аномалии определяются геометрией многообразия — бери Autoencoder reconstruction error</td>
        </tr>
        <tr>
          <td>Нужен online-скоринг потоковых данных (fraud в реальном времени)</td>
          <td>Количество признаков маленькое ($d &lt; 5$) и легко построить density-модель — GMM или KDE дадут лучше</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы — что взять вместо и почему</h3>
      <ul>
        <li><b>Extended Isolation Forest (EIF)</b> — прямое улучшение: вместо axis-parallel разрезов использует случайные гиперплоскости. Лучше ловит косые аномалии ценой чуть большей сложности.</li>
        <li><b>Local Outlier Factor (LOF)</b> — для плотностных аномалий в невысокой размерности. Считает отношение плотности точки к плотностям соседей. Дорогой ($O(n^2)$), но интуитивнее в интерпретации.</li>
        <li><b>One-Class SVM</b> — строит границу вокруг «нормы» в ядерном пространстве. Хорош для медиум-данных с нелинейной структурой, плохо масштабируется на миллионы точек.</li>
        <li><b>Autoencoder reconstruction error</b> — нейросеть учится реконструировать нормальные данные; аномалии = большая ошибка реконструкции. Лучше для изображений, временных рядов, высокоструктурированных данных.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('dbscan')">DBSCAN</a> / HDBSCAN</b> — если хочется одновременно кластеризовать и находить аномалии. Точки, помеченные как noise, = кандидаты в outliers.</li>
        <li><b>Supervised models (<a class="glossary-link" onclick="App.selectTopic('gradient-boosting')">XGBoost/LightGBM</a>)</b> — если у тебя накоплена размеченная история fraud/аномалий. Supervised всегда сильнее unsupervised, когда есть метки.</li>
      </ul>
    `,
