        ...
        content: `
          ...
          <div class="step" data-step="6">
            ...
            <div class="calc">
              ...
              x &lt; 1.5: взвеш. MSE = 1.373<br>
              x &lt; 2.5: взвеш. MSE = 0.750<br>
              x &lt; 3.5: взвеш. MSE = 0.986<br>
              x &lt; 4.5: взвеш. MSE = <b>0.547</b> ← лучший!<br><br>
              <b>Разрез x &lt; 4.5</b> (L: {1,2,3,4}, R: {5})<br>
...
for name, model in [('MSE', mse_model), ('Huber', huber_model)]:
    ...
    applications: `
      ...
      <ul>
        ...
        <li><b>Оценка стоимости активов и недвижимости.</b> Банки и PropTech (Домклик, Циан, Zillow) используют GBM для автоматической оценки жилья. Нативная работа с категориями (район, тип дома), устойчивость к выбросам и SHAP для объяснения «почему 8.3 млн» — идеальный набор.</li>
        <li><b>Прогноз LGD и EAD в банках.</b> Loss Given Default и Exposure at Default — регуляторные задачи Basel III, где нужна модель, которая даёт не только точку, но и диапазон. Quantile loss в GBM даёт верхний/нижний квантиль напрямую.</li>
        <li><b>Страхование: расчёт премий и резервов.</b> Prediction of claim amount — классический use case Gamma loss или Tweedie loss, которые поддерживает XGBoost. Линейные GLM проигрывают GBM 10-20% точности.</li>
        <li><b>Энергетика и прогноз потребления.</b> Прогноз нагрузки на сеть, цен на оптовом рынке электроэнергии, выработки ВИЭ — задачи со смешанными признаками (погода + календарь + исторические ряды), где GBM — стандарт.</li>
      ...
      <p><b>Нативная обработка пропусков и категорий.</b> XGBoost выучивает лучшее направление для NaN, LightGBM оптимально разбивает категориальные фичи, CatBoost применяет ordered target encoding. Целый этап препроцессинга просто исчезает.</p>
      <p><b>Early stopping автоматически подбирает сложность.</b> Задал <code>early_stopping_rounds=50</code> — и алгоритм сам найдёт оптимальное число деревьев на валидации. Не нужно вручную тюнить <code>n_estimators</code> или бояться переобучения.</p>
      ...
      <p><b>Много гиперпараметров с неинтуитивным взаимодействием.</b> <code>learning_rate × n_estimators</code>, <code>max_depth</code> vs <code>num_leaves</code>, <code>subsample × colsample</code>, регуляризации — тюнинг без Optuna может занять день и не дать результата. «Дефолты работают плохо» чаще, чем у RF.</p>
      ...
      <table>
        ...
        <tr>
          <td>Нужны квантильные прогнозы (риск, прогноз с запасом)</td>
          <td>Задача требует экстраполяции за пределы обучающего диапазона</td>
        </tr>
        <tr>
          <td>Есть время на грамотный тюнинг через Optuna + early stopping</td>
          <td>Нужен быстрый baseline за 5 минут без тюнинга — RF проще</td>
        </tr>
        <tr>
          <td>Смешанные признаки, пропуски, категории — LightGBM/CatBoost справятся</td>
          <td>Целевая переменная полна выбросов, а лосс привязан к MSE — бери Huber/MAE</td>
        </tr>
        <tr>
          <td>Нужны SHAP-объяснения для регулятора или бизнеса</td>
          <td>Latency жёстко ограничена (&lt; 5 мс) и нет бюджета на дистилляцию</td>
        </tr>
        <tr>
          <td>Нестандартный лосс (Gamma, Tweedie, Poisson, Quantile) важен для задачи</td>
          <td>Задача онлайн-обучения с постоянным дообучением</td>
        </tr>
        <tr>
          <td>Большие данные (&gt; 100K строк) и нужно сочетание скорости + качества — LightGBM</td>
          <td>Данные неструктурированные (изображения, текст) — нужны нейросети</td>
        </tr>
      </table>
      ...
      <ul>
        ...
        <li><b>CatBoost</b> отдельно — если много категориальных признаков высокой кардинальности. Ordered target encoding бьёт ручной target encoding и избавляет от class of bugs.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('linear-regression')">Линейная регрессия</a> / Ridge / Lasso</b> — если зависимость близка к линейной, нужна экстраполяция или интерпретируемость через коэффициенты. На чистых линейных данных GBM проигрывает по скорости и не выигрывает по качеству.</li>
        <li><b>Prophet / ARIMA / Neural Forecast</b> — если задача — это временной ряд с сильным трендом и сезонностью. GBM на trend-данных проваливается там, где специализированные модели работают.</li>
      </ul>
    `,
