    ...
    examples: [
      ...
      },
      {
        ...
        content: `
          ...
          <div class="lesson-box">
            95% доверительный интервал для OR: exp(w ± 1.96·SE). Если ЦИ включает 1 — эффект статистически незначим. Логрег — стандарт в медицинских исследованиях именно из-за интерпретируемости через OR.
          </div>
        `,
      },
      {
...
plt.show()</code></pre>
    ...
    proscons: `
      <div class="proscons">
        ...
        <div class="cons">
          ...
          <ul>
            ...
            <li>Не работает «из коробки» с категориями — нужен encoding</li>
          </ul>
        </div>
      </div>
      ...
      <table>
        ...
        <tr>
          <td>Нужна интерпретация: какие факторы влияют и насколько</td>
          <td>Важна только точность, интерпретация не важна</td>
        </tr>
        <tr>
          <td>Данных мало (&lt; 1000 строк) — сложные модели переобучатся</td>
          <td>Границы между классами сильно нелинейные</td>
        </tr>
        <tr>
          <td>Нужен быстрый бейзлайн для сравнения с более сложными моделями</td>
          <td>Много сильно коррелированных признаков без регуляризации</td>
        </tr>
        <tr>
          <td>Нужны калиброванные вероятности (для порогов, ROC-AUC)</td>
          <td>Все признаки категориальные с высокой кардинальностью</td>
        </tr>
        <tr>
          <td>Регуляторика требует объяснимости (скоринг, медицина)</td>
          <td>У вас уже есть XGBoost/LightGBM и не нужна интерпретация</td>
        </tr>
      </table>
      <p><b>Альтернативы:</b> Random Forest / XGBoost для нелинейности, Naive Bayes для текстов, SVM с kernel для сложных границ.</p>
    `,
