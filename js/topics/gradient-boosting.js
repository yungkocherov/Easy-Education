        ...
        content: `
          ...
          <div class="step" data-step="4">
            ...
            <div class="calc">
              ...
              F₂(2) = 5.05 + 0.5·0.76 = <b>5.43</b><br>
              F₂(3..5) = 7.635 + 0.5·0.76 = <b>8.015</b><br><br>
              MSE₂ ≈ <b>3.3</b> — ещё улучшение
            </div>
            <div class="why">Каждая итерация исправляет текущие ошибки. MSE падает монотонно на обучении.</div>
          </div>
          <div class="answer-box">
            <div class="answer-label">Ответ</div>
            <p>MSE: 9.84 → 5.03 → 3.30 — каждое дерево снижает ошибку. При η=0.5 нужно меньше итераций, чем при η=0.1, но риск переобучения выше.</p>
          </div>
...
plt.show()</code></pre>
    ...
    applications: `
      ...
      <ul>
        ...
        <li><b>CTR-prediction и рекламные системы.</b> Предсказание «кликнет ли пользователь на баннер» — миллиарды событий, сотни признаков. LightGBM на исторических данных — стандарт рекламных платформ (Facebook, Яндекс.Директ, ВК).</li>
        <li><b>Learning-to-rank в поиске.</b> LambdaMART (вариант GBM с попарным лоссом) — базовая модель ранжирования поисковой выдачи в Яндексе, Bing, e-commerce. CatBoost YetiRank, XGBoost ranking — производственные решения.</li>
        <li><b>Прогноз спроса, продаж, оттока.</b> Ритейл, телеком, SaaS — везде, где есть табличная история с сезонностью и категориями. Бустинг хорошо ест лаговые признаки и категории (регион, сегмент, тариф).</li>
        <li><b>Медицинская диагностика.</b> Предсказание осложнений, сепсиса, повторной госпитализации — задачи, где нужна максимальная точность и SHAP-объяснения для врача. GBM обычно бьёт нейросети на этих табличных данных.</li>
      ...
      <p><b>Категориальные признаки из коробки.</b> LightGBM делает оптимальные разбиения на категориях, CatBoost применяет ordered target encoding внутри себя. Ни one-hot, ни ручной encoding не нужен — это экономит часы работы и убирает класс багов.</p>
      <p><b>Хорошо калиброванные вероятности.</b> В отличие от RF, где <code>predict_proba</code> сжата к середине, бустинг с log-loss даёт вероятности, которые можно напрямую использовать в ценообразовании или оптимизации порогов без post-hoc калибровки.</p>
      ...
      <p><b>Много гиперпараметров.</b> <code>learning_rate</code>, <code>n_estimators</code>, <code>max_depth</code>/<code>num_leaves</code>, <code>subsample</code>, <code>colsample_bytree</code>, <code>reg_alpha</code>, <code>reg_lambda</code>, <code>min_child_weight</code> — их взаимодействие не интуитивно, без Optuna/Bayesian search тюнинг может занять день.</p>
      ...
      <table>
        ...
        <tr>
          <td>Есть время на тюнинг гиперпараметров через Optuna/Bayesian search</td>
          <td>Нужен супер-быстрый baseline за 5 минут без настройки — RF проще</td>
        </tr>
        <tr>
          <td>Смешанные признаки (числа + категории + пропуски) — CatBoost/LightGBM справятся</td>
          <td>Сильно зашумлены метки — бустинг усилит шум вместо его подавления</td>
        </tr>
        <tr>
          <td>Нужны откалиброванные вероятности для скоринга или оптимизации порогов</td>
          <td>Требуется экстраполяция за пределы обучающего диапазона</td>
        </tr>
        <tr>
          <td>Нужны SHAP-объяснения для регулятора или бизнеса</td>
          <td>Latency жёстко ограничена (&lt; 5 мс) и нет возможности дистиллировать</td>
        </tr>
        <tr>
          <td>Learning-to-rank или кастомный бизнес-лосс (квантильная регрессия)</td>
          <td>Онлайн-обучение, где модель должна переобучаться каждую секунду</td>
        </tr>
        <tr>
          <td>State-of-the-art бенчмарк для любой табличной задачи 2025</td>
          <td>Данные неструктурированные (изображения, текст, аудио) — нужны нейросети</td>
        </tr>
      </table>
      ...
      <ul>
        ...
        <li><b>CatBoost</b> отдельно — если много категориальных признаков высокой кардинальности (ID магазина, страна, подкатегория товара). Ordered target encoding работает лучше, чем one-hot или ручное target encoding.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('logistic-regression')">Логистическая регрессия</a></b> — если нужна максимальная интерпретируемость (коэффициенты вместо SHAP) и низкая latency. Часто используется в финтехе как «дешёвый» компонент рядом с GBM.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('stacking')">Stacking</a> GBM + RF + LR</b> — если ещё нужны 0.5-2% качества и нет ограничений по latency. Последний рубеж для Kaggle gold.</li>
      </ul>
    `,
