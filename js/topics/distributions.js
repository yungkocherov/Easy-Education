...
App.registerTopic({
  ...
  tabs: {
    theory: `
      ...
      <div class="illustration bordered">
        <svg viewBox="0 0 760 280" xmlns="http://www.w3.org/2000/svg" style="max-width:760px;">
          <text x="380" y="22" text-anchor="middle" font-size="15" font-weight="700" fill="#1e293b">Пуассон: Poisson(λ) — PMF</text>
          <line x1="60" y1="230" x2="720" y2="230" stroke="#475569" stroke-width="1.5"/>
          <!-- X labels -->
          <g font-size="11" fill="#64748b" text-anchor="middle" id="poisson-x-labels"></g>
          <text x="380" y="268" text-anchor="middle" font-size="12" fill="#64748b">число событий k</text>
          <!-- Bars generated by script -->
          <g id="poisson-bars"></g>
          <!-- Legend -->
          <rect x="500" y="50" width="16" height="16" fill="#dc2626" fill-opacity="0.5" stroke="#dc2626" stroke-width="1.5" rx="2"/>
          <text x="522" y="63" font-size="12" font-weight="700" fill="#dc2626">λ = 3</text>
          <rect x="500" y="74" width="16" height="16" fill="#1e40af" fill-opacity="0.5" stroke="#1e40af" stroke-width="1.5" rx="2"/>
          <text x="522" y="87" font-size="12" font-weight="700" fill="#1e40af">λ = 7</text>
        </svg>
        <div class="caption">Пуассон с λ=3 (красный) и λ=7 (синий). При малом λ — скошено вправо. С ростом λ становится симметричнее. При λ&gt;20 приближается к нормальному.</div>
        <script>
        (function() {
          // Compute Poisson PMF: P(k) = λ^k * e^(-λ) / k!
          ...
          var kMax = 14;
          var baselineY = 230, maxBarH = 180;
          var x0 = 80, barSpacing = 45, barWidth = 16;

          // Find max PMF across both λ for scaling
          var maxP = 0;
          for (var k = 0; k <= kMax; k++) {
            maxP = Math.max(maxP, poissonPMF(k, 3), poissonPMF(k, 7));
          }

          var barsG = document.getElementById('poisson-bars');
          var labelsG = document.getElementById('poisson-x-labels');
          var html = '';
          var labelsHtml = '';

          for (var k = 0; k <= kMax; k++) {
            var cx = x0 + k * barSpacing;
            var p3 = poissonPMF(k, 3);
            var p7 = poissonPMF(k, 7);
            var h3 = Math.max(1, Math.round((p3 / maxP) * maxBarH));
            var h7 = Math.max(1, Math.round((p7 / maxP) * maxBarH));
            // λ=3 bars (left of center)
            html += '<rect x="' + (cx - barWidth - 1) + '" y="' + (baselineY - h3) + '" width="' + barWidth + '" height="' + h3 + '" fill="#dc2626" fill-opacity="0.5" stroke="#dc2626" stroke-width="1.2" rx="1"/>';
            // λ=7 bars (right of center)
            html += '<rect x="' + (cx + 1) + '" y="' + (baselineY - h7) + '" width="' + barWidth + '" height="' + h7 + '" fill="#1e40af" fill-opacity="0.5" stroke="#1e40af" stroke-width="1.2" rx="1"/>';
            labelsHtml += '<text x="' + cx + '" y="' + (baselineY + 18) + '">' + k + '</text>';
          }
      ...
      <div class="illustration bordered">
        <svg viewBox="0 0 760 280" xmlns="http://www.w3.org/2000/svg" style="max-width:760px;">
          <defs>
            <linearGradient id="expGrad1" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#059669" stop-opacity="0.35"/><stop offset="100%" stop-color="#059669" stop-opacity="0.05"/></linearGradient>
            <linearGradient id="expGrad05" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#1e40af" stop-opacity="0.3"/><stop offset="100%" stop-color="#1e40af" stop-opacity="0.05"/></linearGradient>
          </defs>
          <text x="380" y="22" text-anchor="middle" font-size="15" font-weight="700" fill="#1e293b">Экспоненциальное Exp(λ) — PDF</text>
          ...
          <!-- Two curves generated by init — same style, different colors -->
          <path id="dist-exp-lam05-area" d="" fill="url(#expGrad05)"/>
          <path id="dist-exp-lam05" d="" fill="none" stroke="#1e40af" stroke-width="2.8"/>
          <path id="dist-exp-lam1-area" d="" fill="url(#expGrad1)"/>
          <path id="dist-exp-lam1" d="" fill="none" stroke="#059669" stroke-width="2.8"/>
          <!-- Legend -->
          <g font-size="13" font-weight="700">
            <line x1="480" y1="65" x2="510" y2="65" stroke="#059669" stroke-width="3"/>
            <text x="515" y="69" fill="#059669">λ = 1 (средний = 1)</text>
            <line x1="480" y1="90" x2="510" y2="90" stroke="#1e40af" stroke-width="3"/>
            <text x="515" y="94" fill="#1e40af">λ = 0.5 (средний = 2)</text>
          </g>
        ...
        (function() {
          ...
          // λ=0.5 drawn first (behind, wider), then λ=1 on top
          U.setPath(document, 'dist-exp-lam05-area', U.exponentialPath(x0, baselineY, 135, length, 0.5));
...
print("Нормальное" if p > 0.05 else "Не нормальное")</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>Нормальное распределение — шум измерений и статистические тесты.</b> Ошибки сенсоров, отклонения от линейного тренда в OLS, остатки регрессии — всё это по умолчанию моделируется как $N(0, \\sigma^2)$. Вся классическая статистика (t-тест, ANOVA, ДИ для среднего) явно или через ЦПТ опирается на нормальность.</li>
        <li><b>Log-normal — деньги, цены, размеры файлов, время.</b> Всё, что не может быть отрицательным и имеет мультипликативные эффекты, живёт в логнормальном мире: зарплаты, цены акций, размеры загруженных файлов, время отклика API, время жизни сессии пользователя. Log-трансформация превращает их в нормальные.</li>
        <li><b>Пуассон — поток редких событий.</b> Количество звонков в call-центр за час, число дефектов на квадратный метр, заявки в минуту, клики в секунду. Параметр $\\lambda$ — средняя интенсивность. Используется в массовом обслуживании (queueing theory) и capacity planning.</li>
        <li><b>Экспоненциальное — время между событиями.</b> Время до следующего отказа сервера, интервал между поступлением заявок, время жизни электронного компонента. Обладает «отсутствием памяти»: если прожил 5 лет, шанс прожить ещё год тот же, что у нового. Используется в reliability и survival analysis.</li>
        <li><b>Weibull — отказы с изнашиванием или приработкой.</b> Обобщение экспоненциального, где интенсивность отказов меняется со временем. Классика reliability engineering: «ванна» с высокой смертностью в начале (приработка), низкой в середине, и ростом к концу жизни (износ).</li>
        <li><b>Биномиальное / Бернулли — конверсии и доли.</b> A/B-тесты, CTR, success rate, доля дефектов. Основа для z-теста пропорций и теста Фишера.</li>
        <li><b>Power law / Парето — «правило 80/20».</b> Распределение доходов, размеров городов, популярности сайтов, длин слов в тексте, размеров файлов в интернете. Ключевая особенность — тяжёлые хвосты: среднее и дисперсия могут быть бесконечными, ЦПТ не работает классически.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Компактное описание реальности.</b> Вместо того чтобы хранить миллион чисел, ты описываешь их двумя параметрами ($\\mu, \\sigma$) или одним ($\\lambda$). Это критично для коммуникации, симуляции и моделирования: «данные ~ $N(100, 15)$» передаёт всё, что нужно для большинства задач.</p>
      <p><b>Готовые формулы для всего.</b> Как только ты знаешь распределение, у тебя есть формулы для среднего, дисперсии, квантилей, доверительных интервалов, статистических тестов. Не нужно ничего выводить заново — scipy.stats и statsmodels всё уже считают.</p>
      <p><b>Основа для симуляций.</b> Monte Carlo методы требуют, чтобы ты мог сэмплить из распределения. Если ты угадал правильное распределение, симуляции отражают реальность; если промахнулся — результат красиво, но ложно.</p>
      <p><b>Предпосылка для ML-моделей.</b> Linear regression предполагает нормальность ошибок. Logistic regression — бернуллиевские метки. Naive Bayes — распределения признаков (Gaussian NB, Multinomial NB, Bernoulli NB). Понимание распределений — ключ к пониманию, почему модель работает или не работает.</p>
      <p><b>Связи между распределениями.</b> Сумма биномиальных → нормальное (ЦПТ). Сумма экспоненциальных → гамма. Квадрат нормальной → хи-квадрат. Это позволяет переводить задачи между распределениями и использовать уже известные результаты.</p>
      <p><b>Простая генерация синтетики.</b> Когда нужны тестовые данные для unit-тестов, бенчмарков, симуляций — np.random.normal, np.random.poisson, np.random.exponential делают это в одну строку. Без распределений пришлось бы изобретать «фейковую реальность» руками.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Предположение нормальности без проверки.</b> Классическая ошибка: «возьмём t-тест, ведь выборка большая». Но если распределение log-normal или с тяжёлыми хвостами, среднее волатильно, std бесконечно, и t-тест врёт. Всегда смотри на histogram, QQ-plot, делай Shapiro-Wilk — или бери непараметрические методы.</p>
      <p><b>Тяжёлые хвосты ломают всё.</b> Распределения типа Парето, Коши, Lévy имеют бесконечную дисперсию (а Коши — даже бесконечное среднее). ЦПТ для них не работает классически. Финансовые данные, размеры катастроф, распределение богатства — это тяжёлые хвосты, и ошибка «оно примерно нормальное» может стоить миллиардов (см. LTCM 1998, 2008 кризис).</p>
      <p><b>Подгонка распределения без домейн-знания.</b> Ты можешь построить MLE-подгон для любого распределения, и scipy выдаст параметры. Но если выбор распределения неверный (подогнал нормальное к бимодальному), параметры будут формально «лучшими», а модель — систематически неверной. Нужно понимать процесс, а не слепо фитить.</p>
      <p><b>Смешанные распределения (mixture).</b> Реальные данные часто — смесь: пол × рост, регион × доход. Ни одно стандартное распределение не опишет это напрямую. Нужно либо сегментировать, либо моделировать через GMM, либо использовать непараметрические методы.</p>
      <p><b>Нестационарность.</b> Распределение как абстракция предполагает, что оно фиксировано во времени. Пользовательские метрики, финансовые временные ряды, поведенческие данные часто дрейфуют. Параметры вчерашнего распределения сегодня — вчерашний снег.</p>

      <h3>🧭 Когда применять — и когда не стоит</h3>
      <table>
        <tr><th>✅ Бери распределение когда</th><th>❌ НЕ бери когда</th></tr>
        <tr>
          <td>Данные ~ $N(\\mu, \\sigma^2)$: симметричные, unimodal, без тяжёлых хвостов — нормальное</td>
          <td>Данные явно бимодальны или мультимодальны — ни одно стандартное распределение не подходит</td>
        </tr>
        <tr>
          <td>Данные положительные, скошенные вправо, мультипликативные эффекты — log-normal</td>
          <td>У тебя 10 точек — подгон распределения статистически ненадёжен, используй непараметрику</td>
        </tr>
        <tr>
          <td>Считаешь количество редких событий в единицу времени — Пуассон</td>
          <td>Подозреваешь тяжёлые хвосты (финансы, катастрофы) — классические распределения врут</td>
        </tr>
        <tr>
          <td>Моделируешь время до отказа / между событиями без износа — экспоненциальное</td>
          <td>Процесс нестационарный — распределение «плывёт», параметры устаревают</td>
        </tr>
        <tr>
          <td>Отказы с износом или приработкой — Weibull (обобщение экспоненциального)</td>
          <td>Нет domain-знания о природе процесса — слепой подгон MLE даёт ложно-уверенные ответы</td>
        </tr>
        <tr>
          <td>Конверсии, доли, Bernoulli-эксперименты — биномиальное</td>
          <td>Данные — смесь нескольких популяций (мужчины/женщины, платящие/бесплатные) — моделируй mixture</td>
        </tr>
        <tr>
          <td>«Правило 80/20», power law эффекты (богатство, популярность) — Парето/power law</td>
          <td>Нужна точная форма с подгонкой по CDF — бутстрэп или эмпирическая функция лучше</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('glossary-bootstrap')">Непараметрический бутстрэп</a></b> — когда не хочешь или не можешь предполагать форму распределения. Просто ресемплируешь и считаешь статистику — работает всегда.</li>
        <li><b>Эмпирическая функция распределения (ECDF)</b> — просто считаешь долю точек ≤ $x$. Никаких предположений, но и никакой компактности: нужны все данные.</li>
        <li><b>Kernel Density Estimation (KDE)</b> — сглаженная непараметрическая оценка плотности. Ловит бимодальность, асимметрию, любую форму. Цена — нужно выбирать bandwidth.</li>
        <li><b>Gaussian Mixture Model (GMM)</b> — когда данные — это смесь нескольких нормальных. Отличный компромисс между «одно распределение» и «ничего не предполагаем».</li>
        <li><b>Копулы</b> — когда важны не отдельные маржинальные распределения, а их совместная структура зависимости (финансы, risk modeling).</li>
      </ul>
    `,
