...
App.registerTopic({
  ...
  tabs: {
    theory: `
      ...
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('glossary-curse-dimensionality')">Проклятие размерности</a></b> — все алгоритмы работают хуже.</li>
      ...
      <div class="key-concept">
        <div class="kc-label">Интуиция: что такое <a class="glossary-link" onclick="App.selectTopic('glossary-eigen')">собственные векторы и значения</a>?</div>
        <p>Представь облако точек — оно вытянуто как эллипс. <b>Собственный вектор</b> — это направление одной из осей эллипса. <b>Собственное значение</b> — длина этой оси (насколько данные растянуты вдоль неё).</p>
        <p>PCA ищет: <b>в каком направлении данные разбросаны сильнее всего?</b> Это и есть 1-я главная компонента (самая длинная ось эллипса). 2-я компонента — перпендикулярное направление с наибольшим оставшимся разбросом. И так далее.</p>
        <p><b>Собственное значение λ₁ = 5</b> означает: дисперсия данных вдоль 1-й оси равна 5. Если λ₂ = 0.3, то вдоль 2-й оси данные почти не варьируются — её можно выкинуть с минимальной потерей информации.</p>
      </div>
      ...
      <ol>
        ...
        <li><b><a class="glossary-link" onclick="App.selectTopic('glossary-covariance')">Ковариационная матрица</a>:</b> $\\Sigma = \\frac{1}{n-1} X^T X$.</li>
      ...
      <ul>
        ...
        <li><b>Сжатие</b> — изображения, <a class="glossary-link" onclick="App.selectTopic('glossary-embedding')">embedding</a>.</li>
      ...
      <div class="deep-dive">
        <summary>Подробнее: PCA через <a class="glossary-link" onclick="App.selectTopic('glossary-svd')">SVD</a></summary>
...
plt.show()</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>Предобработка перед тяжёлыми моделями.</b> BERT-эмбеддинги 768-d или фичи CNN 2048-d часто содержат тонны коррелированных компонент. PCA до 50-128 компонент сохраняет 95% дисперсии и ускоряет последующую кластеризацию, поиск ближайших соседей и тренировку классификаторов в 5-20 раз.</li>
        <li><b>Визуализация высокоразмерных данных в EDA.</b> Быстрый «первый взгляд» на датасет: есть ли в нём структура, какие объекты похожи, есть ли явные выбросы. PCA даёт глобальную картину за секунды — прежде чем запускать дорогие t-SNE или UMAP.</li>
        <li><b>Face recognition (Eigenfaces).</b> Исторический, но до сих пор используемый подход: лица 128×128 = 16384 признака раскладываются по ~100 главным компонентам («собственным лицам»), поиск ближайшего соседа в этом пространстве = распознавание. Основа для всех последующих face-методов.</li>
        <li><b>Факторный анализ в финансах.</b> Доходности сотен акций раскладываются на небольшое число факторов: рыночный, секторные, стилевые. PCA — математический фундамент APT и риск-моделей (Barra, Axioma).</li>
        <li><b>Genomics и bioinformatics.</b> Матрица экспрессии генов: десятки тысяч генов × тысячи образцов. Первые 2-3 компоненты часто разделяют здоровые/больные ткани, мужчин/женщин, батчи экспериментов — ставит в тупик, сколько информации в первых главных осях.</li>
        <li><b>Сжатие изображений и шумоподавление.</b> Отбросить малые компоненты = убрать высокочастотный шум, сохранив структуру. Применяется в МРТ, астрономии, спутниковых снимках.</li>
        <li><b>Whitening перед нейросетями и ICA.</b> PCA-преобразование с делением на $\\sqrt{\\lambda_i}$ даёт декоррелированные признаки единичной дисперсии — стандартный preprocessing шаг для многих методов.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Математическая строгость.</b> PCA — это SVD-разложение, и у него нет случайности, нет гиперпараметров в алгоритме, нет локальных минимумов. Получил собственные векторы — это оптимальная ортогональная проекция с максимумом сохранённой дисперсии. Точка.</p>
      <p><b>Детерминированность и воспроизводимость.</b> Один и тот же датасет → один и тот же результат. В отличие от t-SNE/UMAP/нейронных эмбеддингов, здесь нет «запусти ещё раз, может повезёт». Для репорта и production это критично.</p>
      <p><b>Скорость через SVD.</b> Randomized SVD и truncated SVD дают топ-$k$ компонент за $O(n \\cdot d \\cdot k)$ — линейно по размеру данных. Работает на миллионах строк и десятках тысяч признаков без GPU.</p>
      <p><b>Работает как встроенный denoiser.</b> Шум в данных обычно равномерно «размазан» по всем направлениям, а сигнал сосредоточен в нескольких направлениях с большой дисперсией. Взяв топ-компоненты, мы отбрасываем шум и удерживаем сигнал — бесплатный side-effect.</p>
      <p><b>Декорреляция признаков.</b> Выход PCA — ортогональные компоненты, полностью некоррелированные между собой. Это лечит мультиколлинеарность для линейной регрессии и стабилизирует градиенты в нейросетях (вплоть до batch-norm'а и whitening-трюков).</p>
      <p><b>Понятная метрика «сколько потеряли».</b> Explained variance ratio прямо отвечает на вопрос: «если я оставлю 30 компонент — сколько информации сохраню?». 90%? 95%? Решение принимается осознанно, а не наугад.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Только линейные зависимости.</b> PCA ищет гиперплоскости. Если данные лежат на спирали, швейцарском рулете или сфере, проекция на прямую уничтожит структуру. На «two moons» PCA не разделит классы вообще. Для нелинейных структур бери Kernel PCA, Autoencoder или t-SNE/UMAP.</p>
      <p><b>Чувствительность к масштабу.</b> Если один признак в миллиметрах, другой в километрах — второй будет доминировать дисперсию, первая компонента будет почти равна ему. Без <code>StandardScaler</code> или <code>RobustScaler</code> PCA даёт мусор — это не «рекомендация», а обязательное условие.</p>
      <p><b>«Дисперсия ≠ информативность».</b> PCA максимизирует дисперсию, но классы могут быть разделены по направлению с маленькой дисперсией. Классический пример: два класса отличаются только по одному признаку из сотни — PCA его отбросит, классификация рухнет. Для supervised-задач разумнее LDA.</p>
      <p><b>Новые компоненты не интерпретируемы.</b> Компонента $\\text{PC1} = 0.3 \\cdot \\text{age} - 0.5 \\cdot \\text{income} + 0.7 \\cdot \\text{tenure} - \\ldots$ — это «смесь», а не бизнес-понятный признак. Для регуляторных моделей и отчётности такие преобразования часто запрещены.</p>
      <p><b>Чувствительность к выбросам.</b> Один экстремум в направлении $X$ увеличивает дисперсию в этом направлении в разы — и первая компонента PCA ложится на выброс, а не на реальную структуру. Лечится через Robust PCA или предварительный outlier removal.</p>
      <p><b>Глобальный метод без учёта локальной структуры.</b> PCA «усредняет» все точки, поэтому кластеры, которые хорошо разделены локально, но перекрываются глобально, в проекции сольются. Для визуализации кластерной структуры t-SNE/UMAP лучше.</p>

      <h3>🧭 Когда брать PCA — и когда точно не стоит</h3>
      <table>
        <tr><th>✅ Бери PCA когда</th><th>❌ НЕ бери когда</th></tr>
        <tr>
          <td>Много признаков с линейными корреляциями, нужно убрать мультиколлинеарность</td>
          <td>Данные лежат на нелинейном многообразии (спираль, сфера) — Kernel PCA/UMAP/Autoencoder</td>
        </tr>
        <tr>
          <td>Нужно ускорить downstream ML (кластеризация, kNN, NN-обучение)</td>
          <td>Нужны интерпретируемые признаки — новые оси это непонятные смеси</td>
        </tr>
        <tr>
          <td>Быстрый EDA-взгляд на высокоразмерные данные в 2D/3D</td>
          <td>Цель — сохранить разделение классов — LDA делает это по определению лучше</td>
        </tr>
        <tr>
          <td>Предобработка перед t-SNE/UMAP (сбить размерность с 768 до 50)</td>
          <td>Признаков мало ($d &lt; 10$) — снижать нечего, скорее испортишь, чем улучшишь</td>
        </tr>
        <tr>
          <td>Шумоподавление в изображениях, сигналах, embedding'ах</td>
          <td>Есть сильные выбросы и нет возможности их почистить — первая компонента уедет на выброс</td>
        </tr>
        <tr>
          <td>Декорреляция признаков перед линейной регрессией, GMM, Naive Bayes</td>
          <td>Признаки на разных единицах измерения и нельзя стандартизовать — результат будет мусорный</td>
        </tr>
        <tr>
          <td>Нужна детерминированная, воспроизводимая редукция (в отличие от t-SNE/UMAP)</td>
          <td>Visual cluster-separation важнее глобальной геометрии — t-SNE/UMAP покажут кластеры чётче</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы — что взять вместо и почему</h3>
      <ul>
        <li><b>Kernel PCA</b> — если структура нелинейная, но хочется сохранить математическую строгость. RBF-ядро ловит кластеры любой формы, но сложность $O(n^2)$ — только для средних датасетов.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('tsne-umap')">t-SNE / UMAP</a></b> — если цель визуализации важнее сохранения расстояний. Лучше разделяют кластеры в 2D, но искажают глобальную геометрию и не применимы к новым данным (в классическом виде).</li>
        <li><b>LDA (Linear Discriminant Analysis)</b> — если есть метки классов и нужна редукция, сохраняющая классовое разделение. Ограничен $c-1$ компонентами ($c$ — число классов), но мощнее PCA для classification-preprocessing.</li>
        <li><b>Autoencoder</b> — нелинейная «PCA на стероидах»: нейросеть с bottleneck. Ловит любые структуры, применим к новым данным, но требует тонны данных и тюнинга.</li>
        <li><b>Truncated SVD</b> — PCA без центровки, специально для разреженных матриц (TF-IDF, bag-of-words) и задач латентно-семантического анализа.</li>
        <li><b>NMF (Non-negative Matrix Factorization)</b> — если нужна интерпретируемость (компоненты = «темы», «паттерны»): NMF даёт неотрицательные компоненты, которые читаются как «части целого».</li>
      </ul>
    `,
