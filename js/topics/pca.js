...
plt.show()</code></pre>
    ...
    proscons: `
      ...
      <table>
        ...
        <tr>
          <td>Много признаков с линейными корреляциями</td>
          <td>Нелинейная структура данных (спирали, кольца) → t-SNE/UMAP/Kernel PCA</td>
        </tr>
        <tr>
          <td>Нужно ускорить обучение ML-модели (меньше признаков)</td>
          <td>Нужна интерпретируемость признаков — новые оси это смесь</td>
        </tr>
        <tr>
          <td>Визуализация многомерных данных в 2D/3D</td>
          <td>Цель — разделение классов → LDA сохраняет классовую структуру лучше</td>
        </tr>
        <tr>
          <td>Предобработка перед кластеризацией/t-SNE (убрать шум)</td>
          <td>Мало признаков (&lt; 10) — снижение размерности не нужно</td>
        </tr>
        <tr>
          <td>Шумоподавление — малые компоненты часто шум</td>
          <td>Есть выбросы без предобработки — они доминируют дисперсию</td>
        </tr>
        <tr>
          <td>Декорреляция признаков перед линейной регрессией</td>
          <td>Признаки на разных масштабах и нет возможности стандартизовать</td>
        </tr>
      </table>
      <p><b>Альтернативы:</b> Kernel PCA для нелинейности, t-SNE/UMAP для визуализации, LDA для supervised снижения размерности, Autoencoder для глубокого сжатия.</p>
    `,
