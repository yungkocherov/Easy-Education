...
print(df.round(3))</code></pre>
    ...
    applications: `
      ...
      <ul>
        ...
        <li><b>A/B-тесты моделей.</b> Чтобы сказать «новая модель лучше», нужна метрика, измеренная на одной и той же тестовой выборке. F1, ROC-AUC или Precision@k — в зависимости от продукта.</li>
        <li><b>Мониторинг в проде и детекция деградации.</b> На prod модели измеряется выбранная метрика день за днём. Если она падает — триггер переобучения или алерт команде.</li>
        <li><b>Отчётность для стейкхолдеров.</b> «У нас accuracy 92%» — понятно продакту. «У нас F1 0.87 при recall 0.91 и precision 0.84» — уже требует ML-культуры. Правильная метрика — это общий язык команды и бизнеса.</li>
        <li><b>Калибровка и uncertainty.</b> Brier score, log loss, calibration plots — когда важны честные вероятности, а не бинарные предсказания (медицина, ценообразование).</li>
      ...
      <p><b>ROC-AUC — метрика без порога.</b> Измеряет способность модели ранжировать positive выше negative независимо от выбора порога. Удобно сравнивать модели ещё до продуктовых решений.</p>
      ...
      <p><b>ROC-AUC оптимистичен на сильном дисбалансе.</b> При соотношении 1:999 ROC-AUC может быть 0.95 при практически бесполезной precision. Бери PR-AUC или смотри на precision@k.</p>
      ...
      <table>
        ...
        <tr>
          <td><b>Precision</b> — FP дорог: спам-фильтр, реклама, автоматическое отклонение платежей</td>
          <td>Нужно поймать все positive, а цена FP низкая — бери recall</td>
        </tr>
        <tr>
          <td><b>Recall</b> — FN критичен: онкология, fraud, антитеррор, утечки данных</td>
          <td>Слишком много FP обесценивают продукт — бери F-beta или precision</td>
        </tr>
        <tr>
          <td><b>F1</b> — баланс P/R, умеренный дисбаланс, нет явной цены ошибок</td>
          <td>У тебя есть реальные стоимости FP/FN — считай expected cost</td>
        </tr>
        <tr>
          <td><b>F-beta</b> — явное предпочтение precision или recall задано бизнесом</td>
          <td>Классы сбалансированы и ошибки равнозначны — F1 проще и привычнее</td>
        </tr>
        <tr>
          <td><b>ROC-AUC</b> — ранжирование без конкретного порога, сравнение моделей</td>
          <td>Дисбаланс очень сильный (1:1000) — бери PR-AUC</td>
        </tr>
        <tr>
          <td><b>PR-AUC / Average Precision</b> — очень редкий positive-класс (fraud, медицина)</td>
          <td>Нужно сравнить с литературой, где принят ROC-AUC — держи оба</td>
        </tr>
        <tr>
          <td><b>Log loss / Brier score</b> — важны калиброванные вероятности (ценообразование, риски)</td>
          <td>Нужны только бинарные решения — излишняя сложность</td>
        </tr>
        <tr>
          <td><b>RMSE</b> — регрессия, большие ошибки штрафуются сильнее (финансы, прогноз спроса)</td>
          <td>В данных выбросы, которые не должны доминировать метрику — бери MAE</td>
        </tr>
        <tr>
          <td><b>MAE</b> — регрессия с выбросами, интерпретируемая ошибка в единицах таргета</td>
          <td>Важно наказывать именно крупные промахи — RMSE</td>
        </tr>
      </table>
      ...
      <ul>
        ...
        <li><b>Lift и Gain charts</b> — в маркетинге и скоринге: насколько модель лучше случайного выбора при разных охватах. Ближе к бизнесу, чем ROC.</li>
        <li><b>Calibration curve и Brier score</b> — если вероятности используются для принятия решений, а не просто для порога, нужна калибровка (Platt, isotonic) и метрика качества вероятностей.</li>
        <li><b>Метрики ранжирования: NDCG, MAP, MRR</b> — для рекомендательных систем и поиска, где важен порядок, а не бинарное решение.</li>
      </ul>
    `,
