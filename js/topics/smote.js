...
App.registerTopic({
  ...
  tabs: {
    theory: `
      ...
      <div class="callout warn"><b>SMOTE — только на train!</b> Никогда не применяй его к validation/test. Иначе синтетические точки утекут из train в test → завышенная оценка качества. Это грубая форма data leakage.</div>
...
print('UnderSampling F1:', f1_score(y_test, lr.predict(X_test), average='macro').round(3))</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>Fraud detection и антифрод.</b> Мошеннических транзакций обычно 0.1-1% от общего трафика. SMOTE и его варианты (Borderline-SMOTE, ADASYN) генерируют синтетических «мошенников» в окрестности реальных, чтобы модель научилась их отличать. Используется поверх XGBoost, LightGBM, логрег — стандартный приём в банковском ML.</li>
        <li><b>Медицинская диагностика редких заболеваний.</b> Позитивных случаев — 1-5% в датасете. Без балансировки модель вырождается в «всегда здоровый» и формально имеет 95% accuracy. SMOTE в табличных медицинских признаках позволяет повысить recall на редкий класс без потери specificity.</li>
        <li><b>Churn prediction.</b> Отток обычно 5-15% — умеренный дисбаланс. SMOTE помогает, если нужно чётко различать «собирается уйти» vs «лояльный», особенно на ранних этапах, когда class_weights в бустинге дают недостаточный эффект.</li>
        <li><b>Predictive maintenance.</b> Поломки оборудования — редкие события по определению. Синтетическая генерация примеров «скоро сломается» даёт модели больше сигнала и уменьшает false negatives, которые стоят дорого (простой, срыв SLA).</li>
        <li><b>Классификация редких намерений в NLP/диалоговых системах.</b> У популярных интентов тысячи примеров, у экзотических — десятки. SMOTE в пространстве эмбеддингов (или один из embedding-aware вариантов) помогает выровнять классы для intent-классификатора.</li>
        <li><b>Defects detection в manufacturing.</b> Хорошие детали: 99%, брак: 1%. Supervised-модель без балансировки просто не учится на дефектах. SMOTE — один из стандартных приёмов вместе с data augmentation и focal loss.</li>
        <li><b>Кредитный скоринг для default prediction.</b> Дефолтов обычно 2-8%. SMOTE используется в составе пайплайна вместе с class_weight и изменением threshold — перед этапом калибровки вероятностей.</li>
      </ul>

      <h3>Сильные стороны — и почему они важны</h3>
      <p><b>SMOTE честнее, чем random oversampling.</b> Дублирование минорного класса увеличивает его частоту, но не даёт модели новой информации — только усиливает overfit на конкретные точки. SMOTE создаёт <i>новые</i> синтетические точки в окрестности минорных, расширяя decision boundary, а не закрепляя старую.</p>
      <p><b>Работает как регуляризатор decision boundary.</b> Генерация точек между соседями минорного класса плавнее заполняет пространство — модель учится более гладкой границе, меньше склонна к резким «прыжкам» между классами. В табличных данных это часто даёт +2-5% к F1 редкого класса.</p>
      <p><b>Совместим с любой моделью.</b> SMOTE работает в пространстве признаков, независимо от того, какая модель идёт дальше: логрег, дерево, бустинг, kNN, нейросеть. Это чистый preprocessing-шаг, вставляемый в пайплайн до обучения.</p>
      <p><b>Много доработанных вариантов под разные задачи.</b> Borderline-SMOTE генерирует точки только у границы классов (там, где ошибок больше), ADASYN адаптивно генерирует больше примеров около «сложных» точек, SMOTE-NC работает с числовыми + категориальными признаками, SVMSMOTE использует SVM для определения границы. Экосистема imbalanced-learn даёт готовые реализации.</p>
      <p><b>Не теряет данные, в отличие от undersampling.</b> Random undersampling мажоритарного класса выбрасывает информацию — что болезненно, когда датасет и так небольшой. SMOTE, наоборот, создаёт новые примеры, не жертвуя ничем.</p>
      <p><b>Легко встраивается в CV через imblearn Pipeline.</b> <code>imblearn.pipeline.Pipeline</code> гарантирует, что SMOTE применяется только на train fold, а не на validation — это критично для предотвращения утечек.</p>

      <h3>Ограничения — и когда они реально бьют</h3>
      <p><b>Ломается в высоких размерностях.</b> SMOTE ищет $k$ ближайших соседей в евклидовой метрике. При $d &gt; 50$ все расстояния становятся похожими (curse of dimensionality), «соседи» превращаются в случайные точки, и синтетика становится бессмысленной. Для эмбеддингов и dense features это серьёзный барьер.</p>
      <p><b>Смешивает шум.</b> Если в минорном классе есть выбросы (а в реальных данных они почти всегда есть), SMOTE построит синтетические точки между «настоящими» минорами и выбросами — и внесёт в обучение ложные паттерны. Лечится предварительной чисткой и Borderline-SMOTE.</p>
      <p><b>Утечка через CV.</b> Если применить SMOTE до разбиения на train/val, синтетические точки из val оказываются близки к синтетике из train (они порождены из общих соседей) — метрики на validation завышаются, production падает. Это самая частая ошибка при работе со SMOTE.</p>
      <p><b>Не помогает, когда проблема не в дисбалансе.</b> Если модель плохо работает на редком классе не из-за количества, а из-за того, что признаки не разделяют классы (fundamental overlap), SMOTE ничего не решит — только замаскирует проблему. Сначала убедись, что есть сигнал.</p>
      <p><b>Ломает калибровку вероятностей.</b> После SMOTE распределение классов в обучении не соответствует реальному, и <code>predict_proba</code> выдаёт искажённые вероятности. Для калиброванных вероятностей нужна post-hoc калибровка (Platt, isotonic) или альтернатива через class_weights без SMOTE.</p>
      <p><b>Плохо работает с категориальными признаками.</b> Базовый SMOTE интерполирует числовые значения — «среднее между мужчиной и женщиной» не имеет смысла. Нужен SMOTE-NC (Nominal-Continuous) или target encoding перед применением.</p>

      <h3>Когда брать SMOTE — и когда точно не стоит</h3>
      <table>
        <tr><th>Бери SMOTE когда</th><th>НЕ бери когда</th></tr>
        <tr>
          <td>Табличные данные с умеренной размерностью ($d &lt; 50$) и сильным дисбалансом (1-10% минорного класса)</td>
          <td>Высокая размерность ($d &gt; 100$, эмбеддинги, изображения) — соседи теряют смысл</td>
        </tr>
        <tr>
          <td>Модель явно недоучивается на минорный класс (recall/F1 низкие)</td>
          <td>Достаточно class_weight или scale_pos_weight — часто это решает проблему без синтетики</td>
        </tr>
        <tr>
          <td>Используешь imblearn Pipeline и применяешь SMOTE только на train fold</td>
          <td>Применяешь SMOTE до train/test split — гарантированная утечка и завышенные метрики</td>
        </tr>
        <tr>
          <td>Есть чистый минорный класс без большого количества выбросов</td>
          <td>В минорном классе много шума — Borderline-SMOTE или ENN-cleanup нужны сначала</td>
        </tr>
        <tr>
          <td>Метрики оценки: F1, AUPRC, recall на минорный класс — то, что реально важно</td>
          <td>Нужны калиброванные вероятности — SMOTE ломает калибровку, используй class_weights + isotonic</td>
        </tr>
        <tr>
          <td>Числовые признаки доминируют; категориальных мало или можно использовать SMOTE-NC</td>
          <td>Данные — изображения/аудио/текст: бери data augmentation, focal loss, не SMOTE</td>
        </tr>
        <tr>
          <td>Датасет небольшой и undersampling отбросил бы слишком много информации</td>
          <td>Проблема не в дисбалансе, а в отсутствии сигнала — никакая балансировка не поможет</td>
        </tr>
      </table>

      <h3>Альтернативы — что взять вместо и почему</h3>
      <ul>
        <li><b>class_weight / scale_pos_weight</b> — встроенная опция в логрег, деревьях, бустинге. Часто решает проблему дисбаланса без синтетических данных. Первый шаг до SMOTE, не после.</li>
        <li><b>Threshold tuning</b> — просто сдвинуть порог <code>predict_proba &gt; 0.5</code> в сторону редкого класса (например, 0.2). Меняет trade-off precision/recall без изменения модели и данных. Дёшево и сильно.</li>
        <li><b>Focal loss</b> — модификация cross-entropy, которая автоматически down-weight'ит «лёгкие» примеры и фокусируется на трудных. Стандарт в object detection, работает и в табличных задачах.</li>
        <li><b>Borderline-SMOTE / ADASYN</b> — умные варианты SMOTE, генерирующие синтетику только у границы классов или адаптивно у сложных точек. Обычно стабильнее базового SMOTE.</li>
        <li><b>SMOTE-NC, SMOTE-ENC</b> — для данных со смешанными числовыми и категориальными признаками. Не интерполируют категории, а выбирают моду.</li>
        <li><b>Random undersampling + ensemble</b> — EasyEnsemble и BalancedRandomForest строят много базовых моделей на сбалансированных подвыборках. Не теряет информацию за счёт множества моделей и часто работает лучше SMOTE на больших данных.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('isolation-forest')">Isolation Forest</a> или One-Class SVM</b> — если дисбаланс крайне сильный (&lt;0.1%), переформулируй задачу как anomaly detection: учи только на «норме», ищи отклонения.</li>
      </ul>
    `,
