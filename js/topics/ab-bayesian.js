...
plt.show()</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>Продуктовые A/B с гибкой остановкой.</b> «Катимся как только $P(B &gt; A) \\geq 95\\%$ и Expected Loss меньше порога». Не нужно фиксировать срок теста заранее — можно выкатывать раньше, если разница очевидна, или продолжать дольше, если сигнал слабый.</li>
        <li><b>Тесты на малом трафике.</b> Разумный prior (например, Beta$(\\alpha_0, \\beta_0)$ с исторической базовой конверсией) стабилизирует оценки при сотнях, а не десятках тысяч наблюдений. Для стартапов и нишевых продуктов — единственный работающий подход.</li>
        <li><b>Multi-armed bandits и Thompson sampling.</b> Байес здесь лежит в самом сердце: на каждом шаге сэмплим $p_A, p_B \\sim \\text{Posterior}$ и показываем вариант с большей ценой. Балансирует exploration/exploitation, минимизирует regret. Используется в рекламе, рекомендациях, динамическом ценообразовании.</li>
        <li><b>Раннее прекращение клинических испытаний.</b> Этическое ограничение: если одно лечение явно лучше, продолжать давать контрольной группе хуже — нельзя. Байесовский подход позволяет мониторить posterior непрерывно без раздутия ошибки I рода.</li>
        <li><b>Expected Loss-driven бизнес-решения.</b> Вместо «значимо / не значимо» даём ответ в деньгах: «если выкатим B, ожидаемые потери $\\leq 2$ ₽ на юзера» — это напрямую сравнимо со стоимостью раскатки.</li>
        <li><b>Последовательный анализ без peeking-штрафа.</b> Posterior обновляется байес-формулой и не зависит от stopping rule: можно смотреть результаты сколько угодно раз, в отличие от классического фиксированного теста.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Прямой ответ на тот вопрос, который задаёт бизнес.</b> «Какова вероятность, что B лучше A?» — это $P(\\theta_B &gt; \\theta_A \\mid \\text{data})$, прямо то, что считает байесовский подход. Частотный p-value отвечает на другой вопрос («насколько наши данные редкие при $H_0$»), и часто неправильно интерпретируется даже опытными аналитиками.</p>
      <p><b>Нет проблемы peeking и множественных взглядов.</b> Posterior — это распределение, а не тест-статистика. Его можно смотреть в любой момент, на любой выборке, сколько угодно раз — правило Байеса симметрично во времени и stopping rule на корректность не влияет. Это ломает главный эксплойт классических A/B: «смотрим на p-value каждые 5 минут и останавливаемся, когда повезло».</p>
      <p><b>Инкорпорация prior-знания.</b> Если исторически конверсия колеблется в районе 2-3%, $\\text{Beta}(20, 800)$ честно это кодирует. Малая выборка тогда не «стартует с нуля», а аккуратно апдейтит разумное начальное знание — оценки стабильнее, мощность выше.</p>
      <p><b>Expected Loss в бизнес-единицах.</b> $E[\\max(0, \\theta_A - \\theta_B) \\mid \\text{data}]$ — это ожидаемая потеря конверсии, если зря выкатим B. Переводится в рубли напрямую и сравнивается с затратами на раскатку. Частотный подход такого выхода не даёт.</p>
      <p><b>Вероятностный, а не бинарный вывод.</b> Вместо «значимо/не значимо» получаем целое posterior-распределение разности. Можно отчитаться: «$P(B &gt; A) = 91\\%$, с вероятностью 60% разница $\\geq 1\\%$, с вероятностью 95% $\\leq 5\\%$» — полная картина неопределённости, а не булево число.</p>
      <p><b>Естественно обобщается на много вариантов.</b> Считаем $P(\\theta_i = \\max_j \\theta_j)$ для каждого $i$ — никаких поправок Бонферрони. В A/B/C/D/.../Z-тестах байес работает без спецкоррекций.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Субъективность prior.</b> При малых данных выбор prior влияет на posterior и, значит, на решение. Informative-prior может обвинить в cherry-picking; flat-prior обнуляет преимущество метода. Честный путь — pre-registration приора и sensitivity analysis (проверка результата при нескольких разумных вариантах).</p>
      <p><b>Нет гарантированного контроля ошибки I рода.</b> Frequentist с $\\alpha=0.05$ гарантирует: при длинной серии тестов доля ложных срабатываний $\\leq 5\\%$. Байес такой гарантии не даёт — можно построить calibrated Bayes, но это дополнительная работа. Для регуляторно-чувствительных областей (FDA) это серьёзный минус.</p>
      <p><b>Сложнее для непрерывных метрик.</b> Beta-Binomial сопряжённость даёт closed-form posterior для конверсий. Для средних (t-распределение) или LTV (log-normal/gamma) либо нужны более сложные модели, либо MCMC/Stan — не всегда просто внедрить.</p>
      <p><b>Вычислительная стоимость.</b> $P(B &gt; A)$ и Expected Loss чаще всего считаются Monte Carlo сэмплированием (10 000 - 100 000 сэмплов). На больших экспериментах это терпимо, но в real-time bandits накладно и требует кэширования.</p>
      <p><b>Менее знаком регуляторам и команде.</b> Учёные, CRO-фирмы и многие senior-аналитики приучены к p-value и CI. Байесовский отчёт требует объяснений — и иногда отвержения «а почему не p-value». Для внутренних команд это вопрос обучения, для регуляторных задач — блокер.</p>
      <p><b>Ранняя остановка всё равно увеличивает variance решений.</b> Даже без «статистической» проблемы peeking, чем раньше останавливаем тест, тем шумнее оценка эффекта — это не корректируется автоматически, нужно явно следить за dispersion posterior'а и минимальной длиной теста.</p>

      <h3>🧭 Когда применять Bayesian A/B — и когда не стоит</h3>
      <table>
        <tr><th>✅ Применяй Bayesian когда</th><th>❌ НЕ применяй когда</th></tr>
        <tr>
          <td>Нужна гибкая остановка теста и непрерывный мониторинг posterior</td>
          <td>Жёсткое регуляторное требование на контроль ошибки I рода (FDA, ЦБ)</td>
        </tr>
        <tr>
          <td>Трафик ограничен и есть разумный исторический prior</td>
          <td>Prior не из чего построить и нет желания делать sensitivity analysis</td>
        </tr>
        <tr>
          <td>Бизнесу важен ответ в деньгах: Expected Loss, вероятности</td>
          <td>Стейкхолдерам нужно «значимо или нет» в терминах классического p-value</td>
        </tr>
        <tr>
          <td>Много вариантов (A/B/C/D) и не хочется поправок Бонферрони</td>
          <td>Нужна максимально простая, быстрая и стандартизированная процедура</td>
        </tr>
        <tr>
          <td>Multi-armed bandits: нужно автоматически переливать трафик в лучший вариант</td>
          <td>Требуется чистая оценка эффекта, а не минимизация regret — бандит искажает баланс групп</td>
        </tr>
        <tr>
          <td>Метрика — конверсия (Beta-Binomial сопряжённость даёт closed-form posterior)</td>
          <td>Метрика сложная (ratio, LTV с тяжёлыми хвостами) и нет времени на Stan/PyMC</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('ab-z-test')">Frequentist z-тест для пропорций</a></b> — когда нужна жёсткая гарантия ошибки I рода и стандартная процедура, понятная регуляторам.</li>
        <li><b>Always-valid p-values / mSPRT</b> — частотная альтернатива, которая тоже позволяет безопасно подглядывать: корректирует p-value для sequential мониторинга без перехода на байес.</li>
        <li><b>Group Sequential Design (O'Brien-Fleming, Pocock)</b> — классический фреймворк для запланированных промежуточных анализов с жёстким контролем $\\alpha$. Стандарт в клинических испытаниях.</li>
        <li><b>Thompson sampling / UCB bandits</b> — если цель не «оценить эффект», а «минимизировать потери»: алгоритм автоматически перераспределяет трафик.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('ab-multiple-testing')">Benjamini-Hochberg FDR</a></b> — если остаёшься в частотном мире, но боишься множественных сравнений: контролирует долю ложных открытий вместо FWER.</li>
      </ul>
    `,
