...
# Вывод: XOR требует нелинейной границы → нужна нейросеть (MLP)
    ...
    applications: `
      ...
      <ul>
        <li><b>Учебник и первая модель в курсах ML.</b> Перцептрон — это «Hello, world» обучения с учителем: можно показать на 10 строках кода, как веса двигаются в сторону правильного ответа. Именно поэтому он в каждом введении в нейросети.</li>
        <li><b>Строительный блок многослойных сетей.</b> Любой нейрон в MLP, CNN или Transformer — это тот же перцептрон $\\sigma(w^T x + b)$ с нелинейностью. Понимать, как он обновляет веса, — значит понимать, откуда растёт backpropagation.</li>
        <li><b>Онлайн-обучение на потоках.</b> Перцептрон обновляется по одному примеру за шаг — идеально для ситуаций, когда весь датасет не помещается в память или приходит по стриму (логи, клики, сенсоры).</li>
        <li><b>Averaged / voted perceptron в простом текст-классификаторе.</b> До эры BERT усреднённый перцептрон был рабочей лошадкой в задачах POS-tagging, NER и классификации коротких текстов — дёшево и сходится за несколько эпох.</li>
        <li><b>Kernel perceptron как лёгкая альтернатива SVM.</b> Тот же трюк с ядрами, что у SVM, но без квадратичной оптимизации — годится, когда нужна нелинейная граница на небольшом датасете и хочется минимум зависимостей.</li>
        <li><b>Линейно разделимые задачи и baseline.</b> Если данные реально разделимы гиперплоскостью (редко, но бывает — например, простая сегментация по одному-двум признакам), перцептрон сходится за конечное число шагов и даёт 100% accuracy.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Предельная простота алгоритма.</b> Правило обновления умещается в одну строку: $w \\gets w + y_i x_i$ при ошибке. Это делает перцептрон идеальной моделью, чтобы без магии объяснить, что вообще значит «обучение» — есть ошибка, двигаем веса в сторону её уменьшения.</p>
      <p><b>Теорема сходимости Розенблатта.</b> Если данные линейно разделимы с отступом $\\gamma$, перцептрон гарантированно найдёт разделяющую гиперплоскость за $O(R^2 / \\gamma^2)$ шагов. Это редкая математическая гарантия в ML — обычно мы надеемся, что градиентный спуск «где-то сойдётся».</p>
      <p><b>Онлайн и инкрементальность.</b> Не требует хранить весь датасет: обновляется по одному примеру. Память — только веса. Это делает его применимым там, где Random Forest или нейросеть в память не влезут в принципе.</p>
      <p><b>Нулевая вычислительная стоимость.</b> Одно скалярное произведение и сравнение с порогом. Inference на любом микроконтроллере, обучение — за миллисекунды. На современном железе перцептрон обрабатывает миллионы примеров в секунду.</p>
      <p><b>Историческая и педагогическая ценность.</b> Все современные сети — от ResNet до GPT — это стек перцептронов с нелинейностями. Разобрать один нейрон — единственный способ не превратить deep learning в «магию чёрного ящика».</p>
      <p><b>Легко расширяется.</b> Добавил pocket algorithm — работает на неразделимых. Добавил ядро — работает на нелинейных. Добавил нелинейность и стек — получил MLP. Перцептрон — это базовая клетка, из которой растёт всё остальное.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Не работает на неразделимых данных.</b> Если классы пересекаются (а в реальности они почти всегда пересекаются), обычный перцептрон входит в бесконечный цикл: веса колеблются между ошибочными примерами и никогда не стабилизируются. Нужен pocket algorithm или early stopping.</p>
      <p><b>Не решает XOR.</b> Знаменитая атака Минского-Паперта 1969 года: один перцептрон не может разделить XOR, потому что это не линейная задача. Этот результат буквально похоронил нейросети на 15 лет — пока не придумали многослойные модели с backprop.</p>
      <p><b>Нет вероятностей на выходе.</b> Перцептрон возвращает только $\\pm 1$ — не 0.73. Для скорингов, калибровки, ROC-AUC нужна логистическая регрессия или sigmoid поверх.</p>
      <p><b>Нет регуляризации.</b> Ни L1, ни L2 — веса могут расти без ограничений, особенно при шумных данных. Современные замены (LR, SVM) решают это как стандартную часть loss-функции.</p>
      <p><b>Зависимость от порядка примеров.</b> Разный порядок обучающих объектов даёт разные итоговые веса. Воспроизводимость требует фиксированного seed и перемешивания — иначе два запуска дают разные модели.</p>
      <p><b>Не имеет концепции margin.</b> В отличие от SVM, перцептрон останавливается на первой же гиперплоскости, которая разделяет классы — даже если она проходит в миллиметре от точек. Результат нестабилен к новым данным.</p>

      <h3>🧭 Когда брать перцептрон — и когда точно не стоит</h3>
      <table>
        <tr><th>✅ Бери перцептрон когда</th><th>❌ НЕ бери когда</th></tr>
        <tr>
          <td>Ты изучаешь ML и хочешь понять, как вообще работает «обучение весов» — без теории MLP и backprop</td>
          <td>Тебе нужна рабочая модель в продакшен — возьми логистическую регрессию или SVM</td>
        </tr>
        <tr>
          <td>Данные линейно разделимы и можно это доказать — гарантированная сходимость за конечное число шагов</td>
          <td>Классы пересекаются или присутствует шум — обычный перцептрон зациклится</td>
        </tr>
        <tr>
          <td>Онлайн-обучение на потоке, жёсткие ограничения памяти (IoT, микроконтроллер)</td>
          <td>Нужны вероятности или калиброванные скоринги — перцептрон даёт только знак</td>
        </tr>
        <tr>
          <td>Базовая метрика для новой задачи за 30 секунд — «есть ли вообще линейный сигнал»</td>
          <td>Задача нелинейная (XOR, изображения, тексты) — одна гиперплоскость не справится</td>
        </tr>
        <tr>
          <td>Нужно объяснить студенту, что такое обновление весов и сходимость обучения</td>
          <td>Данные табличные со смешанными типами — возьми Gradient Boosting или Random Forest</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы — что взять вместо и почему</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('logistic-regression')">Логистическая регрессия</a></b> — прямой наследник перцептрона с вероятностями, регуляризацией и гладкой loss-функцией. В 99% случаев берут её.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('svm')">SVM</a></b> — если важен максимальный margin, а не «первая попавшаяся» разделяющая гиперплоскость. Устойчивее и теоретически обоснованнее.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('neural-network')">Многослойная нейросеть (MLP)</a></b> — если задача нелинейная. Стек перцептронов с сигмоидой или ReLU решает XOR и любые более сложные границы.</li>
        <li><b>Pocket / averaged / voted perceptron</b> — если нужна именно перцептроноподобная модель, но данные шумные. Минимальное расширение, решающее проблему зацикливания.</li>
      </ul>
    `,
