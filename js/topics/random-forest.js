...
print(f'CV ROC-AUC: {grid.best_score_:.4f}')</code></pre>
    ...
    proscons: `
      ...
      <table>
        ...
        <tr>
          <td>Нужен сильный бейзлайн «из коробки» без тюнинга</td>
          <td>Нужна максимальная точность (XGBoost/LightGBM обычно лучше)</td>
        </tr>
        <tr>
          <td>Табличные данные со смесью числовых и категориальных признаков</td>
          <td>Очень большие данные (&gt; 10M строк) — LightGBM быстрее</td>
        </tr>
        <tr>
          <td>Есть пропуски и выбросы — RF устойчив</td>
          <td>Нужна интерпретируемость одного правила (возьми одно дерево)</td>
        </tr>
        <tr>
          <td>Нужен feature importance для отбора признаков или EDA</td>
          <td>Продакшен с жёсткими ограничениями по latency или размеру модели</td>
        </tr>
        <tr>
          <td>Нужна OOB-оценка без отдельной валидации</td>
          <td>Задача требует экстраполяции за пределы обучающего диапазона</td>
        </tr>
        <tr>
          <td>Обучение можно параллелить — каждое дерево независимо</td>
          <td>Нужны вероятности — RF даёт плохо калиброванные (нужна Platt/isotonic)</td>
        </tr>
      </table>
      <p><b>Альтернативы:</b> XGBoost/LightGBM для точности, одно дерево для интерпретации, ExtraTrees для скорости.</p>
    `,
