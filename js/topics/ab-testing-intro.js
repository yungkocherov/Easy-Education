...
App.registerTopic({
  ...
  tabs: {
    theory: `
      ...
      <h3>Частые ошибки в A/B тестах</h3>
      ...
      <div class="callout warn">Если проверять каждый день 20 дней, вероятность ложно-положительного результата при H₀ составит не 5%, а ~30–40%. Это не анализ — это рулетка.</div>
...
print("(не менее 7 дней для учёта дня-недели эффекта)")</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>E-commerce и продуктовый рост.</b> Кнопки оформления заказа, цены, описания товаров, дизайн карточек, промо-блоки. Amazon, Booking, Ozon — каждое заметное изменение проходит через A/B: один процент конверсии = миллионы в выручке.</li>
        <li><b>Онбординг и удержание в SaaS.</b> Тесты первых экранов, welcome-email, триала, pricing page. Цель — найти шаг, на котором теряется большинство пользователей, и поднять активацию.</li>
        <li><b>Реклама и креативы.</b> Google Ads и Meta крутят десятки вариантов одного баннера параллельно: креатив, текст, аудитория, ставка — всё решается A/B-сплитом.</li>
        <li><b>Рекомендательные системы.</b> Новая модель ранжирования катится через interleaving или классический сплит: сравнивают CTR, dwell time, retention. Без этого нельзя отличить улучшение от случайности.</li>
        <li><b>Клинические испытания (RCT).</b> Золотой стандарт доказательной медицины. Рандомизация + контрольная группа = единственный способ установить причинно-следственную связь «лекарство → эффект».</li>
        <li><b>Маркетинг и рассылки.</b> Тема письма, тон обращения, время отправки, сегмент. Быстрый цикл обратной связи — идеальная песочница для A/B.</li>
        <li><b>ML-модели в продакшене.</b> Shadow deployment + A/B: новая модель обслуживает 5% трафика, сравнивают бизнес-метрики (не только offline accuracy) до раскатки на 100%.</li>
      </ul>

      <h3>Сильные стороны — и почему они важны</h3>
      <p><b>Причинно-следственная связь, а не корреляция.</b> Рандомизация разрывает связь между выбором варианта и характеристиками пользователя: если группы различаются только тем, что им показали — наблюдаемая разница в метрике и есть эффект изменения. Это единственный способ сказать «кнопка подняла конверсию», а не «у нас выросла конверсия и заодно мы поменяли кнопку».</p>
      <p><b>Контролируемая ошибка I рода.</b> Уровень значимости $\\alpha$ задаёт верхнюю границу вероятности «увидеть эффект, которого нет». Ты знаешь риск ложного срабатывания заранее — это принципиально отличается от «посмотрел на дашборд и показалось, что выросло».</p>
      <p><b>Защита от HiPPO и багов интуиции.</b> Highest Paid Person's Opinion перестаёт быть аргументом: если эксперимент не показал эффекта — нельзя «передумать» постфактум. Многие громкие редизайны после A/B оказывались нейтральными или отрицательными.</p>
      <p><b>Масштабируемость и параллельность.</b> Big-tech проводит сотни одновременных тестов: правильная инфраструктура (feature flags, стратификация, ортогональные слои) позволяет Google запускать ~10 000 экспериментов в год без взаимного загрязнения.</p>
      <p><b>Документированная история решений.</b> Каждый тест — запись «что пробовали, что сработало, что нет». Через год новый PM не переизобретает колесо: смотрит в experiment log и видит, что этот эксперимент уже провалился в 2024.</p>
      <p><b>Количественная оценка эффекта.</b> Не просто «лучше/хуже», а «+2.3% конверсии, CI [0.8%; 3.8%]». Бизнес может сразу перевести это в деньги и решить, стоит ли раскатывать.</p>

      <h3>Ограничения — и когда они реально бьют</h3>
      <p><b>Требует большого трафика.</b> Чтобы задетектить подъём конверсии с 2% до 2.1% на уровне $\\alpha=0.05$ и мощности 0.8, нужно под 300k пользователей на группу. У стартапа с 1000 посетителей в день A/B просто не хватит мощности — любой «не найден эффект» ничего не доказывает.</p>
      <p><b>Peeking и раннее подглядывание.</b> Классический фиксированный t/z-тест не допускает повторных проверок: каждый взгляд на p-value до конца теста раздувает ошибку I рода. Исправления — либо последовательный анализ (mSPRT, always-valid p-values), либо байесовский подход.</p>
      <p><b>Novelty effect и краткосрочность.</b> Пользователи реагируют на новое просто потому, что оно новое. Через 2 недели эффект схлопывается. A/B ловит мгновенную реакцию, но не долгосрочную привычку — для неё нужны длинные holdout-группы.</p>
      <p><b>Network effects и SUTVA.</b> В соцсетях, маркетплейсах, многопользовательских продуктах действие одного юзера влияет на другого. Классический сплит нарушает условие «treatment не перетекает между группами» — нужны cluster randomization или switchback-дизайны.</p>
      <p><b>Rare events.</b> Если событие происходит у 0.05% пользователей (например, покупка B2B-подписки), стандартной чувствительности не хватит даже при огромном трафике — на помощь приходят surrogate metrics или CUPED.</p>
      <p><b>Не отвечает на вопрос «почему».</b> Тест показал, что новая страница хуже — но не объяснил, что именно сломалось. Без качественных исследований (интервью, session replay, воронка событий) A/B превращается в слепой перебор гипотез.</p>

      <h3>Когда применять A/B — и когда не стоит</h3>
      <table>
        <tr><th>Применяй A/B когда</th><th>НЕ применяй когда</th></tr>
        <tr>
          <td>Есть достаточный трафик для детекции ожидаемого эффекта (считай sample size заранее)</td>
          <td>Трафика меньше, чем требует power analysis — результат будет либо noise, либо «не значимо» без информации</td>
        </tr>
        <tr>
          <td>Изменение локальное и измеримое одной метрикой (кнопка, копирайтинг, алгоритм ранжирования)</td>
          <td>Изменение системное и затрагивает всех сразу (полный редизайн, смена бренда) — тестируй pre/post или holdback</td>
        </tr>
        <tr>
          <td>Юзеры независимы друг от друга (классический e-commerce)</td>
          <td>Сильные сетевые эффекты (маркетплейс, соцсеть) — нужны кластерные или switchback-тесты</td>
        </tr>
        <tr>
          <td>Цикл обратной связи короткий: эффект виден за часы/дни</td>
          <td>Измеряешь долгосрочный retention / LTV — нужен holdout на месяцы или квази-эксперимент</td>
        </tr>
        <tr>
          <td>Можно честно рандомизировать и не смотреть на промежуточные результаты до конца</td>
          <td>Этически/юридически нельзя показать «худший» вариант части пользователей (регуляторные продукты)</td>
        </tr>
        <tr>
          <td>Событие, которое меришь, происходит часто (конверсия, CTR, engagement)</td>
          <td>Метрика — rare event (&lt;0.1%): используй surrogate, CUPED или observational causal inference</td>
        </tr>
      </table>

      <h3>Альтернативы</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('ab-bayesian')">Bayesian A/B</a></b> — когда трафика мало или нужно смотреть результаты непрерывно без peeking-штрафа. Даёт прямую вероятность $P(B &gt; A)$ вместо p-value.</li>
        <li><b>Multi-armed bandits (Thompson Sampling)</b> — когда цена «проигрышного» варианта высока и хочется уменьшить regret: алгоритм автоматически сливает трафик в лучший рукав. Минус: хуже для чистой оценки эффекта.</li>
        <li><b>Quasi-experiments (diff-in-diff, synthetic control)</b> — когда рандомизировать невозможно (закон, релиз на всех сразу, географический таргетинг). Требует допущений, но даёт причинные выводы из наблюдательных данных.</li>
        <li><b>Interleaving</b> — для ранжирующих систем (поиск, рекомендации): показываем смешанный список из двух алгоритмов одному пользователю. На порядок чувствительнее обычного сплита.</li>
        <li><b>Switchback-эксперименты</b> — для маркетплейсов и сервисов с сетевыми эффектами: попеременно переключаем всю систему между вариантами по времени/региону.</li>
      </ul>
    `,
