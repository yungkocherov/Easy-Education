...
plt.show()  # Выброс 450 виден как отдельная точка</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>Первый шаг EDA в любом проекте.</b> <code>df.describe()</code> за 2 секунды покажет диапазоны, пропуски и «подозрительные» признаки. Без этого не начинается ни один serious ML-пайплайн — иначе ты рискуешь обучать модель на признаке, где 80% значений равно нулю или где скрыт отрицательный возраст.</li>
        <li><b>Продуктовые и бизнес-метрики.</b> Средний чек, медианное время на сайте, DAU/MAU — всё это описательные статистики. Их показывают в дашбордах, по ним принимают решения «выросло / упало / всё стабильно».</li>
        <li><b>SLA и мониторинг производительности.</b> В инженерии задержек используют не среднее, а <b>перцентили</b>: p50 (медиана), p95, p99, p99.9. Среднее задержки — это почти всегда ложь, потому что редкие тормоза (p99) его сильно тянут вверх, но при этом скрывают, насколько плохо живут «хвостовые» пользователи.</li>
        <li><b>Контроль качества (SPC, Six Sigma).</b> Линия производства контролируется по правилу «3 сигмы»: если измерение вышло за пределы $\\mu \\pm 3\\sigma$, станок скорее всего сломался. Вся методология Шухарта построена на std и среднем.</li>
        <li><b>A/B-тестирование.</b> Тест различий средних (t-test, Mann-Whitney) опирается на описательные статистики: средние, дисперсии, размеры выборок. Без них невозможно даже поставить задачу «B лучше A?».</li>
        <li><b>Feature engineering.</b> Стандартизация ($x - \\mu)/\\sigma$, нормализация через квантили, детекция выбросов по правилу IQR — всё это описательная статистика, встроенная в препроцессинг почти любой ML-модели.</li>
        <li><b>Финансы и риск.</b> Value at Risk — это 5-й перцентиль доходности. Волатильность актива — это std доходностей. Вся количественная аналитика на Wall Street построена на этих базовых понятиях.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Мгновенная интерпретация.</b> Любой менеджер понимает «средний чек вырос на 5%» или «медианное время доставки — 2 дня». Не нужно объяснять модель, доверительные интервалы или гиперпараметры — цифра говорит сама за себя. Это делает описательную статистику самым переиспользуемым языком в бизнесе.</p>
      <p><b>Вычислительная дешевизна.</b> Среднее, медиана, std считаются за один проход по данным — $O(n)$. Это позволяет делать их в real-time над потоками событий, в Spark над террабайтами, в Pandas над миллионами строк. Для больших данных это критично: многие «умные» модели просто не влезают в память.</p>
      <p><b>Фундамент для всего остального.</b> Любой статистический тест, любая ML-модель, любая визуализация в итоге сводится к вычислению описательных характеристик. Не знаешь разницы между mean и median — не поймёшь, почему t-тест соврал на log-normal данных.</p>
      <p><b>Робастные варианты есть.</b> Если среднее и std ломаются на выбросах, берёшь медиану и IQR (или MAD — медианное абсолютное отклонение). Это не «меньше информации», это <b>другая</b> информация: устойчивая к 1-2 экстремальным точкам. У тебя всегда есть выбор между «чувствительным» и «робастным» семейством характеристик.</p>
      <p><b>Перцентили отвечают на правильные вопросы.</b> «Как живут худшие 5% пользователей?» — p95 задержки. «Где граница топ-10% клиентов?» — p90 выручки. Одним числом описать «хвост» распределения — то, чего среднее и std сделать не могут в принципе.</p>
      <p><b>Связаны с визуализацией.</b> Boxplot — это буквально описательная статистика в картинке: min, Q1, median, Q3, max и выбросы. Histogram + mean/median — стандартный EDA-слайд. Эти инструменты усиливают друг друга: числа + график всегда лучше, чем что-то одно.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Среднее ломается на скошенных распределениях.</b> Зарплаты, время отклика, размеры файлов, цены — всё это log-normal или с тяжёлым правым хвостом. Среднее тянет вверх несколько экстремальных значений, и «средняя зарплата в компании 200к» не отражает реальности, где медиана — 80к. Правило: для скошенных данных всегда медиана, а не среднее.</p>
      <p><b>Std катастрофически чувствителен к выбросам.</b> Одно экстремальное значение — и дисперсия выросла в разы. Это ломает правило «3 сигмы», ломает стандартизацию признаков, ломает статистические тесты. Робастная альтернатива — MAD или IQR.</p>
      <p><b>Описательные статистики скрывают форму распределения.</b> <a class="glossary-link" onclick="App.selectTopic('anscombe-quartet')">Квартет Анскомба</a> — четыре датасета с одинаковыми mean, std и корреляцией, но совершенно разными формами (прямая, парабола, выброс, и т.д.). Смотреть только на числа — значит не видеть дырок в данных. Всегда рисуй график рядом.</p>
      <p><b>Бимодальные и мультимодальные данные.</b> Если в данных два пика (например, рост мужчин и женщин в одном массиве), среднее попадает ровно между ними — в область, где плотность минимальна. Это типичное «число, которое описывает никого». Мода тут тоже помогает плохо — их две. Решение: сегментировать данные перед подсчётом.</p>
      <p><b>Парадокс Симпсона.</b> Агрегированное среднее по всем данным может показывать тренд, <b>противоположный</b> трендам в подгруппах. Классика: лекарство помогает и мужчинам, и женщинам, но в агрегате «не работает», потому что в одной группе его давали чаще. Описательная статистика на полном датасете тут врёт системно — нужна разбивка по признакам.</p>
      <p><b>Одномерность.</b> Mean, median, std ничего не говорят о <b>связях</b> между переменными. Для этого нужна корреляция, ковариация, парные графики. Можно иметь идеально «нормально выглядящие» признаки, которые друг с другом коррелируют на 0.99 и несут один сигнал — описательная статистика не поймает.</p>

      <h3>🧭 Когда применять — и когда не стоит</h3>
      <table>
        <tr><th>✅ Применяй когда</th><th>❌ НЕ применяй (или будь осторожен) когда</th></tr>
        <tr>
          <td>Начинаешь новый проект и нужен быстрый EDA — <code>describe()</code> первой командой</td>
          <td>Данные сильно скошены, а ты берёшь среднее — возьми медиану и перцентили</td>
        </tr>
        <tr>
          <td>Строишь дашборд для бизнеса — median, p95, top-N понятны всем</td>
          <td>Распределение бимодальное или мультимодальное — среднее попадает в «яму» между пиками</td>
        </tr>
        <tr>
          <td>Нужно поймать поломку на production: метрика вылетела за $\\mu \\pm 3\\sigma$ — алерт</td>
          <td>Агрегируешь по всем данным без разбивки — есть риск парадокса Симпсона</td>
        </tr>
        <tr>
          <td>SLA/perf-аналитика — считай p50, p95, p99, а не «среднюю задержку»</td>
          <td>В данных много выбросов, а ты считаешь среднее и std — бери медиану и MAD</td>
        </tr>
        <tr>
          <td>Feature engineering для ML — нужны $\\mu$ и $\\sigma$ для стандартизации</td>
          <td>Нужно понять форму распределения — одних чисел мало, рисуй histogram/KDE/boxplot</td>
        </tr>
        <tr>
          <td>Нужно быстро передать сложный датасет на словах: «медиана 40, IQR 30–55»</td>
          <td>Нужны связи между переменными — используй корреляции, scatter, PCA</td>
        </tr>
        <tr>
          <td>Описываешь робастно: медиана, IQR, MAD — устойчиво к выбросам</td>
          <td>Данные — временные ряды с трендом/сезонностью, и ты считаешь «среднее» — теряется вся динамика</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('viz-histogram')">Гистограмма и KDE</a></b> — когда нужно увидеть форму распределения целиком, а не сводить её к 2-3 числам. Бимодальность, асимметрию, «дырки» между кластерами видно сразу.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('viz-box-plot')">Boxplot / violin plot</a></b> — визуальный компакт-эквивалент описательной статистики: медиана, квартили, выбросы, с возможностью сравнить несколько групп рядом.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('distributions')">Параметрические распределения</a></b> — когда хочешь не просто описать, а <b>смоделировать</b> данные: подогнать normal/log-normal/Poisson и использовать для прогноза и симуляций.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('glossary-bootstrap')">Бутстрэп</a></b> — когда нужны доверительные интервалы для любой статистики (медианы, квантилей, корреляций), а параметрические формулы не подходят.</li>
        <li><b>Робастные статистики (trimmed mean, Winsorized mean, MAD)</b> — если выбросы есть, но ты не хочешь полностью переходить на медиану. Компромисс между чувствительностью и устойчивостью.</li>
      </ul>
    `,
    ...
    extra: `
      ...
      <p>При расчёте дисперсии по выборке мы оцениваем $\\bar{x}$, которое само зависит от данных. Это «съедает» одну <a class="glossary-link" onclick="App.selectTopic('glossary-degrees-of-freedom')">степень свободы</a>. Деление на $n-1$ компенсирует это и даёт несмещённую оценку.</p>
