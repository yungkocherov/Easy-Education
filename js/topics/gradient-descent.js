...
App.registerTopic({
  ...
  tabs: {
    theory: `
      ...
      <h3>🚀 Улучшения базового SGD</h3>
      <p>На практике «голый» SGD используется редко — его дополняют двумя приёмами:</p>
      <ul>
        <li><b>Momentum</b> — накапливает «скорость» в направлении движения, быстрее проходит плато и гасит колебания.</li>
        <li><b>Adaptive learning rate</b> — у каждого параметра свой η, подстраивается под историю градиентов.</li>
      </ul>
      <p>Сегодняшний дефолт — <b>Adam / AdamW</b>: комбинация momentum + адаптации, работает «из коробки» для большинства задач.</p>
      <p class="callout tip">📚 Полные формулы, сравнение и когда какой оптимизатор выбрать — в глоссарной статье <a onclick="App.selectTopic('glossary-optimizers')">Оптимизаторы (SGD, Adam)</a>.</p>
...
print(f"Найдены: w={w:.3f} (истинное 3), b={b:.3f} (истинное 2)")
    ...
    applications: `
      ...
      <ul>
        <li><b>Обучение нейронных сетей — вообще всех.</b> Backprop считает градиент, оптимизатор (SGD, Adam, AdamW) делает шаг. ChatGPT, Stable Diffusion, AlphaFold — всё обучено градиентными методами. Без этого современного ML просто не существует.</li>
        <li><b>Линейная и логистическая регрессия на больших данных.</b> Когда аналитическое решение $(X^T X)^{-1} X^T y$ не вмещается в память или слишком медленное ($p &gt; 10^4$), SGD даёт приближённое решение за линейное время.</li>
        <li><b>Матричные факторизации и рекомендательные системы.</b> ALS, SVD++, factorization machines — всё это разновидности SGD для минимизации ошибки восстановления матрицы оценок.</li>
        <li><b>Обучение с подкреплением.</b> Policy gradient, actor-critic, PPO, DDPG — все RL-алгоритмы используют градиентный подъём по ожидаемой награде. AlphaGo, роботы, автопилоты — на этом.</li>
        <li><b>Online learning и стриминг.</b> SGD обновляет модель по одному примеру — идеально для потоковых данных (ленты новостей, биржевые котировки), где полный проход по датасету невозможен.</li>
        <li><b>Оптимизация гиперпараметров и meta-learning.</b> MAML, hypergradient descent — градиенты по градиентам. Нетривиально, но работает.</li>
        <li><b>Adversarial examples и generative models.</b> GAN-ы, diffusion models, adversarial attacks — всё градиентные игры между сетями.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Универсальность.</b> Градиентный спуск работает для любой дифференцируемой функции потерь. Это единственный метод, который одинаково применим к линейной регрессии на 3 признаках и к модели на 175 миллиардов параметров. Никакой другой класс оптимизаторов такой масштабируемостью не обладает.</p>
      <p><b>Линейная сложность по данным.</b> SGD делает шаг за $O(1)$ по размеру датасета, а не $O(n)$ или $O(n^3)$. Это принципиально — без SGD обучение на миллионах примеров было бы невозможно: full-batch gradient descent на ImageNet занял бы недели на одну эпоху.</p>
      <p><b>Отличное параллелится.</b> Mini-batch SGD раскладывается по ядрам GPU: тысячи примеров обрабатываются одновременно, градиенты суммируются. Вся индустрия deep learning построена именно на этом — поэтому GPU/TPU и стали стандартом.</p>
      <p><b>Шум — это фича, а не баг.</b> Стохастический градиент шумный, и этот шум помогает вылезать из седловых точек и плоских локальных минимумов. В высоких размерностях (миллионы параметров) седловые точки встречаются чаще локальных минимумов, так что шум SGD буквально необходим.</p>
      <p><b>Онлайн-обучение.</b> SGD может учиться по одному примеру — не нужно собирать датасет, не нужно перетренировывать всё с нуля. Это снимает ограничение на размер памяти и позволяет моделям адаптироваться к новым данным в реальном времени.</p>
      <p><b>Адаптивные варианты (Adam, AdamW) почти не требуют настройки.</b> Где обычный SGD требовал недель поиска правильного learning rate и schedule, Adam с дефолтами (lr=1e-3, betas=(0.9, 0.999)) сходится за разумное время на 90% задач. Это резко снизило порог входа в нейросети.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Жёсткая зависимость от learning rate.</b> Слишком большой — loss взрывается или колеблется; слишком маленький — обучение занимает вечность. Разница между 1e-3 и 3e-3 может означать «модель сошлась за час» и «модель не сошлась вообще». Это самая частая причина провала обучения у новичков.</p>
      <p><b>Локальные минимумы и плато.</b> Для невыпуклых функций (все нейросети) градиент может «застрять» в плохом минимуме или на долгом плато. Моментум и адаптивные методы помогают, но гарантии глобального минимума нет — приходится надеяться на шум и правильную инициализацию.</p>
      <p><b>Нестабильность в больших глубинах.</b> Градиенты могут затухать (vanishing) или взрываться (exploding) в глубоких сетях. Лечится инициализацией (He, Xavier), нормализацией (BatchNorm, LayerNorm), gradient clipping — но это всё костыли поверх фундаментальной проблемы.</p>
      <p><b>Adam запоминает масштаб градиентов.</b> При дообучении (fine-tuning) momentum-статистики из предыдущей задачи мешают. Часто приходится «разогревать» оптимизатор (warmup) или сбрасывать состояние.</p>
      <p><b>Медленный около минимума.</b> В окрестности оптимума градиент становится очень маленьким, а шум SGD — большим. Для точных решений (например, оптимизация с допуском $10^{-8}$) SGD не годится — лучше L-BFGS или аналитика.</p>
      <p><b>Требует дифференцируемости.</b> Любое «if-else», округление, argmax — непропускают градиент. Приходится придумывать гладкие аппроксимации (softmax, straight-through estimator, Gumbel) — не всегда честные.</p>

      <h3>🧭 Когда применять — и когда не стоит</h3>
      <table>
        <tr><th>✅ Что выбрать</th><th>❌ Когда подход провалится</th></tr>
        <tr>
          <td><b>Full-batch GD:</b> небольшие датасеты (&lt; 10k), выпуклые задачи, нужна точная сходимость</td>
          <td>Большие данные — один шаг требует прохода по всему датасету, невозможно масштабировать</td>
        </tr>
        <tr>
          <td><b>Mini-batch SGD (32–512):</b> стандарт для нейросетей, хороший баланс скорости и стабильности, идеален для GPU</td>
          <td>Очень маленькие датасеты — дрожь градиента мешает сходимости</td>
        </tr>
        <tr>
          <td><b>Pure SGD (batch=1):</b> online learning, стриминг, экстремально большие модели</td>
          <td>Сильно зашумлённая сходимость, плохо параллелится на GPU — не используют в deep learning</td>
        </tr>
        <tr>
          <td><b>SGD + Momentum:</b> свёрточные сети, классификация изображений, хорошо изученные архитектуры</td>
          <td>Transformer-ы, новая архитектура, нужно минимальное время на настройку — Adam проще</td>
        </tr>
        <tr>
          <td><b>Adam / AdamW:</b> трансформеры, NLP, RL, любые нестандартные архитектуры, быстрый прототип</td>
          <td>ConvNets при очень большом батче — SGD+Momentum даёт лучшее обобщение (generalization gap)</td>
        </tr>
        <tr>
          <td><b>RMSprop:</b> RL с нестабильными градиентами, рекуррентные сети</td>
          <td>Современный выбор — Adam почти всегда лучше, RMSprop устарел</td>
        </tr>
        <tr>
          <td><b>L-BFGS / Newton:</b> маленькие параметрические модели, научные задачи, нужна высокая точность</td>
          <td>Нейросети с миллионами параметров — $O(p^2)$ память для гессиана невозможна</td>
        </tr>
        <tr>
          <td><b>Градиентный спуск вообще:</b> функция дифференцируема, loss гладкий, есть доступ к градиентам</td>
          <td>Дискретная оптимизация, недифференцируемые потери, комбинаторные задачи — возьми эволюционные методы или RL</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы</h3>
      <ul>
        <li><b>L-BFGS и квази-ньютоновские методы</b> — используют приближение гессиана, сходятся за меньше итераций. Хороши для выпуклых задач с &lt; 10k параметров (логистическая регрессия, условные поля).</li>
        <li><b>Coordinate Descent</b> — оптимизируем по одной координате за раз. Стандарт для LASSO/ElasticNet (<code>sklearn.linear_model</code> использует именно его). Отлично работает при разреженных данных.</li>
        <li><b>Аналитическое решение</b> — для линейной регрессии есть закрытая формула $\\hat\\beta = (X^T X)^{-1} X^T y$. Если параметров мало и $X^T X$ обратима, это быстрее и точнее любого SGD.</li>
        <li><b>Эволюционные алгоритмы (CMA-ES, NES)</b> — не требуют градиентов, работают с любой чёрной коробкой. Имеют смысл для недифференцируемых задач или очень шумных функций.</li>
        <li><b>Linear/Convex Solvers (CVXPY, Gurobi)</b> — если задача — линейное или квадратичное программирование, специализированные решатели в разы эффективнее градиентного спуска.</li>
      </ul>
    `,
