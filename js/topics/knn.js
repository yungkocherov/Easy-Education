...
plt.show()</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>Рекомендательные системы (item-based и user-based CF).</b> «Найти $k$ самых похожих фильмов/товаров» — буквально kNN в пространстве эмбеддингов. Amazon, Netflix, Spotify используют nearest neighbors поверх ALS/SVD/нейросетевых эмбеддингов как финальный шаг ранжирования.</li>
        <li><b>Поиск похожих изображений и semantic search.</b> После того как CNN или CLIP выдали вектор представления, kNN на этих векторах — самый быстрый способ найти «что на картинке похоже на это». Google Images, обратный поиск, дубликаты в фото-архиве.</li>
        <li><b>Impute пропусков (KNNImputer).</b> Если в строке NaN, можно найти $k$ самых похожих полных строк и усреднить их значения. Гораздо точнее, чем подставлять среднее по столбцу, особенно при корреляции между признаками.</li>
        <li><b>Anomaly detection.</b> Точки с большим расстоянием до $k$-го соседа — кандидаты в аномалии. Используется в мониторинге, fraud detection, контроле качества на производстве. LOF (Local Outlier Factor) — это kNN поверх kNN.</li>
        <li><b>Медицинская диагностика по case-based reasoning.</b> «Пациенты с похожими анализами и симптомами» — врачу интуитивно понятно, чем больше помогает: показать 5 аналогичных случаев и их исходы, чем дать score от blackbox-модели.</li>
        <li><b>Baseline для любой классификации с низкой размерностью.</b> $k$NN за 30 секунд скажет, есть ли вообще сигнал в данных. Если даже он даёт 70% accuracy — данные осмысленны.</li>
        <li><b>Geo-задачи и пространственный поиск.</b> «Ближайший ресторан», «таксисты в радиусе» — это kNN по lat/lon с Haversine distance. KD-Tree или R-Tree делает это за $O(\\log n)$.</li>
      </ul>

      <h3>Сильные стороны — и почему они важны</h3>
      <p><b>Нулевое обучение — lazy learning.</b> Нет фазы fit в привычном смысле: модель — это просто сам датасет. Можно добавлять новые примеры «на лету», без переобучения. Для быстро меняющихся каталогов (новости, товары) это критично: не нужно пересобирать модель каждый час.</p>
      <p><b>Интуитивно понятен любому.</b> «Посмотри на 5 ближайших точек и возьми большинство». Не нужно объяснять градиенты, ядра, функции потерь. Отлично подходит для демонстрации идеи ML нетехническим людям и как первая модель на учебном курсе.</p>
      <p><b>Автоматически ловит нелинейные границы.</b> Не нужно ядер, feature engineering или глубоких архитектур. Если два класса образуют спираль или кольцо — $k$NN справится без единой настройки, в отличие от логрега.</p>
      <p><b>Нативно multi-class.</b> Где логрегу и SVM нужны OvO/OvR-обёртки, kNN из коробки работает с любым числом классов: просто считаем голоса соседей. И легко расширяется до multi-label.</p>
      <p><b>Нет предположений о распределении данных.</b> Непараметрический метод — не требует нормальности, линейности, независимости. Данные «как есть» — и метод работает, если есть структура в пространстве расстояний.</p>
      <p><b>Легко делать explanation: «вот похожие примеры».</b> Вместо SHAP-значений или feature importance можно показать пользователю реальные $k$ ближайших объектов — это форма объяснения, которую понимают все.</p>

      <h3>Ограничения — и когда они реально бьют</h3>
      <p><b>Проклятие размерности.</b> В $d &gt; 20$ все точки становятся почти одинаково далёкими: отношение максимального расстояния к минимальному стремится к 1. Понятие «ближайший» теряет смысл, точность падает. Именно поэтому в задачах на сырых изображениях или текстах kNN работает только после снижения размерности (PCA, эмбеддинги).</p>
      <p><b>$O(n)$ на каждое предсказание.</b> В наивной реализации для каждой новой точки нужно пройти весь train. На миллионе точек и real-time запросах это катастрофа. KD-Tree помогает в низкой размерности, FAISS/HNSW — в высокой, но всё это требует дополнительной инфраструктуры.</p>
      <p><b>Обязательное масштабирование.</b> Евклидово расстояние складывает квадраты по всем осям. Если «цена» в рублях ($10^6$) и «возраст» в годах ($10^1$) — цена будет доминировать, возраст просто не учтётся. StandardScaler или MinMaxScaler — обязательны, забыл — модель мусор.</p>
      <p><b>Чувствителен к нерелевантным признакам.</b> Каждая шумовая колонка портит расстояние. Если из 100 признаков важны 10, а остальные случайны, они потопят сигнал. Деревья просто проигнорируют ненужные признаки — kNN нет.</p>
      <p><b>Большая память.</b> Модель хранит весь обучающий датасет. На 100 ГБ данных это проблема и для RAM, и для деплоя в контейнер. Нет компрессии через «выученные веса», как у параметрических методов.</p>
      <p><b>Дисбаланс классов ломает голосование.</b> Если негативов в 100 раз больше, чем позитивов, среди 5 ближайших почти всегда будут негативы — модель никогда не предскажет редкий класс. Нужны веса по расстоянию, undersampling или распределённые метрики.</p>

      <h3>Когда брать kNN — и когда точно не стоит</h3>
      <table>
        <tr><th>Бери kNN когда</th><th>НЕ бери когда</th></tr>
        <tr>
          <td>Размерность низкая ($d &lt; 20$) после очистки или снижения</td>
          <td>Очень высокая размерность сырых данных (изображения, длинные тексты) без эмбеддингов</td>
        </tr>
        <tr>
          <td>Нужна быстрая модель без обучения и с простым объяснением «вот похожие»</td>
          <td>Нужен real-time инференс на миллионах точек без ANN-индекса</td>
        </tr>
        <tr>
          <td>Граница классов нелинейна и нет желания возиться с ядрами или сетями</td>
          <td>Данных очень много (десятки миллионов) и нет ресурса держать датасет в памяти</td>
        </tr>
        <tr>
          <td>Задача — импутация пропусков или поиск похожих объектов (рекомендации, дубликаты)</td>
          <td>Среди признаков много нерелевантных и нет возможности их отфильтровать</td>
        </tr>
        <tr>
          <td>Данные быстро меняются — не хочется переобучать модель каждый час</td>
          <td>Сильный дисбаланс классов — редкий класс будет затоплен большинством</td>
        </tr>
        <tr>
          <td>Нужен простой baseline «есть ли сигнал в данных» за 30 секунд</td>
          <td>Нужно объяснять модель через коэффициенты или feature importance</td>
        </tr>
      </table>

      <h3>Альтернативы — что взять вместо и почему</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('logistic-regression')">Логистическая регрессия</a></b> — если граница классов близка к линейной и нужна интерпретация через коэффициенты. Быстрее на инференсе ($O(d)$ вместо $O(n)$) и даёт откалиброванные вероятности.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('decision-tree')">Дерево решений</a> или <a class="glossary-link" onclick="App.selectTopic('random-forest')">Random Forest</a></b> — если среди признаков есть мусорные или категориальные. Деревья автоматически игнорируют нерелевантные, не требуют масштабирования и дают интерпретацию через feature importance.</li>
        <li><b>FAISS / HNSW / ScaNN</b> — если сам подход kNN подходит, но нужен скейл: миллиарды векторов, миллисекундный отклик. Это не замена идеи, а её продакшен-реализация через approximate nearest neighbors.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('neural-network')">Нейросеть</a> с эмбеддингами</b> — если нужно учить само пространство расстояний (metric learning, triplet loss) под задачу. Это то, что делают современные поисковики и рекомендательные системы: kNN поверх выученных эмбеддингов.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('svm')">SVM</a> с RBF-ядром</b> — если граница сложная, но данных мало (до 10к). Обычно точнее, чем kNN, и не хранит весь датасет для инференса.</li>
      </ul>
    `,
