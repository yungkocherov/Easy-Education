...
App.registerTopic({
  ...
  tabs: {
    theory: `
      ...
      <h3>Экстраполяция: главная слабость</h3>
      ...
      <h3>Регуляризация через параметры</h3>
      <ul>
...
# Обратите внимание: за x=10 линия "застывает" — это экстраполяция</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>Оценка стоимости недвижимости с объяснением.</b> Когда клиенту нужно сказать «почему именно 8 млн»: «площадь &gt; 60 м² И район = центр И этаж &gt; 3 → средняя цена 8.2 млн». Правила из дерева легко превращаются в прайс-лист или бизнес-логику, которую принимает регулятор.</li>
        <li><b>Сегментация по финансовым показателям.</b> Банки и маркетинг используют регрессионные деревья, чтобы разбить клиентов на группы с разным ожидаемым LTV / доходом. Каждый лист = сегмент + его средний чек + понятное правило, по которому клиент туда попал.</li>
        <li><b>Прогноз производственных показателей.</b> Расход материалов, время сборки, износ оборудования — задачи, где инженеру нужна не чёрная коробка, а «если температура &gt; X и скорость &gt; Y — расход будет Z», чтобы можно было вручную подкрутить параметры.</li>
        <li><b>Исследовательский анализ данных.</b> Дерево глубины 3-4 на сырых данных — быстрый способ увидеть, где находятся основные «уступы» в целевой переменной и какие признаки их создают. Часто ставят перед любой серьёзной моделью как первый взгляд на структуру.</li>
        <li><b>Surrogate model для интерпретации.</b> Обучают дерево на предсказаниях чёрного ящика (XGBoost, нейросеть), чтобы получить «объяснялку». Дерево приближает поведение сложной модели в виде правил, которые можно показать заказчику.</li>
        <li><b>Базовый блок для ансамблей.</b> Random Forest и Gradient Boosting — это десятки–сотни регрессионных деревьев. Понимание одного дерева = понимание основы всего tabular ML.</li>
      </ul>

      <h3>Сильные стороны — и почему они важны</h3>
      <p><b>Максимальная интерпретируемость в регрессии.</b> В отличие от RF или GBM, одно дерево можно нарисовать, показать на презентации и перевести в набор if/else. Это критично для регулируемых отраслей (банки, медицина, страхование) и для задач, где прогноз должен объяснять себя.</p>
      <p><b>Нелинейность без feature engineering.</b> Дерево автоматически находит «уступы» в данных: если цена резко растёт после 70 м² — дерево поставит там порог. Линейной регрессии пришлось бы явно добавлять бинарные признаки <code>area &gt; 70</code>.</p>
      <p><b>Автоматические взаимодействия признаков.</b> Последовательные разбиения создают комбинации: «площадь &gt; 60 И этаж &gt; 5 → цена дороже». Линейная модель такое взаимодействие должна получить через произведение признаков, заданное руками.</p>
      <p><b>Не требует масштабирования.</b> Разбиение идёт по порогу <code>x_j &gt; t</code> — масштаб не важен. Можно смешивать площадь в м² и цену в рублях, дереву всё равно. Один этап препроцессинга можно выбросить.</p>
      <p><b>Устойчиво к выбросам (с MAE-критерием).</b> Если разбиение оптимизирует MAE вместо MSE, одиночный выброс не смещает порог. Это удобно для «грязных» продакшен-данных.</p>
      <p><b>Быстрое обучение и предсказание.</b> Одно дерево глубины 10 — это миллисекунды на обучение и микросекунды на предсказание. Подходит для low-latency API, где RF/GBM слишком тяжёлые.</p>

      <h3>Ограничения — и когда они реально бьют</h3>
      <p><b>Не экстраполирует — ответ «застывает».</b> Листья возвращают среднее обучающих $y$. Если обучал на доходах до 200 тыс., а в тесте пришёл 500 тыс. — модель вернёт «среднее по той группе 200-тысячников». Для любого тренда — продаж, роста, временных рядов — дерево бесполезно как прогнозёр.</p>
      <p><b>Кусочно-постоянная аппроксимация.</b> Между порогами значение фиксировано — визуально получается «лесенка». Для гладких зависимостей (физика, климат) это выглядит уродливо и даёт системные ошибки на переходах между листьями.</p>
      <p><b>Высокая дисперсия — одно дерево нестабильно.</b> Добавь в обучающую выборку одну точку — корень может смениться, и дерево перестроится целиком. Это делает одно дерево плохим выбором для продакшена (лучше RF или GBM, где усреднение гасит дисперсию).</p>
      <p><b>Катастрофически переобучается без ограничений.</b> Дерево растёт до листьев с одной точкой = $R^2 = 1$ на train, $R^2 &lt; 0$ на test. Обязательны <code>max_depth</code>, <code>min_samples_leaf</code> или ccp_alpha. Без них модель бесполезна.</p>
      <p><b>Проигрывает RF и GBM 5-20% RMSE.</b> Одно дерево — это почти всегда бейзлайн, а не финальная модель. Ансамбль из 100 деревьев даёт точность, которую одним деревом принципиально не достичь.</p>
      <p><b>Не ловит диагональные и гладкие границы.</b> Разбиения параллельны осям — чтобы приблизить зависимость вида $y = x_1 + x_2$, дерево строит лесенку из десятков уступов. Линейная регрессия решает эту задачу одной формулой.</p>

      <h3>Когда брать Decision Tree Regression — и когда точно не стоит</h3>
      <table>
        <tr><th>Бери дерево регрессии когда</th><th>НЕ бери когда</th></tr>
        <tr>
          <td>Нужна максимальная интерпретируемость для регулятора / эксперта / заказчика</td>
          <td>Нужна максимальная точность — Random Forest или XGBoost почти всегда лучше</td>
        </tr>
        <tr>
          <td>Задача регрессии с выраженными «уступами» в данных (сегментация по y)</td>
          <td>Зависимость гладкая и близка к линейной — возьми линейную регрессию</td>
        </tr>
        <tr>
          <td>Нужно построить правила «если → то», которые превратятся в бизнес-логику</td>
          <td>Задача требует экстраполяции — временные ряды с трендом, прогноз роста</td>
        </tr>
        <tr>
          <td>Нужен surrogate для объяснения сложной модели</td>
          <td>Высокая дисперсия критична — одна точка не должна менять всю модель</td>
        </tr>
        <tr>
          <td>Учишь новичков: показать, как дерево разбивает пространство признаков</td>
          <td>Данные неструктурированные (изображения, текст, временные ряды)</td>
        </tr>
        <tr>
          <td>Маленький датасет, и ты хочешь понять его структуру через глубину 3-4</td>
          <td>Нужна гладкая функция для презентации или физических процессов</td>
        </tr>
      </table>

      <h3>Альтернативы — что взять вместо и почему</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('random-forest-regression')">Random Forest Regression</a></b> — если нужна устойчивость и точность, а интерпретируемость отдельного правила не критична. Усреднение сотен деревьев гасит дисперсию и даёт +5-15% $R^2$.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('gradient-boosting-regression')">Gradient Boosting Regression</a></b> — если важна максимальная точность на табличных данных. XGBoost/LightGBM/CatBoost почти всегда бьют одно дерево на 10-20% RMSE.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('linear-regression')">Линейная регрессия</a> / Ridge / Lasso</b> — если зависимость близка к линейной или нужна экстраполяция. Интерпретируется через коэффициенты (вместо правил) и умеет предсказывать «за пределами обучающих данных».</li>
        <li><b>Spline / GAM (Generalized Additive Models)</b> — если нужна гладкая нелинейность с сохранением интерпретируемости. Каждый признак моделируется отдельной сплайновой функцией — видно, как именно он влияет на прогноз.</li>
      </ul>
    `,
