        ...
        content: `
          ...
          <div class="step" data-step="6">
            ...
            <div class="calc">
              ...
              97.5-й перцентиль ≈ <b>0.861</b><br><br>
            ...
            <div class="why">Credible interval (байесовский): «параметр $p$ лежит в [0.387, 0.861] с вероятностью 95%». Это прямая интерпретация! В отличие от confidence interval, который так интерпретировать нельзя.</div>
          </div>
          ...
          <div class="answer-box">
            <div class="answer-label">Ответ</div>
            <p>Prior Beta(2,2) + 7 орлов из 10 = Posterior Beta(9,5). E[p] сдвинулся с 0.500 к 0.643 (между prior и MLE). 95% CI: [0.387, 0.861] — ещё включает 0.5. При 70/100: posterior Beta(72,32), E[p]=0.692, CI не включает 0.5 — монета нечестная.</p>
          </div>
...
plt.show()</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>Спам-фильтрация и модерация.</b> Multinomial Naive Bayes — классика жанра: Gmail, SpamAssassin, корпоративные фильтры. Модель учится на миллионах писем за секунды и даёт приемлемую точность практически «из коробки».</li>
        <li><b>Тематическая классификация текстов.</b> Новостные агрегаторы, классификация тикетов поддержки, определение языка. Naive Bayes остаётся сильным baseline-ом даже в эпоху трансформеров — быстр и не требует GPU.</li>
        <li><b>Sentiment analysis по подсчёту слов.</b> Быстрая оценка тональности отзывов, комментариев, соцмедиа. Для коротких текстов с явной лексикой NB часто близок по качеству к тяжёлым моделям.</li>
        <li><b>Медицинская диагностика и обновление рисков.</b> Байесовский подход естественно отвечает на вопрос «у пациента положительный тест — какова вероятность болезни?». Это не NB, а общий Bayes theorem — его учат во всех медицинских программах по статистике.</li>
        <li><b>Байесовские A/B-тесты.</b> Вместо p-values — прямая вероятность $P(B &gt; A)$, которую понимает продакт. PyMC, BayesTest, внутренние фреймворки крупных компаний — всё построено на Beta-Bernoulli модели.</li>
        <li><b>Bayesian optimization для гиперпараметров.</b> Optuna, Hyperopt, scikit-optimize используют байесовскую модель над функцией качества — эффективнее случайного поиска при дорогих экспериментах (обучение нейросети).</li>
        <li><b>Bayesian deep learning и оценка неопределённости.</b> Variational Inference, MC Dropout, deep ensembles — когда модели нужно говорить «я не знаю», а не выдавать уверенные мусорные предсказания (беспилотники, медицина).</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Экстремальная скорость обучения и предсказания.</b> Naive Bayes — это подсчёт частот и одно перемножение вероятностей. Обучение на миллионе писем занимает секунды, предсказание — микросекунды. Для фильтров в реальном времени это решающий фактор.</p>
      <p><b>Работает на маленьких данных.</b> NB достаточно сотен примеров, а не десятков тысяч. Байесовский подход (с приором) ещё лучше: ты явно вкладываешь в модель знание эксперта, когда данных мало. В медицине и биологии это часто единственный честный подход.</p>
      <p><b>Естественные вероятности и uncertainty.</b> Байесовские методы дают не точку, а распределение. Ты знаешь, насколько уверен в ответе — что критично для принятия решений с асимметричными последствиями (отказ от операции, автопилот).</p>
      <p><b>Инкрементальное обновление.</b> Новые данные не требуют переобучения с нуля: posterior одного этапа становится prior следующего. Идеально для онлайн-систем, где данные приходят потоком.</p>
      <p><b>Устойчивость к нерелевантным признакам.</b> NB слабо страдает от шумных фич — их условные вероятности близки к равномерным и почти не сдвигают решение. Это выгодно отличает его от k-NN или SVM.</p>
      <p><b>Интерпретируемость.</b> Можно буквально посмотреть: «слово viagra встречается в спаме в 400 раз чаще» — и объяснить решение клиенту. В регулируемых отраслях это полезно.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Предположение независимости.</b> «Naive» в названии не случайно: NB считает, что признаки независимы при известном классе. В реальности слова в тексте коррелируют, пиксели — тоже. Модель работает «вопреки» своему допущению, но на данных со сложными взаимодействиями (изображения, табличка с фичами-дубликатами) проигрывает бустингу катастрофически.</p>
      <p><b>Плохая калибровка вероятностей.</b> NB часто выдаёт $0.0001$ или $0.9999$ — слишком уверен. Для ранжирования это ещё терпимо, но как «честная вероятность» для принятия решения с costs — нет. Нужен CalibratedClassifierCV сверху.</p>
      <p><b>Zero-frequency problem.</b> Если слово не встречалось в обучении, его $P(w|c)=0$ обнуляет всё произведение. Laplace/add-one smoothing обязательна — без неё модель ломается на первом же незнакомом токене.</p>
      <p><b>Общий байесовский подход — дорого.</b> MCMC, Variational Inference, PyMC/Stan требуют часов вычислений на данных, где sklearn бы отработал за секунды. Плюс высокий порог входа: надо понимать приоры, сходимость цепей, ESS, R-hat.</p>
      <p><b>Выбор приора — субъективен.</b> Байесовский метод честно зависит от приора. Два эксперта с разными приорами на одних данных получат разные выводы. В академических публикациях это повод для критики, в индустрии — для бесконечных споров.</p>

      <h3>🧭 Когда брать байесовский подход — и когда точно не стоит</h3>
      <table>
        <tr><th>✅ Бери байесовский/NB когда</th><th>❌ НЕ бери когда</th></tr>
        <tr>
          <td>Задача — классификация текстов с простой лексикой и нужен быстрый baseline</td>
          <td>Данные — изображения, звук, сложные табличные фичи с взаимодействиями</td>
        </tr>
        <tr>
          <td>Данных очень мало и есть экспертное знание для приора</td>
          <td>Данных миллионы и latency не критичен — XGBoost/нейросеть дадут выше качество</td>
        </tr>
        <tr>
          <td>Нужна честная оценка неопределённости (медицина, риски, беспилотники)</td>
          <td>Нужна лучшая возможная точность и uncertainty вторична</td>
        </tr>
        <tr>
          <td>Важна скорость инференса: миллионы предсказаний в секунду на CPU</td>
          <td>Есть сильные корреляции между признаками и NB заведомо сломается</td>
        </tr>
        <tr>
          <td>Нужно обновлять модель потоково, без переобучения с нуля</td>
          <td>Задача требует откалиброванных вероятностей «из коробки» — бери логрег</td>
        </tr>
        <tr>
          <td>A/B-тесты, где продакту нужна вероятность «B лучше A», а не p-value</td>
          <td>Команда не готова инвестировать в PyMC/Stan и разбираться в MCMC</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы — что взять вместо и почему</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('logistic-regression')">Логистическая регрессия</a></b> — такой же быстрый линейный классификатор, но без допущения независимости. Лучше калибрована, обычно точнее на тех же текстах. Стандартная замена NB как baseline.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('gradient-boosting')">Gradient Boosting</a> (XGBoost/LightGBM)</b> — если данных много и точность важнее скорости. Ловит взаимодействия признаков, которые NB игнорирует.</li>
        <li><b>Трансформеры и эмбеддинги (BERT, fastText)</b> — для сложного NLP, где важен контекст и семантика, а не просто bag-of-words. Дороже, но качественно другой уровень.</li>
        <li><b>PyMC / Stan / NumPyro</b> — если нужна полноценная байесовская модель с кастомной структурой и честным posterior, а не просто NB.</li>
        <li><b>Conformal prediction</b> — современная альтернатива байесовской неопределённости: даёт гарантии покрытия без приоров и MCMC.</li>
      </ul>
    `,
