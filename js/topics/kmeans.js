        ...
        content: `
          ...
          <div class="step" data-step="4">
            ...
            <div class="calc">
              ...
              A(1,1): d(μ₁)=√(0.25+0.11)=0.60, d(μ₂)=5.46 → <b>К1</b><br>
              B(1.5,2): d(μ₁)=√(0+0.45)=0.67, d(μ₂)=4.48 → <b>К1</b><br>
              C(2,1): d(μ₁)=√(0.25+0.11)=0.60, d(μ₂)=4.71 → <b>К1</b><br>
              D(5,4): d(μ₁)=4.40, d(μ₂)=√(0.11+0.11)=0.47 → <b>К2</b><br>
              E(5,5): d(μ₁)=5.07, d(μ₂)=√(0.11+0.45)=0.75 → <b>К2</b><br>
              F(6,4): d(μ₁)=5.23, d(μ₂)=√(0.45+0.11)=0.75 → <b>К2</b><br><br>
          ...
          <div class="answer-box">
            ...
            <p>K-Means сошёлся за 2 итерации. Кластер 1: {A, B, C} с центром (1.5, 1.33). Кластер 2: {D, E, F} с центром (5.33, 4.33). Inertia = сумма квадратов расстояний до центров ≈ 2.50.</p>
          </div>
        ...
        content: `
          ...
          <div class="step" data-step="2">
            ...
            <div class="calc">
              ...
              2. Квадраты расстояний d² до μ₁:<br>
                 A: 0, B: 1, C: 1.25, D: 98, E: 113, F: 120.25<br>
                 Сумма: 0+1+1.25+98+113+120.25 = 333.5<br><br>
              3. Вероятность выбора ∝ d²:<br>
                 P(D) = 98/333.5 ≈ 0.294<br>
                 P(E) = 113/333.5 ≈ 0.339<br>
                 P(F) = 120.25/333.5 ≈ 0.361 — наибольшая!<br>
                 P(A) = 0 (уже выбрана)<br><br>
              4. Второй центр μ₂: выбираем одну из далёких точек, например E(9,8) с P≈0.34<br>
          ...
          <div class="step" data-step="3">
            ...
            <div class="calc">
              ...
              Inertia ≈ 2.33 (vs ~50 при плохой инициализации)
            </div>
          </div>
          <div class="answer-box">
            <div class="answer-label">Ответ</div>
            <p>Плохая инициализация: Inertia ≈ 50, неверная кластеризация. K-Means++: Inertia ≈ 2.33, идеальная кластеризация. K-Means++ используется по умолчанию (init='k-means++' в sklearn).</p>
          </div>
          <div class="lesson-box">
...
plt.show()</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>Сегментация клиентов в маркетинге.</b> Классический RFM-анализ: группируем пользователей по Recency, Frequency, Monetary и получаем 4-8 «персон» («лояльные», «спящие», «высокочек-одноразовики»). Каждой персоне — своя CRM-кампания, пуш, рассылка.</li>
        <li><b>Сжатие изображений через цветовую квантизацию.</b> Было 16 млн цветов — стало $k=16$ или $k=256$ центроидов-цветов. PNG/GIF используют именно это. Каждый пиксель заменяется на индекс ближайшего центроида — картинка весит в разы меньше.</li>
        <li><b>Vector quantization в speech и audio.</b> Кодеки вроде CELP разбивают сигнал на короткие фреймы, каждый — на ближайший вектор из codebook. Тот же приём в нейросетях: VQ-VAE учит discrete latent space через K-Means-подобный loss.</li>
        <li><b>Генерация features для supervised-моделей.</b> Вычисляем кластер каждой точки и расстояния до всех центроидов — добавляем их как новые признаки. Деревья и линейные модели получают бесплатную нелинейность «где в пространстве находится объект».</li>
        <li><b>Инициализация других алгоритмов.</b> GMM почти всегда стартует с K-Means-центроидов (иначе EM сходится к плохому локальному максимуму). Тот же приём в semi-supervised и active learning — найти «типичные» точки для разметки.</li>
        <li><b>Document clustering и topic discovery.</b> TF-IDF векторы + spherical K-Means (косинусная метрика) — быстрый способ сгруппировать десятки тысяч документов до того, как запускать тяжёлый LDA или BERTopic.</li>
        <li><b>Image colorization и palette extraction.</b> Дизайнерские инструменты («какие 5 главных цветов у этой картинки») — это K-Means по RGB пикселей за несколько миллисекунд.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Линейная сложность $O(n \\cdot k \\cdot d \\cdot T)$.</b> Единственный алгоритм кластеризации, который без боли проворачивает десятки миллионов точек. Mini-batch K-Means добавляет ещё порядок — можно кластеризовать потоково, прямо в онлайн-сервисе.</p>
      <p><b>Интерпретируемые центроиды.</b> Центроид — это «средний клиент» группы: средний чек, средняя частота, средний возраст. Его можно показать маркетологу или продакту без объяснений, что такое «плотность в $\\varepsilon$-окрестности». Для бизнеса это критично.</p>
      <p><b>Почти нулевой тюнинг.</b> Единственный серьёзный гиперпараметр — $k$. И даже его можно подобрать автоматически (elbow, silhouette, gap statistic). Для сравнения: у DBSCAN нужно настраивать eps и min_samples, у GMM — covariance_type и init, у HDBSCAN — min_cluster_size.</p>
      <p><b>Гарантированная сходимость.</b> Алгоритм Ллойда всегда сходится за конечное число итераций (обычно 10-50), потому что функционал $\\sum ||x - \\mu_c||^2$ монотонно убывает. Нет плавающих «зависаний» и деликатного early stopping, как у итеративных байесовских методов.</p>
      <p><b>Тривиальная параллелизация.</b> Шаг assignment — независим по точкам, шаг update — независим по кластерам. Spark MLlib, sklearn with joblib, RAPIDS cuML — везде K-Means «из коробки» раскладывается на все ядра/GPU.</p>
      <p><b>Работает как anomaly detector.</b> Точки, у которых расстояние до ближайшего центроида в топ-1% от максимума, — кандидаты в выбросы. Дёшево, быстро, без отдельной модели.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Нужно заранее знать $k$.</b> Если ты не знаешь, сколько в данных реальных групп (а это — нормальная ситуация в EDA), приходится перебирать $k$ и смотреть elbow/silhouette. На неоднородных данных «локоть» может быть размыт — тогда выбор $k$ становится субъективным.</p>
      <p><b>Предполагает сферические кластеры одинакового размера.</b> Функционал минимизирует евклидово расстояние — это эквивалентно GMM с изотропной ковариацией и равными весами. Для вытянутых эллипсов, полумесяцев и колец K-Means разрезает их пополам и выдаёт бессмыслицу. На «two moons» проваливается позорно.</p>
      <p><b>Чувствителен к инициализации.</b> Алгоритм Ллойда находит локальный минимум. Плохой старт → плохие кластеры. k-means++ сильно снижает риск, но не устраняет — поэтому всегда запускай с <code>n_init=10</code> или больше и бери лучший результат по inertia.</p>
      <p><b>Выбросы смещают центроиды.</b> Один экстремальный клиент с чеком в 100× от среднего «утащит» центроид на себя и испортит всю группу. Решение — K-Medoids (PAM) с манхэттеновой метрикой или предварительная чистка выбросов.</p>
      <p><b>Масштаб признаков доминирует.</b> Если один признак в рублях ($10^6$), а другой в годах (десятки) — евклидово расстояние определяется только рублями, годы становятся невидимыми. Забыл <code>StandardScaler</code> — получил кластеризацию по одной переменной.</p>
      <p><b>Плохо работает на высоких размерностях.</b> В размерности $d &gt; 50$ все расстояния становятся похожими (curse of dimensionality), центроиды перестают быть информативными. Перед K-Means стоит прогнать PCA до 20-30 компонент или использовать косинусную метрику (spherical K-Means).</p>

      <h3>🧭 Когда брать K-Means — и когда точно не стоит</h3>
      <table>
        <tr><th>✅ Бери K-Means когда</th><th>❌ НЕ бери когда</th></tr>
        <tr>
          <td>Кластеры примерно сферические, похожего размера, без сложной топологии</td>
          <td>Кластеры вытянутые, в виде полумесяцев, колец или произвольной формы — бери DBSCAN/HDBSCAN или spectral</td>
        </tr>
        <tr>
          <td>Известно или можно подобрать $k$ через elbow/silhouette</td>
          <td>Число групп принципиально неизвестно и может меняться — бери HDBSCAN (автоматический $k$)</td>
        </tr>
        <tr>
          <td>Данных миллионы и нужна скорость — K-Means линейный по $n$</td>
          <td>В данных много выбросов и нет возможности чистить — возьми K-Medoids или DBSCAN (автошум)</td>
        </tr>
        <tr>
          <td>Нужны интерпретируемые «типичные представители» группы (центроиды)</td>
          <td>Нужны soft вероятности «точка принадлежит кластеру A на 0.7, B на 0.3» — бери GMM</td>
        </tr>
        <tr>
          <td>Бейзлайн для сегментации клиентов, товаров, документов</td>
          <td>Данные в высокой размерности без предварительного PCA — евклидова метрика ломается</td>
        </tr>
        <tr>
          <td>Препроцессинг перед supervised-моделью (кластер-фичи, расстояния до центроидов)</td>
          <td>Признаки на разных масштабах и нет возможности стандартизовать — результат будет мусорный</td>
        </tr>
        <tr>
          <td>Vector quantization для сжатия (цветовая палитра, audio codebook)</td>
          <td>Задача — не кластеризация, а поиск иерархии или аномалий — бери Agglomerative или Isolation Forest</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы — что взять вместо и почему</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('dbscan')">DBSCAN</a></b> — если кластеры произвольной формы и ты не знаешь $k$. Автоматически помечает выбросы как шум. Платишь сложным подбором eps и плохой работой в высокой размерности.</li>
        <li><b>Gaussian Mixture Models (GMM)</b> — если нужны мягкие принадлежности и эллиптические кластеры разного размера. EM-алгоритм дороже K-Means, но ловит ковариации внутри групп.</li>
        <li><b>HDBSCAN</b> — эволюция DBSCAN: автоматический подбор eps, работа с кластерами разной плотности, иерархия. Де-факто стандарт в биоинформатике single-cell.</li>
        <li><b>Hierarchical (Agglomerative)</b> — если нужна дендрограмма и выбор $k$ постфактум по визуальному осмотру. Сложность $O(n^2)$ — только на небольших данных.</li>
        <li><b>Mini-batch K-Means</b> — тот же K-Means, но на мини-батчах; для потоковых/огромных данных, где полный проход дорог.</li>
      </ul>
    `,
    ...
    links: `
      ...
      <h3>📖 Статьи</h3>
      <ul>
        <li><a href="https://habr.com/ru/search/?q=%D0%BA%D0%BB%D0%B0%D1%81%D1%82%D0%B5%D1%80%D0%B8%D0%B7%D0%B0%D1%86%D0%B8%D1%8F%20k-means" target="_blank">Habr: Алгоритм k-means</a> — разбор с примерами на Python</li>
      </ul>
      <h3>📚 Документация</h3>
      <ul>
