...
plt.show()</code></pre>
    ...
    proscons: `
      ...
      <table>
        ...
        <tr>
          <td>Кластеры примерно сферические и одинакового размера</td>
          <td>Кластеры вытянутые, с непрямыми границами → DBSCAN/спектральная</td>
        </tr>
        <tr>
          <td>Известно (или можно подобрать) число кластеров k</td>
          <td>Число кластеров неизвестно → DBSCAN/HDBSCAN</td>
        </tr>
        <tr>
          <td>Большие данные — K-Means линейный по n</td>
          <td>Есть выбросы — они сильно смещают центроиды → K-Medoids</td>
        </tr>
        <tr>
          <td>Нужен быстрый бейзлайн для сегментации клиентов</td>
          <td>Признаки разного масштаба без стандартизации</td>
        </tr>
        <tr>
          <td>Нужны интерпретируемые центроиды как «представители» групп</td>
          <td>Нужны мягкие вероятности принадлежности → Gaussian Mixture</td>
        </tr>
      </table>
      <p><b>Альтернативы:</b> DBSCAN для произвольных форм, GMM для soft clustering, HDBSCAN для плотностной кластеризации с автоматическим k.</p>
    `,
