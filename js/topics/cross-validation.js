...
print(f'Nested CV ROC-AUC: {nested_scores.mean():.3f} ± {nested_scores.std():.3f}')</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>Честная оценка качества перед деплоем.</b> Один train/test split — это одна случайная оценка. K-fold даёт среднее и разброс: «ROC-AUC 0.87 ± 0.02» куда полезнее, чем одно число, когда решаешь, катить модель в прод или нет.</li>
        <li><b>Тюнинг гиперпараметров.</b> GridSearchCV, RandomizedSearchCV, Optuna — все под капотом гоняют кросс-валидацию на каждой комбинации. Без CV подбор превратится в подгонку под один конкретный split.</li>
        <li><b>Сравнение моделей и feature selection.</b> Ответить «LightGBM лучше CatBoost на этих данных?» честно можно только усреднив по фолдам — разница в 0.3% на одном split ничего не значит, на 5 фолдах уже можно судить.</li>
        <li><b>Stacking и ensembling.</b> Out-of-fold (OOF) предсказания — единственный корректный способ собрать обучающую выборку для мета-модели в stacking, иначе леваем будущее в прошлое.</li>
        <li><b>Nested CV для академических отчётов.</b> Публикации, клинические исследования, соревнования с жёстким контролем — везде, где нужно доказать, что качество не натянуто через подбор гиперов, используется вложенная кросс-валидация.</li>
        <li><b>Time-series валидация (walk-forward).</b> Финансы, трафик, спрос — везде, где есть хронология, TimeSeriesSplit позволяет моделировать «как бы мы обучали модель в прошлом и применяли в будущем».</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Каждая точка побывала и в train, и в test.</b> В маленьких датасетах (сотни-тысячи строк) это критично: один неудачный split может дать оценку, которая отличается от истинной на ±10%. K-fold усредняет этот шум и даёт оценку, близкую к правде.</p>
      <p><b>Оценка разброса, а не только среднего.</b> Стандартное отклонение по фолдам показывает, насколько модель чувствительна к выборке. Большой разброс — сигнал, что модель нестабильна и на проде легко сломается.</p>
      <p><b>Защита от «счастливого» split.</b> Без CV легко обмануть себя: выбрал random_state=42, получил хороший результат, побежал деплоить. CV ломает эту привычку и заставляет смотреть на усреднённую картину.</p>
      <p><b>Стандарт в индустрии и академии.</b> CV-скоры — общий язык: на Kaggle, в статьях, в собеседованиях все говорят «у меня 5-fold CV accuracy 0.89». Игнорировать — значит вылететь из диалога.</p>
      <p><b>Оптимальное использование маленьких данных.</b> Классический split «80/20» выкидывает 20% данных из обучения. CV «видит» все данные и в train, и в test — просто в разных фолдах.</p>
      <p><b>Основа honest тюнинга.</b> GridSearchCV автоматически выбирает лучшие гиперпараметры по CV-скору, защищая от переобучения на одном конкретном test-сете.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>K-кратно дольше обучения.</b> 5-fold = модель учится 5 раз. Для XGBoost на 1M строк и ноутбуке это вечер. Для LLM или нейросети на большом датасете — нереально: обходятся одним train/val split и доверяют размеру выборки.</p>
      <p><b>Стандартный K-Fold ломает временные ряды.</b> Если в train попадают будущие точки, а в test — прошлые, модель «подсматривает» и даёт иллюзорно хорошие результаты. В проде — катастрофа. Нужен TimeSeriesSplit или purged/embargoed CV.</p>
      <p><b>Data leakage между фолдами при группах.</b> Один пациент — много записей, один клиент — много транзакций. Если записи одного субъекта в train и test одновременно, модель фактически «видела» ответ. Решение — GroupKFold, но его легко забыть.</p>
      <p><b>Feature engineering до CV — скрытая утечка.</b> StandardScaler или target encoding, посчитанный на всех данных до разбиения, протаскивает информацию из test в train. Фичи и препроцессинг должны жить внутри Pipeline, чтобы пересчитываться на каждом фолде.</p>
      <p><b>Оверселектинг на валидации.</b> Если ты попробовал 10 000 комбинаций гиперпараметров, лучший CV-скор всё равно будет смещён оптимистично — по тому же принципу, что и обычное переобучение. Лекарство — hold-out test set или nested CV.</p>
      <p><b>LOO — почти всегда плохая идея.</b> Leave-One-Out дорог (N моделей) и при этом даёт высокий variance оценки. Обычно 5 или 10 фолдов — лучший компромисс.</p>

      <h3>🧭 Когда брать CV — и когда точно не стоит</h3>
      <table>
        <tr><th>✅ Бери CV когда</th><th>❌ НЕ бери (или бери аккуратно) когда</th></tr>
        <tr>
          <td>Данных мало — сотни-десятки тысяч строк, один split слишком шумный</td>
          <td>Данных миллионы и более — один большой train/val дешевле и достаточно статистически значим</td>
        </tr>
        <tr>
          <td>Нужно сравнить несколько моделей или тюнить гиперпараметры</td>
          <td>Обучение модели стоит часы-дни (LLM, большие CNN) — ограничься одним hold-out</td>
        </tr>
        <tr>
          <td>Нужен OOF-прогноз для stacking / ensembling</td>
          <td>Данные — временной ряд и ты используешь обычный K-Fold — будет leakage, бери TimeSeriesSplit</td>
        </tr>
        <tr>
          <td>Классы несбалансированы — бери StratifiedKFold, он сохраняет пропорции</td>
          <td>В данных есть группы (пациент, клиент) — обычный K-Fold leaks, нужен GroupKFold</td>
        </tr>
        <tr>
          <td>Академический отчёт или прод-решение, где нужна доверительная оценка</td>
          <td>Готовый бенчмарк с фиксированным train/test split (MNIST, ImageNet) — сравнимость с литературой важнее</td>
        </tr>
        <tr>
          <td>Хочешь увидеть variance оценки и стабильность модели</td>
          <td>Фичи или препроцессинг посчитаны до разбиения — leakage заменит «честность» CV на обман</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы — что взять вместо и почему</h3>
      <ul>
        <li><b>Простой hold-out (train/val/test split)</b> — когда данных &gt;500k и обучение дорогое. Одного split достаточно, чтобы оценка была стабильной, а экономия времени колоссальная.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('glossary-bootstrap')">Bootstrap</a> оценка</b> — повторные выборки с возвращением дают доверительные интервалы качества. Альтернатива k-fold, когда хочется именно распределение, а не точку.</li>
        <li><b>TimeSeriesSplit / walk-forward validation</b> — обязательно при любой хронологии: биржи, трафик, спрос, логи пользователей.</li>
        <li><b>Nested CV</b> — если тюнинг гиперпараметров на CV сам по себе может стать источником переобучения (Kaggle, медицинские исследования).</li>
        <li><b>Out-of-time validation</b> — обучиться на старых месяцах, протестировать на новых. Близко к реальному use case и ловит concept drift, который обычный K-Fold не видит.</li>
      </ul>
    `,
