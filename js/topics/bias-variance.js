...
plt.show()</code></pre>
    ...
    applications: `
      <h3>🎯 Где применяется идея на практике</h3>
      <ul>
        <li><b>Диагностика «почему модель плохая».</b> Классический вопрос: train accuracy 95%, test 70% — что делать? Bias-variance даёт прямой ответ: это high variance (переобучение), и лечение — больше данных/регуляризация/проще модель.</li>
        <li><b>Выбор архитектуры.</b> Решение «линейная регрессия или XGBoost» — это выбор точки на bias-variance кривой. Линейная = высокий bias, низкий variance; бустинг = наоборот. Понимание этого экономит часы экспериментов.</li>
        <li><b>Настройка регуляризации.</b> $\\lambda$ в Ridge, <code>max_depth</code> в деревьях, weight decay в нейросетях — все эти ручки явно управляют позицией модели на оси «сложность». Без понимания bias-variance они подбираются вслепую.</li>
        <li><b>Выбор между bagging и boosting.</b> Bagging (Random Forest) снижает variance, оставляя bias — берём когда базовая модель переобучена. Boosting снижает bias — берём когда базовая модель слаба (stumps).</li>
        <li><b>Learning curves для диагностики.</b> График train/val ошибки в зависимости от размера выборки напрямую показывает, в каком режиме модель: если обе ошибки высокие и сходятся — high bias, данных добавлять бесполезно; если большой разрыв — high variance, больше данных поможет.</li>
        <li><b>Early stopping в нейросетях и бустинге.</b> С ростом числа эпох/итераций variance растёт — early stopping останавливает обучение в «золотой» точке, где test ошибка минимальна.</li>
      </ul>

      <h3>✅ Почему эта рамка полезна — и что она даёт на практике</h3>
      <p><b>Универсальный диагностический язык.</b> Любая проблема с обобщением сводится к «high bias / high variance / шум в данных». Это общий словарь для команды: когда MLE говорит «high variance», все понимают, какие ручки крутить.</p>
      <p><b>Прямая связь с ручками модели.</b> Каждый гиперпараметр явно сдвигает точку на bias-variance кривой: <code>max_depth</code>, $\\lambda$, <code>n_estimators</code>, <code>dropout</code>. Понимаешь рамку — тюнишь осознанно, а не перебором.</p>
      <p><b>Объясняет, почему ансамбли работают.</b> Random Forest работает не магией, а снижением variance через усреднение. Gradient Boosting — снижением bias через последовательную коррекцию. Без bias-variance это выглядело бы случайным набором трюков.</p>
      <p><b>Учит смирению с noise floor.</b> Невозможно получить ошибку ниже неустранимого шума в данных. Это спасает от бесконечного тюнинга модели, когда проблема — в качестве лейблов.</p>
      <p><b>Работает как чеклист при дебаге.</b> Есть train и val метрики — построил learning curve, посмотрел разрыв, поставил диагноз, выбрал лечение. Без этой рамки дебаг превращается в случайные эксперименты.</p>

      <h3>⚠️ Ограничения — и когда классика бьёт</h3>
      <p><b>Классическая U-кривая — упрощение.</b> В современном deep learning наблюдается <b>double descent</b>: test error сначала падает, потом растёт (как в классике), потом <b>снова</b> падает при сильной over-parameterization. Трансформеры с миллиардами параметров — «over-fit», но работают идеально.</p>
      <p><b>Строгое разложение — только для MSE.</b> $Err = Bias^2 + Variance + \\sigma^2$ — красивая формула, но она чисто теоретическая и верна только для квадратичной функции потерь. Для cross-entropy, hinge loss, ranking metrics такого разложения нет.</p>
      <p><b>На практике bias и variance не измеряются напрямую.</b> Чтобы оценить variance, нужен ансамбль обучений на разных выборках. Чаще всего используется прокси: разрыв между train и val.</p>
      <p><b>Не работает, если распределение train и test разное.</b> Bias-variance предполагает, что test — это та же генерация данных, что и train. При distribution shift (covariate/concept) рамка перестаёт быть диагностичной — нужны другие инструменты (domain adaptation, invariance).</p>
      <p><b>Ловушка «больше данных всегда помогает».</b> При high bias больше данных не даёт почти ничего — у тебя модель слишком простая для задачи. Без понимания рамки команды тратят месяцы на сбор данных там, где проблема в модели.</p>

      <h3>🧭 Когда использовать bias-variance как рамку — и когда не стоит</h3>
      <table>
        <tr><th>✅ Применяй рамку когда</th><th>❌ Рамка не поможет (или будет обманчивой) когда</th></tr>
        <tr>
          <td>Диагностируешь классическую ML-модель (линрег, дерево, бустинг, небольшая NN)</td>
          <td>Работаешь с over-parameterized LLM / foundation models — там double descent</td>
        </tr>
        <tr>
          <td>Train и test пришли из одного распределения, изменений в данных нет</td>
          <td>Distribution shift между обучением и продом — проблема не в bias/variance, а в самих данных</td>
        </tr>
        <tr>
          <td>Функция потерь — MSE, RMSE, MAE или близкое — разложение математически корректно</td>
          <td>Метрика — NDCG, MAP, AUC, BLEU — прямой декомпозиции нет, рамка только интуитивная</td>
        </tr>
        <tr>
          <td>Нужно объяснить стажёру, почему random forest работает, а одно дерево нет</td>
          <td>Проблема в labeling noise или data leakage — сначала почини данные, потом думай о bias/variance</td>
        </tr>
        <tr>
          <td>Тюнишь гиперпараметры и хочешь понимать, в какую сторону они двигают обобщение</td>
          <td>Модель обучается на streaming data с нестационарностью — нужна online learning рамка, не bias-variance</td>
        </tr>
      </table>

      <h3>🔄 Смежные концепции — что учить дальше</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('regularization')">Регуляризация</a></b> — практический инструмент управления variance. Bias-variance объясняет зачем, регуляризация — как.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('cross-validation')">Кросс-валидация</a></b> — способ честно измерить разрыв train/val и поставить диагноз.</li>
        <li><b>Learning curves</b> — визуальная диагностика bias vs variance через размер выборки. Минимум, который должен знать каждый ML-инженер.</li>
        <li><b>Double descent и benign overfitting</b> — современное расширение классики для over-parameterized моделей (Belkin et al., 2019). Если работаешь с LLM — обязательно читать.</li>
        <li><b>PAC-Bayes и generalization bounds</b> — теоретическая рамка, которая даёт математические гарантии обобщения. Сложнее, но строже bias-variance.</li>
      </ul>
    `,
