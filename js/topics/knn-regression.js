...
App.registerTopic({
  ...
  tabs: {
    theory: `
      ...
      <ul>
        <li>Много признаков — <a class="glossary-link" onclick="App.selectTopic('glossary-curse-dimensionality')">проклятие размерности</a>.</li>
      ...
      },
      {
        ...
        content: `
          ...
          <div class="step" data-step="2">
            ...
            <div class="calc">
              Для x=4 (5 ближайших — x=2,3,4,5,6, т.к. |4−1|=3 и |4−7|=3 — дальние):<br>
              ŷ = (3.8+6.2+5.9+8.3+9.1)/5 = 33.3/5 = <b>6.66</b><br>
              Истинное значение: 5.9. Ошибка: 0.76<br><br>
...
for name, model in models.items():
    ...
    applications: `
      ...
      <ul>
        <li><b>Оценка недвижимости (automated valuation model, AVM).</b> Цену квартиры удобно предсказывать по $k$ самым похожим сделкам в том же районе и с такими же параметрами (площадь, этаж, год постройки). Это буквально то, что делают Zillow, Авито и Циан — kNN в локальной выборке даёт адекватный прогноз без риска экстраполяции за пределы района.</li>
        <li><b>Аналоговый прогноз погоды и климатологии.</b> Находят $k$ дней в истории, когда атмосферные поля были максимально похожи на сегодняшние, и усредняют, что было на следующий день. Метод «аналогов» — классика метеорологии, работает там, где физические модели дают слишком грубый прогноз.</li>
        <li><b>KNN Imputation (заполнение пропусков).</b> Если в строке NaN, находим $k$ самых похожих полных строк и берём среднее/медиану по пропущенному столбцу. Гораздо осмысленнее, чем подставлять среднее по всей колонке — используется в sklearn.impute.KNNImputer и в медицинских/финансовых датасетах.</li>
        <li><b>Soft sensors в промышленности.</b> Предсказание сложно измеримых параметров (концентрация, вязкость) по показаниям простых датчиков (температура, давление). На небольших исторических выборках kNN регрессия даёт быстрый и устойчивый прогноз без обучения сложной модели.</li>
        <li><b>Collaborative filtering для рейтингов.</b> Предсказание оценки пользователя для товара как взвешенное среднее оценок его $k$ ближайших «сосед-пользователей». Базовая идея MovieLens-подобных рекомендателей до эпохи матричного разложения и нейросетей.</li>
        <li><b>Медицинский прогноз по похожим пациентам.</b> «Пациенты с похожими анализами на старте лечения имели такой-то итоговый показатель через 6 месяцев». Форма case-based reasoning, понятная врачам.</li>
        <li><b>Baseline для регрессии с локальной структурой.</b> Если задача имеет локальный характер (цена зависит от «соседей» в пространстве признаков), kNN-регрессия за минуту покажет верхнюю границу того, что даст любая локальная модель.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Ловит произвольную нелинейность без единой настройки.</b> Линейная регрессия увидит только прямую линию, полиномам нужна степень, нейросети — архитектура и часы обучения. kNN просто усредняет соседей — автоматически моделирует любые изгибы, плато, локальные минимумы. При $k=1$ фактически memorizes train.</p>
      <p><b>Ноль фазы обучения.</b> Можно добавлять новые точки прямо в продакшене без переобучения. Для быстро меняющихся рынков (цены на недвижимость, прогнозы продаж) это серьёзное преимущество: сегодняшняя сделка сразу улучшает завтрашний прогноз.</p>
      <p><b>Нет предположений о распределении $y$.</b> Линейная регрессия требует гомоскедастичности, нормальности остатков. Дерево требует определять разбиения. kNN не требует ничего — просто расстояние. Работает и при асимметричных, и при мультимодальных распределениях таргета.</p>
      <p><b>Интерпретация через «вот эти 5 похожих».</b> Вместо feature importance можно показать клиенту реальные 5 квартир, по которым оценена цена его квартиры. Это честнее и убедительнее, чем blackbox-score.</p>
      <p><b>Weighted kNN бесплатно улучшает точность.</b> Веса $1/d_i$ вместо простого среднего — ближние соседи влияют больше дальних. Плавный, устойчивый прогноз без дополнительной модели или тюнинга.</p>
      <p><b>Один главный гиперпараметр — $k$.</b> Cross-validation по одному параметру занимает минуты. Не нужно подбирать learning rate, depth, batch size, architecture — только одно число.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Не экстраполирует — вообще.</b> За пределами обучающего диапазона прогноз равен усреднению ближайших train-точек, то есть эффективно плато. Для временных рядов с трендом (рост пользователей, инфляция) — катастрофа: модель никогда не предскажет значение выше максимального в train.</p>
      <p><b>$O(n)$ на каждый запрос.</b> Линейная регрессия предсказывает за $O(p)$, kNN — за $O(np)$. На миллионе точек real-time невозможен без KD-Tree, Ball Tree или ANN-индекса, и даже эти структуры ломаются в высокой размерности.</p>
      <p><b>Проклятие размерности.</b> При $p &gt; 20$ все точки примерно одинаково далеки, понятие «ближайший» размывается, и прогноз становится почти константой по всему пространству. Методы снижения размерности (PCA, отбор признаков) становятся обязательными.</p>
      <p><b>Чувствителен к масштабу и нерелевантным признакам.</b> Если «площадь в м²» и «этаж» без стандартизации — площадь доминирует, этаж игнорируется. Одна шумовая колонка может испортить расстояния и, соответственно, прогноз. Feature selection обязателен.</p>
      <p><b>Выбросы в train напрямую попадают в прогноз.</b> Если среди $k$ соседей оказался экстремум по цене, он тянет среднее. При $k=1$ прогноз просто копирует ближайшую train-точку со всеми её шумами. Помогают медианный kNN или увеличение $k$, но на малых $k$ модель нестабильна.</p>
      <p><b>Нет формулы, нет коэффициентов.</b> Бизнесу нельзя сказать «рост площади на 1 м² даёт +X₽». Только «у похожих квартир цена была такая». Для регуляторных задач, где нужны коэффициенты и доверительные интервалы, kNN не подходит.</p>

      <h3>🧭 Когда брать kNN-регрессию — и когда точно не стоит</h3>
      <table>
        <tr><th>✅ Бери kNN-регрессию когда</th><th>❌ НЕ бери когда</th></tr>
        <tr>
          <td>Задача локальна: «похожие объекты имеют похожий y» (недвижимость, погода, рейтинги)</td>
          <td>Нужна экстраполяция за пределы train (временные ряды, рост, прогноз в будущее)</td>
        </tr>
        <tr>
          <td>Размерность признаков низкая ($p &lt; 20$) после отбора</td>
          <td>Очень высокая размерность сырых данных без embedding'ов или снижения размерности</td>
        </tr>
        <tr>
          <td>Зависимость сильно нелинейна, и не хочется городить полиномы или нейросеть</td>
          <td>Данных очень много (миллионы), нужен быстрый инференс — kNN не скейлится без ANN-индекса</td>
        </tr>
        <tr>
          <td>Нужен быстрый baseline без обучения — за минуту понять, есть ли сигнал</td>
          <td>Нужна интерпретация через коэффициенты или доверительные интервалы</td>
        </tr>
        <tr>
          <td>Данные быстро меняются: не хочется переучивать модель каждый час</td>
          <td>Среди признаков много шумовых и нет способа их отфильтровать</td>
        </tr>
        <tr>
          <td>Нужно объяснение «вот эти похожие объекты», а не score</td>
          <td>В train есть выбросы, которые нельзя удалить — они напрямую попадут в прогноз</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы — что взять вместо и почему</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('linear-regression')">Линейная регрессия</a></b> — если зависимость близка к линейной и нужна экстраполяция. Быстрый инференс $O(p)$, интерпретируемые коэффициенты, доверительные интервалы.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('random-forest-regression')">Random Forest Regression</a></b> — если признаков много и среди них есть мусорные. RF игнорирует нерелевантные признаки, не требует масштабирования, быстрее на инференсе и обычно точнее на табличных данных.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('gradient-boosting')">Gradient Boosting</a> (XGBoost/LightGBM/CatBoost)</b> — если нужна максимальная точность на табличной регрессии. Почти всегда побеждает kNN на 10–30% RMSE.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('svr')">SVR</a></b> — если данных мало (&lt; 10к), признаков много и нужна устойчивость к выбросам через $\\varepsilon$-трубу. Работает лучше kNN в умеренно высокой размерности.</li>
        <li><b>Локальная регрессия (LOESS/LOWESS)</b> — если нужна идея kNN, но с гладкой локальной кривой вместо ступенчатой. Даёт тот же «локальный» характер, но сглаженный: хорошо для визуализации трендов.</li>
      </ul>
    `,
