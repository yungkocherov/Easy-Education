...
print(classification_report(y_te, svc_best.predict(X_te_s), target_names=data.target_names))</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>Классификация текстов и NLP до эпохи трансформеров.</b> На TF-IDF векторах с линейным ядром SVM долгое время был state-of-the-art в sentiment analysis, spam filtering, категоризации новостей. В высокой разреженной размерности ($d \\gg n$) линейный SVM обучается быстро и обобщается лучше логрега на малых выборках.</li>
        <li><b>Биоинформатика.</b> Классификация белков, предсказание функции генов, выявление онкомаркеров. Здесь типично $n \\approx 100$ пациентов и $p \\approx 10000$ генов — SVM с RBF устойчив именно в таких «широких и коротких» данных, где деревья и нейросети переобучаются.</li>
        <li><b>Распознавание рукописных цифр и OCR (эпоха до CNN).</b> MNIST benchmark долго возглавлялся SVM с polynomial или RBF ядром — около 99% accuracy. Сейчас уступил CNN, но для задач с ограниченными данными до сих пор живой вариант.</li>
        <li><b>Распознавание лиц (эпоха до deep learning).</b> Eigenfaces + SVM был стандартной связкой в security-системах в 2000-х. До сих пор применяется в embedded-устройствах, где нейросеть не влезает в память.</li>
        <li><b>One-Class SVM для anomaly detection.</b> Обучение только на нормальных примерах, граница вокруг «нормы». Используется в мониторинге (выявление атак), контроле качества на производстве, обнаружении сбоев оборудования. Работает там, где аномалии редки и трудно собрать размеченный датасет.</li>
        <li><b>Задачи с заранее определённым ядром.</b> String kernels для биологических последовательностей, graph kernels для молекул, kernels на time series. SVM — единственная популярная модель, где ядро определяет всё, и domain-эксперт может вложить в него знания.</li>
        <li><b>Baseline для small-data классификации.</b> При $n &lt; 10^4$ и нелинейной границе SVM с RBF часто даёт результат лучше, чем Random Forest или нейросеть, и обучается за секунды.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Максимизация margin — принципиальная гарантия обобщения.</b> SVM не просто ищет любую разделяющую гиперплоскость, а ту, что максимально удалена от ближайших точек обоих классов. Это прямо связано с bound'ами PAC-теории и статистической теорией обучения Вапника — в условиях малых выборок это даёт лучший generalization, чем ad-hoc методы.</p>
      <p><b>Ядерный трюк: нелинейность без явного расширения.</b> RBF-ядро эффективно переводит данные в бесконечномерное пространство и строит там линейную границу. В исходном пространстве это произвольно сложная граница. И всё это — без хранения расширенных признаков и без явного feature engineering.</p>
      <p><b>Отлично работает в высокой размерности.</b> Когда $p &gt; n$ (тексты, геномика, fMRI), большинство моделей теряются или переобучаются. SVM с линейным ядром и L2-регуляризацией — одна из немногих, что справляется: границу определяет margin, а не число признаков.</p>
      <p><b>Memory-efficient через опорные векторы.</b> После обучения хранятся только support vectors — обычно 5–30% выборки. Остальные точки не влияют на предсказание и не нужны. Экономия памяти в продакшене по сравнению с kNN, где хранится весь датасет.</p>
      <p><b>Устойчив к небольшому шуму через soft margin.</b> Параметр $C$ регулирует баланс между широтой margin и штрафом за нарушения. При правильном $C$ модель игнорирует выбросы и шумные точки, концентрируясь на реально разделяющей границе.</p>
      <p><b>Гибкость через выбор ядра.</b> Linear, polynomial, RBF, sigmoid, string, graph, custom kernel — все они работают через один и тот же Solver. Можно встроить domain knowledge напрямую в ядро, например, через meaningful similarity measure для биологических последовательностей.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Кубическая сложность по данным.</b> Обучение — $O(n^2)$ до $O(n^3)$ по памяти и времени. На $10^5$ точках — уже часы, на $10^6$ — практически невозможно без линейных аппроксимаций. В продакшене на больших данных SVM вытеснен градиентным бустингом именно по этой причине.</p>
      <p><b>Не выдаёт вероятности нативно.</b> Выход — signed distance до гиперплоскости, не $P(y=1|x)$. Для бизнес-порогов и ожидаемой ценности нужно прикручивать Platt scaling (обучать логрег поверх score) или isotonic regression. Это ещё один этап, который легко забыть и получить нерабочий threshold.</p>
      <p><b>Тяжёлый тюнинг.</b> Даже при фиксированном ядре RBF нужно подбирать $C$ и $\\gamma$ — обычно GridSearch по сетке $10^{-3}$ до $10^3$ с CV. Каждая точка сетки — полное обучение, которое на умеренных данных уже дорогое. Плохой $\\gamma$ даёт катастрофический overfit или underfit.</p>
      <p><b>Обязательное масштабирование.</b> RBF-ядро считает евклидово расстояние — без стандартизации один признак доминирует. Забыл StandardScaler — получил мусор. Не ошибка алгоритма, а подводный камень, на который регулярно спотыкаются новички.</p>
      <p><b>Multi-class — неестественно.</b> SVM — бинарный по природе. Multi-class реализуется через OvO ($K(K-1)/2$ моделей) или OvR ($K$ моделей), и это либо дорого, либо несбалансированно. На задачах с 100+ классов (классификация продуктов, рубрикаторов) уже не вариант.</p>
      <p><b>Чёрный ящик при нелинейных ядрах.</b> Линейный SVM ещё можно интерпретировать через веса. RBF — нет: граница решения в исходном пространстве произвольно сложная, feature importance не определена. Для регулируемых отраслей (банки, медицина) это минус.</p>

      <h3>🧭 Когда брать SVM — и когда точно не стоит</h3>
      <table>
        <tr><th>✅ Бери SVM когда</th><th>❌ НЕ бери когда</th></tr>
        <tr>
          <td>Данных мало ($n &lt; 10^4$) и нелинейная граница — RBF SVM часто точнее RF и нейросети</td>
          <td>Данных много ($n &gt; 10^5$) — обучение займёт часы или не завершится из-за $O(n^2)$ памяти</td>
        </tr>
        <tr>
          <td>Высокая размерность и разреженные данные (тексты, геномы) — линейный SVM блистает</td>
          <td>Нужны откалиброванные вероятности — SVM их не даёт без Platt scaling</td>
        </tr>
        <tr>
          <td>Есть domain knowledge, которое можно вложить в кастомное ядро (sequences, graphs)</td>
          <td>Много классов ($K &gt; 10$) — OvO/OvR неэффективны</td>
        </tr>
        <tr>
          <td>Нужна хорошая граница с минимальной возможностью переобучения на малых выборках</td>
          <td>Смесь числовых и категориальных признаков с high cardinality — деревья/бустинг обработают лучше</td>
        </tr>
        <tr>
          <td>Задача anomaly detection без разметки негативов — One-Class SVM</td>
          <td>Нужна интерпретируемость — RBF SVM совершенно непрозрачен</td>
        </tr>
        <tr>
          <td>Допустимо потратить время на тюнинг $C$ и $\\gamma$ через cross-validation</td>
          <td>Нужен real-time инференс на миллионах запросов — линейные модели/деревья быстрее</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы — что взять вместо и почему</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('logistic-regression')">Логистическая регрессия</a></b> — если граница линейна или данные высокоразмерные (тексты). Даёт откалиброванные вероятности, масштабируется до миллиардов признаков через SGD, проще интерпретировать.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('gradient-boosting')">Gradient Boosting</a> (XGBoost/LightGBM/CatBoost)</b> — если данные табличные и их много. Обрабатывает категории, обычно точнее SVM на tabular ML, работает на миллионах строк. Современный дефолт вместо SVM на Kaggle.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('random-forest')">Random Forest</a></b> — если нужна нелинейность, но тюнинг $C, \\gamma$ слишком дорогой. Работает почти «из коробки» с разумными дефолтами, даёт feature importance.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('neural-network')">Нейросеть</a> (MLP/CNN/Transformer)</b> — если данных много и задача на сырых изображениях/тексте/звуке. В эпоху embedding'ов SVM на сырых пикселях уже не конкурент, но на выученных фичах всё ещё может работать лучше на малых выборках.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('knn')">kNN</a></b> — если хочется lazy learning и «похожие примеры» как форму объяснения. Проще, но медленнее на инференсе и сильно страдает от размерности.</li>
      </ul>
    `,
