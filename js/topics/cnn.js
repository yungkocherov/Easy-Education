...
# Типичный результат: test accuracy ~99% за 10 эпох
    ...
    applications: `
      ...
      <ul>
        <li><b>Классификация изображений и ImageNet-задачи.</b> ResNet, EfficientNet, ConvNeXt — до сих пор работают в продакшене e-commerce (категоризация товаров), модерации контента, fashion-тэггинга. Там, где картинок миллионы, а latency критичен, CNN часто выгоднее Vision Transformer.</li>
        <li><b>Детекция объектов.</b> YOLO (v5–v8), Faster R-CNN, SSD — основа автономного вождения (Tesla Autopilot, Waymo), видеонаблюдения, анализа спутниковых снимков. Модель находит bounding boxes и классифицирует содержимое в реальном времени.</li>
        <li><b>Медицинская диагностика.</b> Рентген (туберкулёз, пневмония), МРТ/КТ (опухоли, инсульты), гистология (раковые клетки), дерматология (меланома). U-Net для сегментации органов — де-факто стандарт в медицинском imaging.</li>
        <li><b>Распознавание лиц и биометрия.</b> FaceNet, ArcFace, InsightFace. Разблокировка телефонов, системы контроля доступа, поиск по лицу в соцсетях и среди пропавших без вести.</li>
        <li><b>OCR и распознавание документов.</b> Сканирование чеков, паспортов, водительских прав, распознавание рукописного текста. CNN в связке с CTC или attention-декодером — основа Google Lens, Tesseract 4+, Yandex Cloud OCR.</li>
        <li><b>Промышленный контроль качества.</b> Поиск дефектов на конвейере — царапины, сколы, неправильная сборка. Предобученные CNN fine-tune на сотнях примеров брака, заменяя команды ручных контролёров.</li>
        <li><b>Генерация изображений (внутри diffusion).</b> Stable Diffusion построен вокруг U-Net — свёрточного бэкбона, который предсказывает шум на каждом шаге denoising. Без CNN нет современной генерации.</li>
        <li><b>Аудио через спектрограммы.</b> Классификация звуков (хлопки, лай, аварии), anti-spoofing в речи, музыкальный тэггинг. Спектрограмма — это «картинка», и CNN обрабатывает её так же, как фотографию.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Translation invariance.</b> Один и тот же фильтр применяется ко всем позициям изображения, поэтому «кот в левом верхнем углу» и «кот в правом нижнем углу» активируют одни и те же нейроны. Это свойство подарено архитектурой — без него классификация требовала бы показать сети каждый объект в каждой возможной позиции.</p>
      <p><b>Weight sharing — огромная экономия параметров.</b> Полносвязный слой для 224×224 RGB нуждался бы в 150 000 весов на один нейрон. CNN использует тот же фильтр 3×3 (27 весов) по всей картинке. ResNet-50 — это 25M параметров на всю модель, а не на один слой. Меньше параметров = меньше переобучения.</p>
      <p><b>Иерархия признаков.</b> Первые слои учат края и текстуры, средние — формы и детали (глаза, колёса), глубокие — объекты целиком. Это не задаётся руками, а эмерджентно возникает из задачи — и именно это сделало feature-инженеров в CV ненужными.</p>
      <p><b>Локальная связность.</b> Свёртка смотрит только на соседние пиксели, что совпадает со статистикой изображений: важные корреляции — локальные (край, пятно), а глобальные строятся из локальных. Это inductive bias, который помогает учиться на меньших данных.</p>
      <p><b>Иерархия масштабов через pooling и stride.</b> Каждый следующий слой «видит» всё больший рецептивный участок входа. К глубоким слоям нейроны смотрят на весь объект целиком, хотя каждый фильтр по-прежнему локальный.</p>
      <p><b>Отличный transfer learning.</b> ImageNet-предобученный ResNet/EfficientNet переносится практически на любую image-задачу с сотнями примеров. Это радикально снижает порог входа в CV и делает CNN применимыми там, где нейросети с нуля были бы невозможны.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Требуют много данных.</b> Обучение CNN с нуля на 500 картинках — верная дорога к переобучению. Без <a class="glossary-link" onclick="App.selectTopic('transfer-learning')">transfer learning</a> и аугментации серьёзные модели требуют десятки тысяч размеченных примеров, а state-of-the-art — миллионы.</p>
      <p><b>Не rotation- и scale-invariant из коробки.</b> Если на обучении все коты смотрят влево, а на тесте — вправо, сеть может не узнать объект. Требуется augmentation (повороты, масштабирование, отражения), иначе инвариантности не возникает — архитектура ею не наделена.</p>
      <p><b>Проигрывают Vision Transformer на очень больших датасетах.</b> На JFT-300M и выше ViT и Swin-Transformer обгоняют CNN, потому что их inductive bias слабее и они лучше масштабируются с данными. На ImageNet-1k CNN ещё конкурентны, дальше — нет.</p>
      <p><b>Плохая интерпретируемость.</b> Grad-CAM, saliency maps, feature visualization дают интуицию, но не полноценное объяснение. Для регуляторов в медицине и финансах это проблема — «модель увидела что-то на снимке» редко принимается как обоснование диагноза.</p>
      <p><b>Дорогая инференс-стоимость.</b> Большие CNN (ResNet-152, EfficientNet-B7) требуют GPU и десятки миллисекунд на картинку. Для real-time видео нужна либо квантизация, либо специализированные мобильные архитектуры (MobileNet, ShuffleNet), либо ускорители (Jetson, Coral).</p>
      <p><b>Fixed input size.</b> Классическая CNN с FC-слоями в конце принимает строго заданный размер картинки. Глобальный average pooling решает проблему, но это надо закладывать в архитектуру с самого начала.</p>

      <h3>🧭 Когда брать CNN — и когда точно не стоит</h3>
      <table>
        <tr><th>✅ Бери CNN когда</th><th>❌ НЕ бери когда</th></tr>
        <tr>
          <td>Задача — классификация, детекция или сегментация изображений</td>
          <td>Данные табличные — возьми Gradient Boosting, CNN там бесполезна</td>
        </tr>
        <tr>
          <td>Есть предобученный ImageNet-чекпоинт для fine-tuning (1000+ размеченных примеров)</td>
          <td>У тебя меньше сотни картинок и нет возможности transfer learning</td>
        </tr>
        <tr>
          <td>Нужен real-time inference на GPU или мобильном устройстве (MobileNet/YOLO)</td>
          <td>Есть доступ к гигантским датасетам (JFT-300M+) — ViT/Swin обгонят</td>
        </tr>
        <tr>
          <td>Аудио-задача через спектрограмму — классификация звуков, anti-spoofing</td>
          <td>Нужны long-range зависимости через весь вход — Transformer справится лучше</td>
        </tr>
        <tr>
          <td>Медицинский imaging с небольшим датасетом и требованием segmentation (U-Net)</td>
          <td>Последовательности слов или временные ряды — RNN/Transformer нативнее</td>
        </tr>
        <tr>
          <td>Промышленный контроль качества, OCR, распознавание лиц — классические CV-задачи</td>
          <td>Нужна строгая интерпретируемость для регулятора — CNN слишком непрозрачна</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы — что взять вместо и почему</h3>
      <ul>
        <li><b>Vision Transformer (ViT, Swin, DeiT)</b> — если есть огромный датасет или готовый предобученный чекпоинт. Лучше масштабируется с данными, гибче по задачам.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('transfer-learning')">Transfer learning</a> с ImageNet-моделью</b> — если данных мало. Практически всегда лучше, чем обучать CNN с нуля.</li>
        <li><b>MobileNet / EfficientNet-Lite</b> — если inference на мобильном устройстве или edge: специально спроектированы под малое число FLOPs.</li>
        <li><b>U-Net / DeepLab</b> — если нужна именно сегментация, а не классификация: архитектуры с encoder-decoder и skip-connections.</li>
        <li><b>Классический CV (SIFT, HOG, OpenCV)</b> — если задача простая (подсчёт объектов, определение цвета), данных 10 штук, а запускать всё надо на Raspberry Pi без GPU.</li>
      </ul>
    `,
