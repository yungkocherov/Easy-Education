...
App.registerTopic({
  ...
  tabs: {
    theory: `
      ...
      <p>Это классический <span class="term" data-tip="Overfitting. Переобучение. Модель учит не общий закон, а шум обучающих данных. Плохо работает на новых примерах."><a class="glossary-link" onclick="App.selectTopic('glossary-overfitting')">overfitting</a></span>. Регуляризация — один из главных способов с ним бороться.</p>

      <h3>🧮 Базовая идея</h3>
      <p>К обычной <a class="glossary-link" onclick="App.selectTopic('glossary-loss-functions')">функции потерь</a> добавляем <b>штраф за сложность</b> модели:</p>
      <div class="math-block">$$L_{\\text{reg}}(\\mathbf{w}) = L_{\\text{data}}(\\mathbf{w}) + \\lambda \\cdot R(\\mathbf{w})$$</div>

      <ul>
        <li>$L_{\\text{data}}$ — обычная функция потерь (<a class="glossary-link" onclick="App.selectTopic('glossary-loss-functions')">MSE</a>, log-loss и т.д.).</li>
      ...
      <div class="key-concept">
        ...
        <p>Регуляризация — это <b>управление сложностью модели</b> через гиперпараметр $\\lambda$. Маленькое $\\lambda$ → модель гибкая (risk overfitting). Большое $\\lambda$ → модель жёсткая (risk <a class="glossary-link" onclick="App.selectTopic('glossary-overfitting')">underfitting</a>). Оптимальное $\\lambda$ подбирается через кросс-валидацию.</p>
      </div>
      ...
      <div class="key-concept">
        <div class="kc-label">Ещё проще: субградиент L1</div>
        <p>У L2-штрафа ($w^2$) <a class="glossary-link" onclick="App.selectTopic('glossary-gradient')">градиент</a> = $2w$ → чем меньше $w$, тем слабее «давление» к нулю. У нуля давление = 0 — вес никогда не станет ровно нулём.</p>
        <p>У L1-штрафа ($|w|$) градиент = $\\pm 1$ <b>всегда</b>, независимо от величины $w$. Даже когда вес маленький ($w = 0.001$), штраф «давит» с силой 1. Если полезность признака < λ, штраф побеждает и вес схлопывается до <b>ровно нуля</b>.</p>
        <p>Числовой пример: $w = 0.01$. L2-штраф = $0.01^2 = 0.0001$. L1-штраф = $|0.01| = 0.01$ — <b>в 100 раз больше</b>. При малых весах L1 штрафует несоразмерно сильно → проще занулить, чем держать.</p>
      </div>
      ...
      <div class="deep-dive">
        ...
        <div class="deep-dive-body">
          ...
          <p>Эффект похож на L2: предотвращает ко-адаптацию нейронов и <a class="glossary-link" onclick="App.selectTopic('glossary-overfitting')">переобучение</a>. На inference dropout не применяется.</p>
        </div>
      </div>
...
plt.show()</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>Линейные модели с большим числом признаков.</b> Когда фичей сотни, а строк тысячи — без регуляризации линейная/логистическая регрессия переобучится мгновенно. Ridge/Lasso спасают, превращая плохо обусловленную задачу в корректную.</li>
        <li><b>Feature selection через L1.</b> Lasso зануляет неважные коэффициенты — получаешь интерпретируемый список «что реально влияет». Особенно полезно в биоинформатике, где фичей десятки тысяч, а образцов сотни.</li>
        <li><b>Борьба с мультиколлинеарностью в эконометрике.</b> Если фичи сильно коррелируют (цена и площадь, возраст и стаж), OLS даёт нестабильные коэффициенты. Ridge стабилизирует их за счёт небольшого смещения.</li>
        <li><b>SVM и параметр C.</b> В SVM регуляризация спрятана под именем $C$: маленький $C$ — сильная регуляризация, широкий отступ. Без этой ручки SVM переобучался бы под каждый шумный пример.</li>
        <li><b>Нейросети: weight decay, dropout, label smoothing.</b> В DL регуляризация — не опция, а обязательная часть рецепта. Weight decay (L2 по весам), dropout, BatchNorm, early stopping, data augmentation — все они разновидности одной идеи.</li>
        <li><b>Градиентный бустинг.</b> XGBoost и LightGBM имеют явные L1/L2 штрафы на значения листьев (<code>reg_alpha</code>, <code>reg_lambda</code>) + структурные ограничения (min_child_weight, max_depth). Без них бустинг переобучается моментально.</li>
        <li><b>Baseline для продакшен-моделей.</b> Elastic Net — стандартный выбор в финансовом скоринге, маркетинговой аналитике, A/B-платформах: простая, интерпретируемая, регулируемая.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Прямой контроль над overfitting.</b> У тебя есть одна ручка $\\lambda$, которой ты плавно переходишь от «полностью доверяю данным» (OLS) к «предпочитаю простые модели» (сильный штраф). Это самый простой и предсказуемый способ бороться с переобучением.</p>
      <p><b>Улучшает обобщение даже на чистых данных.</b> Без регуляризации модель подгоняется под шум. С L2 она чуть смещена, но её variance ниже — и на test-сете получается ниже средняя ошибка. Это прямое следствие bias-variance trade-off.</p>
      <p><b>L1 даёт «бесплатный» feature selection.</b> Lasso сам выкидывает ненужные фичи, зануляя коэффициенты. В задачах с тысячами признаков это экономит часы ручной работы и делает модель интерпретируемой.</p>
      <p><b>L2 стабилизирует плохо обусловленные задачи.</b> Когда матрица $X^T X$ почти вырождена (коллинеарность, $n &lt; p$), OLS не существует или даёт гигантские коэффициенты. Ridge добавляет $\\lambda I$ к диагонали — и задача становится корректной.</p>
      <p><b>Elastic Net = лучшее из обоих миров.</b> L1 для отбора + L2 для стабильности. Стандартный выбор для боевых линейных моделей, особенно при коррелированных признаках, где чистый Lasso нестабилен.</p>
      <p><b>Снижает чувствительность к шуму в лейблах.</b> Если таргет зашумлён, сильная регуляризация не даёт модели «гоняться» за выбросами — получаешь более устойчивые, менее шумные предсказания.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Нужна стандартизация признаков.</b> Регуляризация штрафует все веса одинаково, но у «цены в рублях» и «возраста в годах» разный масштаб. Без StandardScaler ridge/lasso дадут мусор. Это одна из самых частых ошибок новичков.</p>
      <p><b>Тюнинг $\\lambda$ обязателен.</b> Нет «универсального» значения — только кросс-валидация. LassoCV/RidgeCV встроены в sklearn, но люди всё равно забывают, оставляя дефолт.</p>
      <p><b>Слишком сильная регуляризация → underfit.</b> Большой $\\lambda$ делает коэффициенты микроскопическими, и модель начинает предсказывать почти среднее по выборке. Это тихая форма недообучения — метрики падают, причина неочевидна.</p>
      <p><b>L1 нестабилен при коррелированных признаках.</b> Если две фичи почти идентичны, Lasso случайно выкинет одну из них и оставит другую — при перезапуске может выбрать наоборот. Для интерпретации это катастрофа. Elastic Net частично лечит.</p>
      <p><b>Плохо работает с разреженными дискретными таргетами.</b> В логистической регрессии сильная регуляризация может сильно искажать откалиброванные вероятности, особенно если классы дисбалансированы.</p>
      <p><b>Не решает проблемы с фичами.</b> Регуляризация не заменит нормальный feature engineering, очистку данных и борьбу с утечками. Если данные плохие — штраф лишь слегка маскирует это.</p>

      <h3>🧭 Когда брать какую регуляризацию — и когда вообще не стоит</h3>
      <table>
        <tr><th>✅ Бери когда</th><th>❌ НЕ бери (или выбери другую) когда</th></tr>
        <tr>
          <td><b>Ridge (L2)</b> — признаки коррелированы, нужна стабильность, интерпретация через коэффициенты не нужна</td>
          <td>Нужен sparse вектор коэффициентов для отбора фичей — L2 всё оставляет ненулевым</td>
        </tr>
        <tr>
          <td><b>Lasso (L1)</b> — много шумных признаков, нужен автоматический отбор, данные не сильно коррелированы</td>
          <td>Признаки сильно коррелируют — Lasso случайно выкинет один и оставит дубликат, решение нестабильно</td>
        </tr>
        <tr>
          <td><b>Elastic Net</b> — реальные табличные данные с коррелированными фичами, хочется отбора + стабильности</td>
          <td>Очень маленький датасет и важны все фичи — лучше простой Ridge</td>
        </tr>
        <tr>
          <td><b>Weight decay в нейросетях</b> — всегда, как дефолт: современные оптимизаторы (AdamW) берут его из коробки</td>
          <td>Нейросеть недообучена и train loss высокий — регуляризация лишь усугубит</td>
        </tr>
        <tr>
          <td><b>Dropout</b> — полносвязные слои нейросети, защита от co-adaptation</td>
          <td>BatchNorm + dropout вместе в современных CNN — часто конфликтуют, сейчас редко сочетают</td>
        </tr>
        <tr>
          <td><b>Early stopping</b> — обучение нейросети/бустинга, когда есть validation set</td>
          <td>Нет честной валидации — early stopping по train loss бесполезен</td>
        </tr>
        <tr>
          <td><b>Data augmentation</b> — картинки, текст, звук, любые задачи DL с маленьким датасетом</td>
          <td>Задача с жёстко фиксированными фичами (финансовые таблицы) — аугментировать нечего</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы — что взять вместо и почему</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('cross-validation')">Кросс-валидация</a> + раннее прекращение.</b> Если не хочешь тюнить $\\lambda$, можно ограничить обучение по validation loss — это тоже форма регуляризации, особенно в бустинге и нейросетях.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('random-forest')">Random Forest</a> и бэггинг.</b> Усреднение по независимым моделям — альтернативный способ снизить variance без явного штрафа за большие веса.</li>
        <li><b>Больше данных.</b> Самое мощное «регуляризующее средство»: модель, обученная на 10× данных, почти не переобучается. Если есть возможность собрать больше — это дешевле, чем тюнить $\\lambda$.</li>
        <li><b>Bayesian methods с приорами.</b> Гауссовский приор на веса — это ровно L2-регуляризация. Лапласовский приор — ровно L1. Байесовский подход даёт более общую и интерпретируемую рамку.</li>
        <li><b>Feature selection вручную или через mutual information.</b> Если понимаешь данные, убрать мусор руками часто эффективнее, чем полагаться на L1.</li>
      </ul>
    `,
