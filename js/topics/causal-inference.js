...
App.registerTopic({
  ...
  tabs: {
    theory: `
      ...
      <h3>Метод 2: Propensity Score Matching (PSM)</h3>
    ...
    examples: [
      {
        ...
        content: `
          ...
          <div class="step" data-step="3">
            ...
            <div class="why">Регрессионная форма удобна: можно добавить ковариаты (возраст, доход) для повышения точности, а также получить <a class="glossary-link" onclick="App.selectTopic('glossary-standard-error')">стандартные ошибки</a> и p-value из коробки.</div>
          </div>
...
print(refute)  # Если ATE не изменился при добавлении случайного конфаундера — хороший знак</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>Оценка эффекта фичи, когда A/B-тест невозможен.</b> Раскатили на всех сразу, или политически нельзя «отказать» половине пользователей в новой функции, или эффект проявляется слишком медленно. Остаётся только наблюдательная causal-оценка: DID по когортам, synthetic control, propensity score.</li>
        <li><b>Оценка экономических политик.</b> Minimum wage → занятость, ввод налога → потребление, новый закон → ДТП. Classic DID (Card & Krueger), regression discontinuity на порогах, IV через географию. RCT тут просто невозможен — нельзя рандомизированно ввести закон в половине страны.</li>
        <li><b>Маркетинг и incremental lift.</b> Реальный вклад рекламного канала = сколько пользователей НЕ пришли бы без рекламы. Uplift-модели и geo-experiments (ghost ads, PSA holdout) отвечают на «кого имеет смысл таргетить», а не просто «кто вероятно купит».</li>
        <li><b>Медицина и наблюдательные исследования.</b> RCT дорог, долог, часто этически невозможен (нельзя рандомизированно назначить курение). Propensity score matching, inverse probability weighting позволяют получить каузальные оценки из регистров и EHR.</li>
        <li><b>HR-аналитика и образование.</b> Эффект программы обучения, бонусной схемы, новой учебной программы. В программе участвуют мотивированные (self-selection) — нужен PSM или DID, иначе завысишь эффект в разы.</li>
        <li><b>Debugging корреляционных моделей.</b> ML-модель выучила, что «пользователи, которым показали баннер, покупают чаще». Но им показали баннер, потому что они уже собирались купить. Causal DAG помогает увидеть этот reverse causality и не задеплоить модель, которая «работает» на train, но рушит бизнес-метрики в проде.</li>
        <li><b>Policy evaluation в правительстве и NGO.</b> Эффект реформы здравоохранения, школьной программы, субсидий — всё через RDD, DID, synthetic control. World Bank, IMF, CBO используют эти методы стандартно.</li>
      </ul>

      <h3>Сильные стороны — и почему они важны</h3>
      <p><b>Отвечает на «что будет, если» вместо «что скоррелировано».</b> Корреляционный ML говорит «пользователи с промокодом покупают на 30% чаще». Causal inference отвечает на реальный бизнес-вопрос: «сколько мы заработаем, если раздадим промокод всем». Разница — порядок величины в ROI маркетинга.</p>
      <p><b>Работает там, где RCT невозможен.</b> Есть целые классы задач, где рандомизация недоступна: политические решения, медицина с редкими заболеваниями, макроэкономика, ретроспективный анализ исторических данных. Causal inference — единственный способ получить количественную оценку эффекта в этих условиях.</p>
      <p><b>DID и RDD почти не требуют допущений при хорошем дизайне.</b> RDD при достаточно плотных данных вблизи порога даёт квази-экспериментальную оценку — настолько чистую, что используется как бенчмарк для RCT. DID требует только параллельных трендов до воздействия — это допущение тестируется визуально и статистически.</p>
      <p><b>DAG формализует размышления о конфаундерах.</b> Вместо «давай на всякий случай включим все признаки в регрессию» (классическая ошибка, приводящая к контролю коллайдера и post-treatment bias) DAG показывает, что контролировать, а что — категорически нельзя. Это <a class="glossary-link" onclick="App.selectTopic('glossary-dag')">отдельный мыслительный инструмент</a>, который полезен даже без формальных расчётов.</p>
      <p><b>Sensitivity analysis даёт честную оценку неопределённости.</b> Ты можешь сказать: «эффект +5%, и чтобы его обнулил ненаблюдаемый конфаундер, он должен быть сильнее курения для рака лёгких». Это переводит критику «ты не учёл X» из качественной в количественную.</p>
      <p><b>DoWhy, EconML, CausalML делают анализ воспроизводимым.</b> Раньше causal inference был искусством эконометриста. Сейчас — pipeline: указываешь DAG, метод, treatment/outcome — получаешь оценку + refutation tests. Можно положить в git и перезапустить.</p>

      <h3>Ограничения — и когда они реально бьют</h3>
      <p><b>Все методы требуют непроверяемых допущений.</b> Параллельные тренды в DID, exclusion restriction в IV, no unmeasured confounders в PSM. Эти допущения нельзя доказать — только сделать правдоподобными. Любая публикация в топ-журнале содержит секцию «robustness checks», и всё равно её можно разгромить на семинаре.</p>
      <p><b>PSM не устраняет смещение от ненаблюдаемых конфаундеров.</b> Если на treatment влияет мотивация (которую ты не измерил), PSM даст смещённую оценку, как и обычная регрессия. Matching создаёт иллюзию контроля, но контролирует только то, что у тебя есть в данных.</p>
      <p><b>IV почти невозможно найти в реальности.</b> Настоящий инструмент должен влиять на outcome ТОЛЬКО через treatment и быть «случайным». В учебниках — лотерея призыва во Вьетнам, расстояние до колледжа. В твоём SaaS-стартапе — удачи. Слабые инструменты (weak IV) дают смещённые оценки хуже, чем OLS.</p>
      <p><b>RDD требует много данных вблизи порога.</b> Эффект оценивается локально — там, где $X$ близко к cutoff. Если у тебя 10k пользователей, но только 50 возле порога, доверительный интервал будет безнадёжно широк. Экстраполяция за пределы окрестности порога недопустима.</p>
      <p><b>Нужна предметная экспертиза — автоматизировать нельзя.</b> DAG нельзя «выучить из данных» в общем случае (без дополнительных допущений). Выбор метода, определение treatment/outcome, defense of assumptions — всё требует понимания, как устроен процесс. Junior-аналитик с DoWhy сделает хуже, чем senior с OLS и головой.</p>
      <p><b>Всегда уязвим к критике «ты не учёл X».</b> В отличие от A/B-теста, где рандомизация закрывает эту линию атаки, causal inference на наблюдательных данных всегда можно оспорить. Это не баг, а фундаментальное свойство — будь готов к дискуссии на ревью.</p>

      <h3>Когда применять — и когда не стоит</h3>
      <table>
        <tr><th>Бери causal inference когда</th><th>НЕ бери когда</th></tr>
        <tr>
          <td>A/B-тест невозможен: раскатка на всех, политические/этические ограничения, слишком медленный эффект</td>
          <td>Можешь провести <a class="glossary-link" onclick="App.selectTopic('ab-testing-intro')">A/B-тест</a> — он всегда чище и требует меньше допущений</td>
        </tr>
        <tr>
          <td>Нужно ответить на «что будет, если…» — оценить контрфактуальный сценарий</td>
          <td>Нужен прогноз «что произойдёт в следующем месяце» — возьми корреляционный ML, это другая задача</td>
        </tr>
        <tr>
          <td>Есть качественный дизайн: параллельные тренды (DID), чёткий порог (RDD), честный инструмент (IV)</td>
          <td>Слабый дизайн: сомнительные тренды, размытый порог, weak IV — оценка будет хуже честного «мы не знаем»</td>
        </tr>
        <tr>
          <td>ML-модель дала подозрительную корреляцию, и ты хочешь проверить, не reverse causality ли это</td>
          <td>Нет данных о ключевых конфаундерах, и нет правдоподобного дизайна, чтобы их обойти</td>
        </tr>
        <tr>
          <td>Оцениваешь policy, закон, программу — классический кейс для DID/RDD/synthetic control</td>
          <td>Задача просто predictive: ранжирование, классификация, регрессия без вопроса «почему»</td>
        </tr>
        <tr>
          <td>Нужна количественная оценка эффекта с доверительным интервалом и refutation tests</td>
          <td>Нужен немедленный ответ, и нет ни эксперта по методам, ни времени на validation</td>
        </tr>
      </table>

      <h3>Альтернативы</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('ab-testing-intro')">A/B-тестирование</a></b> — золотой стандарт причинного вывода через рандомизацию. Всегда предпочтительнее, когда возможен. Causal inference — это «что делать, когда A/B-тест невозможен».</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('hypothesis-testing')">Проверка гипотез</a> + регрессия с контролями</b> — если предметная область подсказывает, какие конфаундеры важны, и ты готов принять их полноту на веру. Быстрее causal, но не даёт формальной гарантии.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('glossary-dag')">DAG</a>-моделирование без численной оценки</b> — просто нарисовать причинный граф и обсудить его с командой. Часто этого достаточно, чтобы увидеть ошибку в дизайне измерения или фичи.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('bayesian')">Байесовские модели</a> с информативными приорами</b> — когда данных мало, а экспертиза есть. Приоры кодируют знания о механизме, posterior даёт оценку с неопределённостью.</li>
        <li><b>Uplift-моделирование (causal ML)</b> — если задача не «средний эффект», а «кому применить treatment». Причинная версия ML: обучает модель различать treatment effect по сегментам, а не просто выучивать корреляции.</li>
      </ul>
    `,
