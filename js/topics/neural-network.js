...
plt.legend(); plt.show()
    ...
    applications: `
      ...
      <ul>
        <li><b>Компьютерное зрение.</b> Классификация изображений (ResNet, EfficientNet), детекция объектов (YOLO, Faster R-CNN), сегментация (U-Net), распознавание лиц. Везде, где нужно извлекать иерархию визуальных признаков — от краёв до объектов целиком.</li>
        <li><b>NLP и большие языковые модели.</b> GPT, Claude, LLaMA — это стек Transformer-блоков, т.е. специализированные нейросети с attention. Машинный перевод, суммаризация, чат-боты, code generation — всё построено на нейросетях.</li>
        <li><b>Распознавание и синтез речи.</b> Whisper, Tacotron, VALL-E. Преобразование звука в текст и обратно — одна из первых задач, где нейросети уверенно обогнали классические HMM-модели.</li>
        <li><b>Рекомендательные системы.</b> Neural collaborative filtering, two-tower модели, deep &amp; cross networks. Yandex, TikTok, YouTube — все крупные продуктовые рекомендеры давно на нейросетях с <a class="glossary-link" onclick="App.selectTopic('glossary-embedding')">эмбеддингами</a>.</li>
        <li><b>Reinforcement Learning и робототехника.</b> DQN (игры Atari), AlphaGo/AlphaZero, PPO в Dota 2 и StarCraft II. Нейросеть аппроксимирует функцию ценности или политику агента.</li>
        <li><b>Генеративный AI.</b> GAN, VAE, Diffusion (Stable Diffusion, Midjourney), Sora. Всё, что создаёт изображения, музыку и видео по тексту — это нейросети.</li>
        <li><b>Научные приложения.</b> AlphaFold 2 (структура белков), прогноз погоды (GraphCast), ускорители физических симуляций. Там, где аналитических решений нет или они слишком медленные.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Универсальный аппроксиматор.</b> Теорема Цыбенко/Хорника: MLP с одним скрытым слоем и достаточным числом нейронов может приблизить любую непрерывную функцию. На практике это значит — если сигнал в данных есть, правильно сконструированная сеть его найдёт.</p>
      <p><b>Автоматическое обучение признакам (representation learning).</b> В отличие от классических моделей, где признаки надо придумывать руками (TF-IDF, HOG, MFCC), нейросеть сама учит иерархию представлений: от краёв к текстурам, от текстур к объектам. Это убрало целую профессию feature-инженеров в CV и NLP.</p>
      <p><b>Масштабируемость с данными и вычислениями.</b> Классические модели быстро упираются в плато: больше данных — та же точность. Нейросети продолжают улучшаться на миллиардах примеров — это и есть основа «scaling laws» современных LLM.</p>
      <p><b>Гибкая архитектура под задачу.</b> CNN для картинок, RNN/Transformer для последовательностей, GNN для графов, attention для long-range зависимостей. Готовые блоки (<a class="glossary-link" onclick="App.selectTopic('glossary-dropout')">Dropout</a>, <a class="glossary-link" onclick="App.selectTopic('glossary-batchnorm')">BatchNorm</a>, residual connections) собираются как LEGO.</p>
      <p><b>Transfer learning из коробки.</b> Предобученный ResNet/BERT/GPT переносится на новую задачу с тысячей примеров — то, что классическим моделям недоступно в принципе. Это радикально снижает порог входа в deep learning.</p>
      <p><b>End-to-end обучение.</b> Одна loss-функция, один градиентный спуск — все слои учатся совместно. Не надо склеивать pipeline из 5 отдельно обученных блоков, каждый со своими ошибками.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Жадные до данных.</b> Нейросеть на 10M параметров с 500 обучающими примерами почти всегда переобучится. Классические методы (LR, RF, GBM) работают на том же датасете и часто дают лучший результат. Пока у тебя меньше ~10k размеченных примеров — deep learning в 9 случаях из 10 не нужен.</p>
      <p><b>Дорогие вычисления.</b> Обучение серьёзной модели требует GPU/TPU, десятки часов и сотни долларов за один прогон. Один эксперимент с LR на той же задаче — минуты CPU. На малых проектах это убивает скорость итерации.</p>
      <p><b>Чёрный ящик.</b> Понять, почему сеть приняла именно это решение, сложно даже с SHAP, LIME или Grad-CAM. В регулируемых отраслях (банки, медицина) это часто дисквалифицирует нейросеть в пользу дерева или линейной модели.</p>
      <p><b>Легко переобучаются без регуляризации.</b> Нужен <a class="glossary-link" onclick="App.selectTopic('glossary-dropout')">dropout</a>, weight decay, <a class="glossary-link" onclick="App.selectTopic('glossary-early-stopping')">early stopping</a>, data augmentation — и всё это надо тюнить. Забыл что-то из этого — модель запоминает train и проваливает test.</p>
      <p><b>Много гиперпараметров.</b> Learning rate, размер батча, архитектура, оптимизатор, регуляризация, schedule — всё это надо подбирать. На малых задачах тюнинг стоит дороже самой задачи.</p>
      <p><b>Часто проигрывают бустингу на табличных данных.</b> XGBoost, LightGBM, CatBoost стабильно побеждают нейросети на классических tabular benchmarks. Это подтверждено на Kaggle сотни раз — нейросети для таблиц почти никогда не лучший выбор.</p>

      <h3>🧭 Когда брать нейросеть — и когда точно не стоит</h3>
      <table>
        <tr><th>✅ Бери нейросеть когда</th><th>❌ НЕ бери когда</th></tr>
        <tr>
          <td>Задача — изображения, тексты, аудио, видео или графы, где классические модели не умеют</td>
          <td>Данные табличные со смешанными типами — Gradient Boosting почти всегда лучше</td>
        </tr>
        <tr>
          <td>Есть десятки тысяч размеченных примеров (или хороший предобученный чекпоинт для transfer)</td>
          <td>У тебя меньше 1000 примеров — возьми LR, SVM или RF</td>
        </tr>
        <tr>
          <td>Нужна end-to-end архитектура без ручных признаков — представления учатся сами</td>
          <td>Нужна интерпретируемость для регулятора или бизнес-аналитика — возьми дерево</td>
        </tr>
        <tr>
          <td>Есть GPU/TPU и бюджет на обучение и тюнинг гиперпараметров</td>
          <td>Жёсткие ограничения по latency/памяти (edge-устройства) без возможности квантизации</td>
        </tr>
        <tr>
          <td>Нужен transfer learning с предобученной модели (ImageNet, BERT, GPT)</td>
          <td>Нужен быстрый baseline за 5 минут, чтобы понять «есть ли вообще сигнал»</td>
        </tr>
        <tr>
          <td>Нелинейные зависимости, взаимодействия признаков, многоуровневая иерархия</td>
          <td>Простая линейная зависимость — LR даст ту же точность за тысячную долю времени</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы — что взять вместо и почему</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('gradient-boosting')">Gradient Boosting</a> (XGBoost/LightGBM/CatBoost)</b> — если данные табличные. Быстрее, точнее, проще в тюнинге и интерпретации.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('random-forest')">Random Forest</a></b> — если нужен надёжный baseline без тюнинга. Хорошо работает на малых и средних датасетах.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('logistic-regression')">Логистическая регрессия</a></b> — если важна интерпретируемость и зависимость линейна. Гарантированные вероятности, стабильность, объяснимость.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('transfer-learning')">Transfer learning</a> поверх предобученной модели</b> — если данных мало, но задача всё равно про картинки или тексты. Компромисс между «обучать сеть с нуля» и «использовать классику».</li>
      </ul>
    `,
