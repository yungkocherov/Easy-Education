...
for k in [1, 3, 5]:
    ...
    applications: `
      ...
      <ul>
        <li><b>Стриминг видео (Netflix, YouTube, Кинопоиск).</b> Главная лента, «Вам может понравиться», продолжить просмотр. Netflix публично заявлял, что ~80% просмотров идут из рекомендаций — для них это не украшение, а основной driver retention и CAC.</li>
        <li><b>Музыкальные стриминги (Spotify, Яндекс.Музыка, Apple Music).</b> Discover Weekly, Release Radar, персональные радио. Комбинируют collaborative filtering на прослушиваниях + content-based на аудио-эмбеддингах (CNN по спектрограммам) — чтобы рекомендовать новые треки без истории прослушиваний.</li>
        <li><b>E-commerce (Amazon, Wildberries, Ozon).</b> «С этим товаром покупают», «Похожие товары», персональная главная. Item-item CF от Amazon (статья 2003 года) — один из самых цитируемых industrial-алгоритмов всех времён, и до сих пор в обновлённом виде двигает миллиарды.</li>
        <li><b>Ленты контента (TikTok, Дзен, Instagram Reels).</b> Ранжирование бесконечной ленты — чистый recsys с implicit feedback (время просмотра, лайки, replay, скип). TikTok — учебный пример: deep two-tower модели + real-time обучение на поведении.</li>
        <li><b>Социальные связи (ВКонтакте, Facebook, LinkedIn).</b> «Люди, которых вы можете знать», «Группы для вас». Тут расчёт не на сходство потребления, а на графовую близость и common neighbors — отдельная ветка recsys (graph-based).</li>
        <li><b>Job matching (LinkedIn, hh.ru, Glassdoor).</b> Подбор вакансий по резюме и истории откликов. Two-sided marketplace: рекомендовать надо сразу обеим сторонам — кандидату вакансии и рекрутеру кандидатов. Классика hybrid content + CF.</li>
        <li><b>Реклама и CTR prediction.</b> Выбор баннера/объявления, которое покажется конкретному пользователю в конкретный момент. Формально не «рекомендация», но технологически — тот же стек: embeddings, two-tower, ANN-поиск, ранжирование.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Collaborative filtering не требует признаков items.</b> Это самая мощная идея recsys: «тебе понравится то, что понравилось людям с похожими вкусами». Не надо описывать каждый фильм жанрами и актёрами — достаточно матрицы оценок. Именно поэтому CF побеждает на платформах с миллионами items, где feature engineering физически невозможен.</p>
      <p><b>Находит неожиданные рекомендации (serendipity).</b> Content-based зациклен на «больше того же»: посмотрел боевик → ещё 20 боевиков. CF через латентные факторы ловит скрытые связи — «люди, смотрящие этот боевик, также смотрят артхаусную драму» — и открывает новый контент. Именно serendipity делает ленту ценной, а не просто повторяющей очевидное.</p>
      <p><b>Content-based работает для новых items без cold start.</b> Только что загрузил трек на Spotify — CF его никогда не покажет (никто ещё не лайкал). Content-based по аудио-эмбеддингам покажет сразу. Именно поэтому в проде почти всегда hybrid: CF для стабильного ядра, content для новинок и длинного хвоста.</p>
      <p><b>Matrix factorization масштабируется на миллионы пользователей и items.</b> Вместо $O(n^2)$ user-user сходств хранятся компактные латентные векторы ($k = 50-200$). Рекомендация сводится к dot-product или ANN-поиску (FAISS, HNSW) — миллисекунды на запрос. Смотри подробнее в статье про <a class="glossary-link" onclick="App.selectTopic('glossary-matrix-factorization')">матричную факторизацию</a>.</p>
      <p><b>Two-tower neural модели дают state-of-the-art для больших каталогов.</b> Одна башня кодирует пользователя, вторая — item, скалярное произведение = оценка. Учатся на implicit feedback через sampled softmax. Это фундамент YouTube/TikTok recsys и одновременно читаемый, масштабируемый фреймворк.</p>
      <p><b>Хороший recsys напрямую двигает retention и LTV.</b> В отличие от классификаторов, где улучшение accuracy на 1% часто незаметно бизнесу, у recsys +1% к CTR или watch time переводится в миллионы выручки. Именно поэтому в больших продуктах recsys — это целые команды, а не одна модель.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Cold start — главная боль всех recsys.</b> Новый пользователь без истории → нечего рекомендовать через CF. Новый item без лайков → никогда не попадёт в выдачу. Решения (content-based для items, onboarding-опросник для пользователей, explore/exploit) — это не «дополнения», а обязательная часть продакшен-пайплайна. Игнорировать нельзя.</p>
      <p><b>Popularity bias и фильтр-пузыри.</b> CF рекомендует популярное, потому что популярное видят и лайкают все. Длинный хвост (90% каталога) никогда не попадает в выдачу → создатели уходят → платформа теряет разнообразие. Борьба: re-ranking, diversity penalty, explore-политики — и всё равно проблема не решается «в ноль».</p>
      <p><b>Простой popularity baseline часто обыгрывает сложный CF.</b> Если у тебя 1000 пользователей и 100 items — нет смысла в neural recsys, просто показывай топ-10 по популярности, лично фильтруй. Это не шутка: на MovieLens-small популярность реально даёт competitive NDCG@10 против ALS. CF имеет смысл начиная с десятков тысяч взаимодействий на активного пользователя.</p>
      <p><b>Sparsity matrix: обычно заполнена на &lt;1%.</b> Типичный пользователь взаимодействовал с 50 items из миллиона. Стандартные методы на такой sparsity работают плохо — нужны implicit feedback-модели (ALS, BPR), сэмплирование негативов, регуляризация. Без этого — мусор на выходе.</p>
      <p><b>Evaluation — отдельная дисциплина.</b> Offline-метрики (NDCG, recall@k, hit rate) слабо коррелируют с реальной бизнес-метрикой (watch time, CTR, revenue). А/B-тест обязателен — и нередко модель «лучше» в offline, но «хуже» в online. Это не баг, а фундаментальное свойство recsys: train/test split не отражает feedback loop.</p>
      <p><b>Feedback loop и алгоритмическая предвзятость.</b> Что модель показывает → на чём она учится → что она покажет завтра. Если ранжирование плохое, оно самоусиливается. Нужны: принудительный explore (ε-greedy, Thompson sampling), дебайсинг (IPS), мониторинг diversity. Без этого recsys медленно деградирует в «показывай одни и те же 10 вещей всем».</p>

      <h3>🧭 Когда применять — и когда не стоит</h3>
      <table>
        <tr><th>✅ Строй RecSys когда</th><th>❌ НЕ надо когда</th></tr>
        <tr>
          <td>Каталог большой (10k+ items), пользователи активны (десятки взаимодействий) — CF/neural имеет смысл</td>
          <td>Каталог маленький (&lt;1000 items) — простая популярность или ручной редакторский отбор победят любой ML</td>
        </tr>
        <tr>
          <td>Есть implicit feedback (просмотры, клики, покупки) в больших объёмах — именно на нём учатся ALS, BPR, two-tower</td>
          <td>Данных о взаимодействиях очень мало (&lt;10 на пользователя) — cold start у всех, лучше content-based или ручные правила</td>
        </tr>
        <tr>
          <td>Бизнес-метрика реально зависит от персонализации (retention, watch time, CTR) — recsys окупится</td>
          <td>Задача — поиск по явному запросу, а не открытие нового — там нужен search, а не recsys</td>
        </tr>
        <tr>
          <td>Есть ресурсы на A/B-тесты, мониторинг diversity, борьбу с feedback loop</td>
          <td>Нет команды для offline + online evaluation — запустишь без A/B → получишь регресс, которого не заметишь</td>
        </tr>
        <tr>
          <td>Нужен hybrid для нового контента: content-based закрывает cold start items, CF — стабильное ядро</td>
          <td>Интерпретируемость критична (регулятор требует объяснение): CF и neural recsys — чёрные ящики</td>
        </tr>
        <tr>
          <td>Two-sided marketplace (работа, знакомства, недвижимость): рекомендовать надо обеим сторонам</td>
          <td>Пользователи меняют интересы слишком быстро (новости за минуту устаревают) — нужны online learning и session-based модели, а не классика CF</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы</h3>
      <ul>
        <li><b>Popularity baseline (топ по количеству взаимодействий)</b> — всегда бейзлайн номер один. Если твой recsys его не обыгрывает в A/B — что-то не так. Для маленьких каталогов часто достаточно.</li>
        <li><b>Content-based + <a class="glossary-link" onclick="App.selectTopic('glossary-cosine-similarity')">cosine similarity</a> на эмбеддингах</b> — решает cold start для items, интерпретируем, не требует истории пользователей. Хорошо для нишевых каталогов и новых пользователей.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('glossary-matrix-factorization')">Matrix Factorization</a> (ALS, SVD, BPR)</b> — классика CF, до сих пор боевой выбор для многих задач. Масштабируется, даёт good-enough качество, понятен и отлаживается.</li>
        <li><b>Two-tower neural network</b> — state-of-the-art для больших каталогов и implicit feedback. Обучается end-to-end, поддерживает боковые признаки, легко деплоится через ANN-поиск.</li>
        <li><b>Ручная редакторская подборка</b> — не шутка. Для небольших каталогов редактор человеческого вкуса обыгрывает ML. Также нужна для fresh content, safety и brand-критичных позиций.</li>
      </ul>
    `,
