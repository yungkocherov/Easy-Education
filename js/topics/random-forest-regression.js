...
plt.show()</code></pre>
    ...
    applications: `
      ...
      <ul>
        <li><b>Оценка недвижимости.</b> Zillow Zestimate и аналоги используют RF как один из ключевых компонентов: площадь, локация, этаж, возраст дома — всё смешанных типов, с выбросами (особняки), с пропусками (не всегда известен ремонт). RF ест это без предобработки и даёт устойчивую оценку.</li>
        <li><b>Прогноз спроса и расхода в ритейле.</b> Когда нужна модель, которая не разваливается от выбросов (чёрная пятница, аномальные дни) и работает на сотнях региональных магазинов без индивидуального тюнинга каждого.</li>
        <li><b>Медицинские прогнозы.</b> Длительность госпитализации, дозировка препарата, риск осложнений — задачи, где нужна устойчивость к разнообразию пациентов и объяснимость через feature importance (какие показатели важны в модели).</li>
        <li><b>Биоинформатика и QSAR.</b> Предсказание свойств молекул, активности лигандов, структуры белков — тысячи дескрипторов, сотни–тысячи объектов. RF с <code>max_features='sqrt'</code> — классический подход, упомянутый в сотнях публикаций.</li>
        <li><b>Агротех и экология.</b> Прогноз урожайности по спутниковым данным, погоде и почвам. Данные шумные и смешанные, требуется устойчивость, и часто важнее не точность, а интерпретируемость факторов.</li>
        <li><b>Feature importance для EDA.</b> До постройки серьёзной модели часто обучают RF-регрессор просто чтобы понять, какие признаки стоит вообще использовать, а какие — шум.</li>
        <li><b>Baseline для регрессионных задач.</b> «Давайте сначала RF запустим» — стандартный первый шаг на любой табличной регрессии. Если твоя новая модель не бьёт этот бейзлайн, что-то не так.</li>
      </ul>

      <h3>✅ Сильные стороны — и почему они важны</h3>
      <p><b>Устойчивость к выбросам в целевой переменной.</b> Один аномально дорогой дом не смещает все предсказания — он влияет только на листья, где оказался. В отличие от линейной регрессии с MSE, где один выброс тянет всю прямую. Это критично для реальных данных, где выбросы — норма.</p>
      <p><b>Автоматический захват нелинейностей и взаимодействий.</b> Зависимость «цена = площадь × коэф_района × нелинейность_этажа» RF строит сам через последовательные разбиения. Линейной регрессии пришлось бы явно добавлять произведения и полиномы.</p>
      <p><b>Нет необходимости масштабировать признаки.</b> Смешивай квадратные метры (десятки–сотни) с ценой в рублях (миллионы) — дереву плевать, оно работает с порогами. Экономит этап препроцессинга, убирает класс ошибок «забыл StandardScaler на тесте».</p>
      <p><b>OOB-оценка заменяет валидацию.</b> На маленьких данных (&lt; 1000 строк) потеря 20% на тест = потеря точности модели. OOB использует те самые 37% данных, что не вошли в bootstrap каждого дерева — получаешь честную оценку бесплатно.</p>
      <p><b>Почти не переобучается с ростом $T$.</b> Увеличение числа деревьев только улучшает (до плато) — в отличие от бустинга, где без early stopping ты загонишь модель в оверфит. Спокойно ставишь $T=500$ и забываешь.</p>
      <p><b>Feature importance из коробки.</b> Видно сразу, какие признаки двигают таргет — помогает в EDA и объяснении модели бизнесу.</p>
      <p><b>Отличная параллелизация.</b> Каждое дерево обучается независимо — <code>n_jobs=-1</code> и обучение линейно ускоряется. На 16-ядерной машине лес из 500 деревьев обучается за минуты.</p>

      <h3>⚠️ Ограничения — и когда они реально бьют</h3>
      <p><b>Не экстраполирует — вообще.</b> Предсказания ограничены диапазоном $[\\min(y_{train}), \\max(y_{train})]$. Если обучал на ценах квартир до 20 млн, а в тесте 50 млн — получишь ~20 млн. Для временных рядов с трендом (рост пользователей, прогноз цен) — катастрофа, модель буквально не может предсказать «больше, чем видела».</p>
      <p><b>Ступенчатые предсказания.</b> RF возвращает среднее по листьям — функция кусочно-постоянная. Для гладких физических зависимостей (температура, давление) это даёт визуально «рваный» прогноз, который плохо смотрится на графиках и вызывает вопросы у заказчика.</p>
      <p><b>Уступает бустингу в точности.</b> На идеально оттюненном XGBoost/LightGBM разница 2-5% RMSE — на Kaggle это разница между медалью и ничем. На продакшене — разница между деньгами и их отсутствием.</p>
      <p><b>Прожорлив по памяти.</b> 500 глубоких деревьев на датасете в миллион строк — это гигабайты RAM. На больших данных уйди на LightGBM, который строит histograms вместо хранения всех точек.</p>
      <p><b>Медленный инференс на больших лесах.</b> Каждое предсказание = обход 500 деревьев. Для real-time API (SLA &lt; 10 мс) это может быть слишком много — либо урезай деревья, либо дистиллируй в меньшую модель.</p>
      <p><b>MDI importance ненадёжна.</b> Sklearn-овский <code>feature_importances_</code> завышает значимость признаков с большим числом уникальных значений. Для production-выводов всегда используй permutation importance или SHAP.</p>

      <h3>🧭 Когда брать Random Forest Regression — и когда точно не стоит</h3>
      <table>
        <tr><th>✅ Бери RF-регрессию когда</th><th>❌ НЕ бери когда</th></tr>
        <tr>
          <td>Нужен сильный baseline для регрессии без тюнинга</td>
          <td>Нужна максимальная точность — используй XGBoost/LightGBM/CatBoost</td>
        </tr>
        <tr>
          <td>Много выбросов в целевой переменной, которые не хочется вручную чистить</td>
          <td>Задача требует экстраполяции за пределы обучающего диапазона — временные ряды с трендом, прогноз роста</td>
        </tr>
        <tr>
          <td>Маленький датасет (&lt; 5000 строк) — бустинг переобучится быстрее</td>
          <td>Миллионы строк — RF жрёт память, LightGBM будет в десятки раз быстрее</td>
        </tr>
        <tr>
          <td>Нужен feature importance для отчёта или отбора признаков</td>
          <td>Требуется гладкий прогноз (физические процессы) — бери GBM или kernel regression</td>
        </tr>
        <tr>
          <td>Смешанные признаки с пропусками, нет времени на preprocessing</td>
          <td>Real-time API с жёсткой latency — 500 деревьев не пролезут</td>
        </tr>
        <tr>
          <td>OOB-валидация важна (мало данных, жалко резать на train/val)</td>
          <td>Зависимость близка к линейной — Ridge/Lasso дадут лучше и быстрее</td>
        </tr>
      </table>

      <h3>🔄 Альтернативы — что взять вместо и почему</h3>
      <ul>
        <li><b><a class="glossary-link" onclick="App.selectTopic('gradient-boosting-regression')">Gradient Boosting Regression</a> (XGBoost/LightGBM/CatBoost)</b> — если нужна максимальная точность. Последовательные деревья с градиентным спуском почти всегда дают +2-5% $R^2$ при грамотном early stopping.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('linear-regression')">Линейная регрессия</a> / Ridge / Lasso</b> — если зависимость близка к линейной, данных очень много, или нужна экстраполяция. Быстрее на порядки, меньше модель, не «застывает» за границами обучающих данных.</li>
        <li><b>ExtraTrees Regressor</b> — если RF слишком медленный, а пара процентов качества не критична. Случайные пороги вместо оптимальных дают ускорение при сопоставимом качестве.</li>
        <li><b><a class="glossary-link" onclick="App.selectTopic('decision-tree-regression')">Decision Tree Regression</a></b> — если нужна одна интерпретируемая «формула»: правило, которое можно показать экспертам и превратить в бизнес-логику.</li>
        <li><b>Gaussian Process / Kernel Ridge</b> — если данных мало (&lt; 500), нужна гладкость и доверительные интервалы «из коробки». RF даёт точки, GP даёт распределение.</li>
      </ul>
    `,
