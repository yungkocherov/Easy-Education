...
for train_idx, val_idx in kf.split(df):
    ...
    math: `
      <h3>📐 Математика Feature Engineering</h3>

      <h4>Стандартизация (Z-score)</h4>
      <div class="math-block">$$z = \\frac{x - \\mu}{\\sigma}, \\quad \\mu = \\frac{1}{n}\\sum_{i=1}^n x_i, \\quad \\sigma = \\sqrt{\\frac{1}{n}\\sum_{i=1}^n (x_i - \\mu)^2}$$</div>
      <p>После преобразования: $\\mathbb{E}[z] = 0$, $\\mathrm{Var}(z) = 1$. Распределение формы не меняет — только центрирует и нормирует.</p>

      <h4>Min-Max нормализация</h4>
      <div class="math-block">$$x' = \\frac{x - x_{\\min}}{x_{\\max} - x_{\\min}} \\in [0, 1]$$</div>
      <p>Чувствительность к выбросам: если $x_{\\max}$ — выброс, все остальные значения сжимаются к нулю.</p>

      <h4>Mutual Information</h4>
      <div class="math-block">$$MI(X; Y) = \\sum_{x}\\sum_{y} p(x,y) \\log\\frac{p(x,y)}{p(x)p(y)}$$</div>
      <p>MI = 0 если X и Y независимы. MI &gt; 0 если знание X уменьшает неопределённость Y. В отличие от корреляции Пирсона, улавливает <b>нелинейные</b> зависимости.</p>

      <h4>Корреляция Пирсона</h4>
      <div class="math-block">$$r = \\frac{\\sum_{i=1}^n (x_i - \\bar{x})(y_i - \\bar{y})}{\\sqrt{\\sum(x_i-\\bar{x})^2 \\cdot \\sum(y_i-\\bar{y})^2}} \\in [-1, 1]$$</div>
      <p>Улавливает только линейные зависимости. Два признака с $|r| > 0.95$ дублируют информацию — один из них можно убрать.</p>

      <h4>One-Hot Encoding и мультиколлинеарность</h4>
      <p>Если категория имеет $k$ значений и создаётся $k$ бинарных столбцов, то их сумма всегда равна 1:</p>
      <div class="math-block">$$\\sum_{j=1}^k x_{ij} = 1 \\quad \\forall i$$</div>
      <p>Это <b>линейная зависимость</b> — матрица $X^TX$ становится вырожденной. Поэтому один столбец убирают ($k-1$ dummy variables). Для Random Forest и нейросетей это не проблема, но для линейных моделей важно.</p>

      <h4>Polynomial Features</h4>
      <p>Для $d$ признаков степени $p$ количество создаваемых признаков:</p>
      <div class="math-block">$$\\binom{d + p}{p} = \\frac{(d+p)!}{d!\\, p!}$$</div>
      <p>Для $d=10$, $p=2$: $\\binom{12}{2} = 66$ признаков. Для $d=100$, $p=2$: 5151 признак. Быстро растёт!</p>
    `,
